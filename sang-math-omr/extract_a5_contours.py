...
for cnt in contours_w:
    ...
    aspect_ratio = float(w)/h
    area = cv2.contourArea(cnt)
    # A bubble has radius ~ 12px -> w~24px -> area ~ 450
    # A5 scaled 88% -> w~21px -> area ~ 340
    # So 150 < area < 550
    if 0.8 < aspect_ratio < 1.2 and 150 < area < 550:
        # verify it's a circle by checking contour area vs bounding box area
        if area / (w*h) > 0.6: 
            bubbles.append({"x": x+w//2, "y": y+h//2, "r": w//2})
...
print(f"{name}: Found {len(unique_coords)} bubbles using contours.")
