...
async function verifyTicket(request, env, expectedOperation) {
  ...
  const key = await crypto.subtle.importKey(
    "raw",
    new TextEncoder().encode(env.OMR_STORAGE_HMAC_SECRET),
  ...
  if (payload.v !== 1 || payload.op !== expectedOperation || !Number.isInteger(payload.exp)) {
  ...
  const requiredPrefix = `omr/users/${payload.uid}/`;
  if (!String(payload.key || "").startsWith(requiredPrefix) || payload.key.includes("..")) {
    throw new Error("invalid-ticket");
  }
  return payload;
}
...
async function putObject(request, env, ticket) {
  ...
  if ((contentLength !== null && contentLength !== expectedBytes) || contentType !== ticket.contentType) {
  ...
  if (ticket.sha256 && checksum !== ticket.sha256) return json(request, env, { error: "checksum-mismatch" }, 400);
  const object = await env.OMR_BUCKET.put(ticket.key, bytes, {
...
async function headObject(request, env, ticket) {
  const object = await env.OMR_BUCKET.head(ticket.key);
...
async function getObject(request, env, ticket) {
  const object = await env.OMR_BUCKET.get(ticket.key);
  if (!object) return json(request, env, { error: "not-found" }, 404);
...
async function deleteObject(request, env, ticket) {
  await env.OMR_BUCKET.delete(ticket.key);
...
export default {
  async fetch(request, env) {
    if (request.method === "OPTIONS") {
      const origin = request.headers.get("Origin");
      if (origin !== env.ALLOWED_ORIGIN) return response(request, env, null, { status: 403 });
    ...
    try {
      ...
      const status = error?.message === "expired-ticket" ? 401 : 403;
      return json(request, env, { error: error?.message || "forbidden" }, status);
    }
  },
};
