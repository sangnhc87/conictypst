...
// Hyperlink counters and icons
#let _hl-q-cnt = counter("se-hl-q")
#let _hl-sol-cnt = counter("se-hl-sol")

#let _hl-icon-to-sol(dest, clr) = {
  link(dest)[#box(baseline: 20%, text(fill: clr, size: 0.9em)[↓])]
}

#let _hl-icon-to-q(dest, clr) = {
  link(dest)[#box(baseline: 20%, text(fill: clr, size: 0.9em)[↑])]
}
...
// ── Lời giải block ────────────────────────────────────────
#let _sol(s, a, hl-id: none, show-hyperlink: false) = {
  let should-hl = show-hyperlink and hl-id != none
  let header = if should-hl {
    [#text(weight: "bold", fill: a)[Lời giải.]#h(0.15em)#_hl-icon-to-q(label("hl-q-" + str(hl-id)), a)]
  } else {
    text(weight: "bold", fill: a)[Lời giải.]
  }
  let sol-block = [
...
#let mcq(
  ...
  let _should-hl = show-hyperlink and loigiai != none
  let _hl-id = if _should-hl { hl_cnt.get().first() } else { none }
  let _q-label-with-hl = if _should-hl and mode == "dethi" and _hl-id != none {
    [#q-label#h(0.18em)#_hl-icon-to-sol(label("hl-sol-" + str(_hl-id)), accent)]
  } else {
    q-label
  }
  ...
  let stem-row = if fig != none and (fig-pos == "right" or fig-pos == "left") {
    if fig-pos == "right" {
      grid(
        ...
        [#_q-label-with-hl #_stem-w-tags], fig,
      )
    } else {
      grid(
        ...
        fig, [#_q-label-with-hl #_stem-w-tags],
      )
    }
  } else if fig != none and fig-pos == "center" {
    [#_q-label-with-hl #_stem-w-tags]
    v(0.4em)
  ...
  [
    #if _should-hl and mode == "dethi" and _hl-id != none {
    ...
    #_question-frame(
      [
        ...
        #pad(left: option-indent)[#opts-r]
          ...
          v(0.7em)
          _sol(loigiai, accent, hl-id: _hl-id, show-hyperlink: show-hyperlink)
        }
      ],
      below: 1.05em,
      boxed: boxed,
  ...
  let stem-row = if fig != none and (fig-pos == "right" or fig-pos == "left") {
    if fig-pos == "right" {
      ...
      grid(
        ...
        column-gutter: 14pt,
        align: (center + top, left + top),
        fig, [#_q-label-with-hl #stem],
      )
    }
  } else if fig != none and fig-pos == "center" {
    [#_q-label-with-hl #stem]
    v(0.4em)
    _fig-scaled-center(fig, fig-width)
  } else {
    [#_q-label-with-hl #stem]
