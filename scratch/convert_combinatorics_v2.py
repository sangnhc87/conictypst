...
theory_positions = []
while True:
    pos = content.find("#theory-box[", idx)
    if pos == -1:
        break
    start_bracket = pos + len("#theory-box")
    end_bracket = -1
    stack = 0
    for i in range(start_bracket, len(content)):
        if content[i] == '[':
            ...
            if stack == 0:
                ...
                break
    if end_bracket != -1:
        theory_positions.append((pos, end_bracket))
        idx = end_bracket + 1
    else:
        idx = pos + 1

for start, end in reversed(theory_positions):
    after_text = content[end+1:end+150]
    if "== Bài Tập Minh Họa" not in after_text:
        content = content[:end+1] + "\n\n== Bài Tập Minh Họa" + content[end+1:]

# 4. Now find and parse all `#example-box` using character scanning
idx = 0
example_boxes = []
while True:
    # Match `#example-box(n: "...")[` or `#example-box[`
    match = re.search(r'#example-box(?:\(n:\s*\"(.*?)\"\))?\[', content[idx:])
    if not match:
        break
    
    start_pos = idx + match.start()
    n_attr = match.group(1) or ""
    
    start_bracket = idx + match.end() - 1
    assert content[start_bracket] == '['
    ...
    if end_bracket != -1:
        example_boxes.append((start_pos, end_bracket, n_attr))
        idx = end_bracket + 1
    else:
        idx = start_pos + 1

print(f"Found {len(example_boxes)} example boxes.")

# Process and replace examples in reverse order
for start, end, n_attr in reversed(example_boxes):
    first_bracket = content.find('[', start)
    body = content[first_bracket+1:end].strip()
...
# 5. Insert new Example 4 problems at key locations
new_ex_chude6_7 = """
#tln(
  "4 (Khó - Giải bằng 2 cách)",
  [
...
#tln(
  ...
  [frac((1+sqrt(2))^(n+1) - (1-sqrt(2))^(n+1), 2sqrt(2))],
  loigiai: [
    #ppgiai[
    ...
    #cach1-box[
      *Cách 1: Thiết lập truy hồi và giải bằng Phương trình đặc trưng*
      ...
      - *Khả năng 1:* Viên gạch cuối là gạch $1 times 1$. Vì có 2 màu (đỏ hoặc xanh) nên có 2 cách chọn gạch cuối. Phía trước còn lại đường đi dài $n-1$ có $a_(n-1)$ cách lát.
        Số cách lát trong trường hợp này là $2 a_(n-1)$.
      - *Khả năng 2:* Viên gạch cuối là gạch $1 times 2$ màu vàng. Chỉ có 1 cách đặt. Phía trước còn lại đường đi dài $n-2$ có $a_(n-2)$ cách lát.
...
# Let's insert the new Example 4 problems at key locations
