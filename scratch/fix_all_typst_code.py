...
# 1. Replace the Theory Block + Guide
theory_pattern = r'(#method-box\[\s*Công thức đóng góp của một phương án phân bổ cột.*?\]\s*\n\s*#rev-box\(title:\s*"🔑 Quy Trình Biện Luận Nghiệm Cho Hệ Phương Trình Cấu Hình Cột"\)\[.*?\])'
match_theory = re.search(theory_pattern, content, re.DOTALL)

if match_theory:
    target_theory = match_theory.group(1)
    print("Found theory block!")
else:
    # Fallback to single method-box search if the rev-box wasn't added yet
    theory_pattern_single = r'(#method-box\[\s*Công thức đóng góp của một phương án phân bổ cột.*?\])'
    match_theory_single = re.search(theory_pattern_single, content, re.DOTALL)
    if match_theory_single:
        target_theory = match_theory_single.group(1)
        print("Found theory block (single)!")
    else:
        target_theory = None
        print("Could not find theory block!")

if target_theory:
    theory_replacement = """#method-box[
...
if match16:
  ...
  *🟣 Cách 2: Phương pháp lập cấu hình cột tổng quát (Biện luận hệ phương trình)*
  ...
  - *Trường hợp 2.1: Có một biến $t_{i j} = 2$ (cấu hình hai cặp song song)*
    ...
    1. $ {2 times (A, B), 2 times (C, D)} $ $arrow.r$ Số cách gán $M = frac(4!, 2! 2!) = 6$.
    2. $ {2 times (A, C), 2 times (B, D)} $ $arrow.r$ Số cách gán $M = 6$.
    3. $ {2 times (A, D), 2 times (B, C)} $ $arrow.r$ Số cách gán $M = 6$.
    Mỗi cấu hình đóng góp: $6 times (2! 2! 2! 2!) times 2^4 = 6 times 16 times 16 = bold(1536)$ cách.
    $arrow.r$ Đóng góp của TH2.1: $3 times 1536 = bold(4608)$ cách.
  ...
  - *Trường hợp 2.2: Tất cả các biến $t_{i j} <= 1$ (cấu hình một vòng lớn 4 đỉnh)*
    ...
    1. Vòng $A-B-C-D-A$ $arrow.r$ Cấu hình $ {1 times (A, B), 1 times (B, C), 1 times (C, D), 1 times (A, D)} $.
    2. Vòng $A-B-D-C-A$ $arrow.r$ Cấu hình $ {1 times (A, B), 1 times (B, D), 1 times (C, D), 1 times (A, C)} $.
    3. Vòng $A-C-B-D-A$ $arrow.r$ Cấu hình $ {1 times (A, C), 1 times (B, C), 1 times (B, D), 1 times (A, D)} $.
    Với mỗi cấu hình, số cách gán kiểu cột vào các vị trí là $M = frac(4!, 1! 1! 1! 1!) = 24$.
    Mỗi cấu hình đóng góp: $24 times (2! 2! 2! 2!) times 2^4 = 24 times 16 times 16 = bold(6144)$ cách.
    $arrow.r$ Đóng góp của TH2.2: $3 times 6144 = bold(18432)$ cách.
...
if match17:
    ...
    # We replaced \\times with times in the math blocks below
  ...
  *🟣 Cách 2: Phương pháp lập cấu hình cột tổng quát (Biện luận hệ phương trình)*
  ...
  - *Trường hợp 2.1: $t_{C D} = 1$*
    ...
    + Số cách gán kiểu cột vào 3 vị trí: $M = frac(3!, 2! 1!) = 3$ cách.
    + Đóng góp số cách xếp: $N_1 = 3 times (2! 2! 1! 1!) times 2^3 = 3 times 4 times 8 = bold(96)$ cách.
  ...
  - *Trường hợp 2.2: $t_{C D} = 0$*
    ...
    3. *Khả năng 3:* $t_{A C} = 1, t_{B D} = 1$ $arrow.r$ $t_{B C} = 0, t_{A D} = 0$. Thay vào A ta được $t_{A B} + 1 = 2 \\implies t_{A B} = 1$. Thay vào B ta được $1 + 0 + 1 = 2$ (thỏa mãn).
       Ta thu được cấu hình: $ {1 times (A, B), 1 times (A, C), 1 times (B, D)} $.
       + Số cách gán $M = frac(3!, 1! 1! 1!) = 6$.
       + Đóng góp: $N_{2a} = 6 times 4 times 8 = bold(192)$ cách.
    4. *Khả năng 4:* $t_{B C} = 1, t_{A D} = 1$ $arrow.r$ $t_{A C} = 0, t_{B D} = 0$. Thay vào A ta được $t_{A B} + 1 = 2 \\implies t_{A B} = 1$. Thay vào B ta được $1 + 1 + 0 = 2$ (thỏa mãn).
       Ta thu được cấu hình: $ {1 times (A, B), 1 times (B, C), 1 times (A, D)} $.
       + Số cách gán $M = frac(3!, 1! 1! 1!) = 6$.
       + Đóng góp: $N_{2b} = 6 times 4 times 8 = bold(192)$ cách.

  #rev-box(title: "🧩 Bảng tính số cách xếp cho từng cấu hình cột (Ví dụ 1.7)")[
    #align(center)[
      #table(
        ...
        [$2 times (A, B) \\ 1 times (C, D)$], [$frac(3!, 2! 1!) = 3$], [$2! 2! 1! 1! = 4$], [$2^3 = 8$], [$3 times 4 times 8 = bold(96)$],
        [$1 times (A, B) \\ 1 times (A, C) \\ 1 times (B, D)$], [$frac(3!, 1! 1! 1!) = 6$], [$4$], [$8$], [$6 times 4 times 8 = bold(192)$],
        [$1 times (A, B) \\ 1 times (B, C) \\ 1 times (A, D)$], [$frac(3!, 1! 1! 1!) = 6$], [$4$], [$8$], [$6 times 4 times 8 = bold(192)$]
      )
    ]
    $arrow.r$ Tổng số cách xếp là: $N = 96 + 192 + 192 = bold(480)$ cách.
  ]
]"""
...
print("All code successfully updated and checked!")
