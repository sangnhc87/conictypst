...
cleaned_dir = os.path.join(exams_dir, "cleaned")
...
category_mapping = {
    ...
    "CD-QuyHoachDong-DemDuongDi.typ": ("Khung tư duy Quy hoạch động & Đếm đường đi (DP on DAG)", 1, "Phương pháp hệ thống thiết lập State và Transition giải quyết mọi bài toán đếm đường đi trên lưới, đồ thị có ràng buộc."),
    "CD-XepGhe-KhongKe.typ": ("Bản chất & Tổng quát hóa bài toán xếp ghế không kề", 1, "So sánh chuyên sâu 2 phương pháp Khung xương Tổ hợp và Bù trừ Cặp kề trùng (Adjacent-Pair PIE) cho các bài toán xếp ghế hàng đơn và 2 dãy ghế song song đối diện."),
    ...
    "Xác-Suât-XTổng-Đối Kháng.typ": ("Xác suất đối kháng & Thi đấu luân lưu", 2, "Tính toán xác suất chiến thắng trong các trận đấu thể thao, loạt sút luân lưu với phong độ thay đổi."),
    ...
    "CD-Kinh-Te-Tich-Phan.typ": ("Tích phân trong Kinh tế: Thặng dư CS & PS", 4, "Tính thặng dư tiêu dùng (CS) và thặng dư sản xuất (PS) tại điểm cân biến cung cầu bằng tích phân."),
...
def strip_page_settings(content):
    pos = 0
    while True:
        idx = content.find("set page", pos)
        if idx == -1:
            break
        ...
        if count == 0:
            content = content[:start_idx] + content[j+1:]
            pos = start_idx
        else:
            pos = idx + 8
...
for filename in category_mapping.keys():
    src_path = os.path.join(exams_dir, filename)
    ...
    # Strip page settings
    cleaned = strip_page_settings(content)
    
    # Adjust relative paths in imports (e.g. "../" to "../../")
    cleaned = re.sub(r'#import\s+["\']\.\./', '#import "../../', cleaned)
    cleaned = re.sub(r'#import\s+["\']@preview/', '#import "@preview/', cleaned) # keep @preview intact
    
    with open(dest_path, "w", encoding="utf-8") as f:
        f.write(cleaned)

print(f"Cleaned {len(category_mapping)} files and saved them in {cleaned_dir}")
...
out.append('')
out.append('// ═══════════════════════════════════════════════')
out.append('// CẤU HÌNH TRANG CHỦ')
out.append('// ═══════════════════════════════════════════════')
out.append('#set page(')
out.append('  paper: "a4",')
out.append('  margin: (x: 1.4cm, y: 1.8cm),')
out.append('  numbering: "1",')
out.append('  header: context {')
out.append('    let page-num = counter(page).get().first()')
out.append('    if page-num > 2 {')
out.append('      grid(')
out.append('        columns: (1fr, 1fr),')
out.append('        align(left)[')
out.append('          #text(size: 8.5pt, fill: rgb("#7f8c8d"), style: "italic")[Bộ tài liệu chuyên sâu HSG & ĐGNL]')
out.append('        ],')
out.append('        align(right)[')
out.append('          #link(<muc-luc>)[#text(size: 8.5pt, fill: rgb("#1A237E"), weight: "bold")[Quay lại Mục lục 📑]]')
out.append('        ]')
out.append('      )')
out.append('      v(2pt)')
out.append('      line(length: 100%, stroke: 0.5pt + rgb("#e0e0e0"))')
out.append('    }')
out.append('  },')
out.append('  footer: align(center, context {')
out.append('    text(size: 9pt, fill: rgb("#7f8c8d"))[Trang #counter(page).display()]')
out.append('  })')
out.append(')')
...
out.append('')
out.append('// ═══════════════════════════════════════════════')
out.append('// ĐỊNH DẠNG MỤC LỤC ĐẸP & MÀU SẮC')
out.append('// ═══════════════════════════════════════════════')
out.append('#show outline.entry: it => {')
out.append('  let h = it.element')
out.append('  let page-num = counter(page).at(h.location()).first()')
out.append('  ')
out.append('  let group-color = rgb("#1A237E")')
out.append('  let group-accent = rgb("#16A085")')
out.append('  ')
out.append('  let preceding-h1s = query(selector(heading.where(level: 1)).before(h.location()))')
out.append('  let h1-text = if preceding-h1s.len() > 0 { repr(preceding-h1s.last().body) } else { repr(h.body) }')
out.append('  ')
out.append('  if "NHÓM 1" in h1-text {')
out.append('    group-color = rgb("#1A237E")')
out.append('    group-accent = rgb("#3F51B5")')
out.append('  } else if "NHÓM 2" in h1-text {')
out.append('    group-color = rgb("#1B5E20")')
out.append('    group-accent = rgb("#4CAF50")')
out.append('  } else if "NHÓM 3" in h1-text {')
out.append('    group-color = rgb("#4A148C")')
out.append('    group-accent = rgb("#9C27B0")')
out.append('  } else if "NHÓM 4" in h1-text {')
out.append('    group-color = rgb("#B71C1C")')
out.append('    group-accent = rgb("#EF5350")')
out.append('  } else if "NHÓM 5" in h1-text {')
out.append('    group-color = rgb("#0D47A1")')
out.append('    group-accent = rgb("#1E88E5")')
out.append('  } else if "NHÓM 6" in h1-text {')
out.append('    group-color = rgb("#E65100")')
out.append('    group-accent = rgb("#FB8C00")')
out.append('  } else if "NHÓM 7" in h1-text {')
out.append('    group-color = rgb("#37474F")')
out.append('    group-accent = rgb("#78909C")')
out.append('  }')
out.append('  ')
out.append('  link(h.location())[')
out.append('    #if it.level == 1 {')
out.append('      v(1.4em, weak: true)')
out.append('      text(fill: group-color, weight: "bold", size: 12pt)[')
out.append('        #h.body')
out.append('        #box(width: 1fr, line(length: 100%, stroke: 1.5pt + group-color))')
out.append('        #page-num')
out.append('      ]')
out.append('    } else {')
out.append('      v(0.5em, weak: true)')
out.append('      text(fill: rgb("#2C3E50"), size: 10pt)[')
out.append('        #h.body')
out.append('        #box(width: 1fr, repeat([ . ]))')
out.append('        #text(fill: group-accent, weight: "bold")[#page-num]')
out.append('      ]')
out.append('    }')
out.append('  ]')
out.append('}')
...
# Custom box configurations just in case
box_definitions = """
...
#let boxed(body) = rect(inset: 3pt, stroke: 0.5pt, radius: 2pt)[#body]
"""
out.append(box_definitions)
out.append('')
...
out.append('    inset: (x: 2cm, y: 2.2cm),')
out.append('    radius: 14pt,')
out.append('  )[')
out.append('    #text(fill: rgb("E3F2FD"), size: 12pt, weight: "bold", tracking: 2pt)[')
out.append('      BỘ TÀI LIỆU CHUYÊN SÂU HSG & ĐGNL')
out.append('    ]')
out.append('    #v(0.8em)')
out.append('    #text(fill: white, size: 24pt, weight: "black")[Tuyển Tập Toàn Bộ 55 Chuyên Đề Nâng Cao]')
out.append('    #v(0.6em)')
out.append('    #text(fill: rgb("E3F2FD"), size: 14pt, weight: "bold")[📚 Trọn bộ Toán nâng cao, Thực tế, Tổ hợp, Xác suất và Giải tích]')
out.append('    #v(0.8em)')
out.append('    #text(fill: rgb("E3F2FD"), size: 11pt, style: "italic")[')
out.append('      "Tổng hợp đầy đủ lý thuyết, ví dụ mẫu trực quan bằng sơ đồ cây và phân tích đa chiều"')
out.append('    ]')
out.append('  ]')
out.append(']')
out.append('')
out.append('#v(1.5em)')
out.append('#outline(title: "Mục Lục Hệ Thống Chuyên Đề", depth: 2, indent: 1.5em) <muc-luc>')
...
# Generate includes grouped pointing to the cleaned subdirectory
for g_id, g_title in groups.items():
    ...
    for idx, (filename, title, desc) in enumerate(topics, 1):
        out.append(f"#pagebreak()")
        out.append(f"== {idx}. {title}")
        out.append(f"*(Tệp nguồn: `{filename}`)*")
        out.append(f"")
        out.append(f"- *Mô tả chuyên đề:* {desc}")
        out.append(f"")
        out.append(f"#[")
        out.append(f"  #show heading: set heading(offset: 2)")
        out.append(f"  #include \"cleaned/{filename}\"")
        out.append(f"]")
        out.append(f"")
...
print(f"Generated unified includes Typst file at {output_typst_path}")
