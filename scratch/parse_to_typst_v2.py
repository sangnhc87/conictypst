...
# Map page numbers to manual overrides for math formulas
page_overrides = {
    ...
    # Page 37: RSA formula
    37: [
        (r"𝑎𝜑\(𝑛\) ≡ 1\(mod 𝑛\)", "$a^(phi(n)) equiv 1 \\ (\"mod\" \\ n)$"),
...
"""
    ...
    for p_num in range(4, len(pages) + 1):
        ...
        for l in lines:
            ...
            # --- 1. Apply multi-character math replacements first ---
            math_multichar = {
                '𝐴𝑖': '$A_i$',
                '𝐴𝑗': '$A_j$',
                '𝐴𝑘': '$A_k$',
            ...
            # Sort keys by length in descending order to avoid prefix collision
            for multichar in sorted(math_multichar.keys(), key=len, reverse=True):
                repl = math_multichar[multichar]
                line_processed = line_processed.replace(multichar, repl)
            
            # --- 2. Apply single-character math replacements ---
            math_chars = {
                '𝑎': '$a$', '𝑏': '$b$', '𝑐': '$c$', '𝑑': '$d$', '𝑒': '$e$', 
                '𝑓': '$f$', '𝑔': '$g$', 'ℎ': '$h$', '𝑖': '$i$', '𝑗': '$j$',
                '𝑘': '$k$', '𝑙': '$l$', '𝑚': '$m$', '𝑛': '$n$', '𝑜': '$o$',
...
print("Typst file version 3 generated successfully!")
