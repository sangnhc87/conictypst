...
with open(log_path, "r", encoding="utf-8") as f:
    for line_num, line in enumerate(f, 1):
        try:
            # Parse line using strict=False to allow control characters
                    ...
                    if func_name == "write_to_file":
                        ...
                        if isinstance(chunks_str, str):
                            try:
                                ...
                                print(f"Line {line_num}: Failed to parse chunks string: {e}")
                                chunks = []
                        else:
                            chunks = chunks_str
...
out_path = "/Users/admin/conictypst/scratch/reconstructed_virtual_v3.typ"
with open(out_path, "w", encoding="utf-8") as out_f:
    out_f.write("\n".join(virtual_file[1:]))
print(f"Saved reconstructed virtual file (v3) to {out_path}")
