...
#example-box(n: 2 (Trung bình))[
  ...
  *Cách 1 (Tính trực tiếp qua số Stirling loại 2):*
  ...
  *Cách 2 (Phân tích theo cấu trúc cỡ nhóm):*
...
#example-box(n: 3 (Khó - Giải bằng 3 cách))[
  ...
  *Cách 2 (Lập công thức truy hồi tr
    $ \int_0^1 (1 + t)^n d t = \left. frac((1 + t)^(n+1), n+1) \right|_0^1 = frac(2^(n+1) - 1, n+1) $
...
#skill-badge(Chủ Đề 14, Nhị Thức Newton Và Biến Đổi Hệ Số, color: c-amber)
...
#example-box(n: 2 (Trung bình))[
  ...
  loigiai: [
    *Cách 1 (Sử dụng phương pháp đổi biến - Vách ngăn):*
    ...
    - Biến $y$ ($y >= 2$): $f_y(t) = t^2 + t^3 + t^4 + ... = frac(t^2, 1-t)$.
    - Biến $z$ ($z >= 3$): $f_z(t) = t^3 + t^4 + t^5 + ... = frac(t^3, 1-t)$.
    - Biến $w$ ($w >= 0$): $f_w(t) = 1 + t + t^2 + ... = frac(1, 1-t)$.
    
    Hàm sinh cho số nghiệm của phương trình là:
    $ f(t) = f_x(t) f_y(t) f_z(t) f_w(t) = frac(t, 1-t) dot frac(t^2, 1-t) dot frac(t^3, 1-t) dot frac(1, 1-t) = frac(t^6, (1-t)^4) $
    Số nghiệm nguyên của phương trình chính là hệ số của $t^(17)$ trong khai triển của $f(t)$:
    $ [t^(17)] f(t) = [t^(17)] frac(t^6, (1-t)^4) = [t^(11)] (1-t)^(-4) = C_(11+4-1)^(11) = C_(14)^3 = 364  nghiệm. $
  ],
)
...
#prob(
  ...
  loigiai: [
    ...
    *Cách 2 (Sử dụng Nguyên lý bù trừ kết hợp vách ngăn):*
    Ta đếm số nghiệm nguyên không âm của phương trình $x_1 + x_2 + x_3 + x_4 = 10$.
    - Tổng số nghiệm không âm tự do (không có giới hạn trên) là:
      $ |S| = C_(10+4-1)^(4-1) = C_(13)^3 = 286 $
    - Gọi $A_i$ là tính chất nghiệm thỏa mãn $x_i >= 4$ với $i in \{1, 2, 3, 4\}$. Ta cần tìm số nghiệm không thỏa mãn bất kỳ tính chất $A_i$ nào. Theo nguyên lý bù trừ:
      $ N = |S| - sum_(i=1)^4 |A_i| + sum_(1 <= i < j <= 4) |A_i inter A_j| - sum_(1 <= i < j < k <= 4) |A_i inter A_j inter A_k| + |A_1 inter A_2 inter A_3 inter A_4| $
...
#prob(
  ...
  loigiai: [
    ...
    *Cách 2 (Sử dụng hàm sinh - Generating Functions):*
    ...
    - $y_i = x_i - x_(i-1) - 1 >= 2$ (với $2 <= i <= 5$) là số lượng số không được chọn nằm giữa $x_(i-1)$ và $x_i$. Do khoảng cách giữa chúng ít nhất là 3 nên phải có ít nhất 2 số không được chọn ở giữa, tức là $y_i >= 2$.
    - $y_6 = 20 - x_5 >= 0$ là số lượng số không được chọn nằm sau $x_5$.
    Tổng số lượng số không được chọn là $20 - 5 = 15$. Ta có phương trình nghiệm nguyên:
    $ y_1 + y_2 + y_3 + y_4 + y_5 + y_6 = 15 $
    Trong đó $y_1, y_6 >= 0$ và $y_2, y_3, y_4, y_5 >= 2$.
    Hàm sinh cho số nghiệm của phương trình là:
