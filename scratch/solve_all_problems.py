    ...
    for x in range(F + 1):
        for z in range(F + 1 - x):
            for t in range(F + 1 - x - z):
                w = F - x - z - t
                # Now we find the range for y:
                # y >= 0
                # y <= E - x - z (since u = E - x - y - z >= 0)
                # y <= G - x - t (since v = G - x - y - t >= 0)
                # y >= E + G + F - R - 2x - z - t (since o = R - E - G - F + 2x + y + z + t >= 0)
                ...
                for y in range(min_y, max_y + 1):
                    ...
                    # Calculate row-configuration count
...
# Run for all three problems:
solve_seating(8, 5, 3, 2)
solve_seating(6, 4, 3, 2)
solve_seating(7, 5, 4, 2)
# An easy one to start:
solve_seating(5, 3, 2, 1)
