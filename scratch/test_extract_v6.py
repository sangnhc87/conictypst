...
def extract_problems_with_solutions_v6(filepath):
    ...
    # Try finding tln, tn, ds first
    pos = 0
    while True:
        m = re.search(r"#(tln|tn|ds)\b", content[pos:])
        if not m:
            break
    ...
    # If none found, fallback to example-box and eg-box
    if not q_matches:
        pos = 0
        while True:
            m = re.search(r"#(example-box|eg-box)\b", content[pos:])
            if not m:
                break
            start_idx = pos + m.start()
            q_type = m.group(1)
    ...
    for idx, (q_type, start_idx) in enumerate(q_matches):
        ...
        if q_type == "tln":
            ...
            if loigiai_idx != -1 and loigiai_idx < next_q_pos:
                ...
                if sol_content:
                    # Parse solution boxes inside the loigiai block to get the specific cach1-box, cach2-box, etc.
                    sol_parts = extract_solution_boxes_in_range(sol_content, 0, len(sol_content))
                    if not sol_parts:
                        sol_parts.append({
                            "type": "loigiai-box",
...
for f in ["CD-HamSinh-GeneratingFunction-content.typ", "CD-HamSinh-LoiBai-NangCao-content.typ"]:
    filepath = os.path.join("/Users/admin/conictypst/typst/exams", f)
    probs = extract_problems_with_solutions_v6(filepath)
    print(f"=== {f} ===")
    print("Num problems extracted:", len(probs))
    for i, p in enumerate(probs):
        print(f"  P{i+1}: q={p['q'][:100]}...")
        print(f"       ans={p['ans']}")
        print(f"       sols count={len(p['solutions'])}")
        if p['solutions']:
            print(f"       first sol={p['solutions'][0]['raw_text'][:200]}...")
