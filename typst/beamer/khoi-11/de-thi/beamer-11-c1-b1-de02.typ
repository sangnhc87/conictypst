// ═══════════════════════════════════════════════════════════════════════════
// BEAMER ĐỀ THI: CHƯƠNG 1. LƯỢNG GIÁC (ĐỀ SỐ 02)
...
#show: lecture-theme.with(
  title: "CHƯƠNG 1. LƯỢNG GIÁC (ĐỀ SỐ 02)",
...
// TN 1
#lt-tn(num: 1, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Bánh xe của một chiếc xe đạp có bán kính $30 " cm"$. Khi xe chạy với vận tốc không đổi, bánh xe quay được $2$ vòng mỗi giây. Số đo góc (tính bằng radian) mà một nan hoa đã quay được trong $5$ giây là],
    (
        [$10pi$],
        [$15pi$],
        True([$20pi$]),
...
// TN 2
#lt-tn(num: 2, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Một chiếc đu quay vòng tròn có đường kính $40"m"$. Nếu đu quay quay hết một vòng trong $60$ giây, thì tốc độ góc của nó là bao nhiêu rad/giây?],
    (
        [$pi/15$],
        True([$pi/30$]),
        [$pi/60$],
...
// TN 3
#lt-tn(num: 3, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Phương trình $sin x = -1/2$ có các nghiệm là],
    (
...
// TN 4
#lt-tn(num: 4, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Cường độ của dòng điện xoay chiều chạy qua một dây dẫn được mô hình hóa bởi hàm số $I(t) = 5 cos(100pi t - pi/4)$ (Ampe), với $t$ tính bằng giây. Cường độ dòng điện cực đại trong dây dẫn là],
    (
        True([$5 " A"$]),
        [$100pi " A"$],
        [$10 " A"$],
...
// TN 5
#lt-tn(num: 5, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Đồ thị của một hàm số biểu diễn sóng biển theo thời gian có hình dạng như hình dưới. 
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    // Vẽ trục tọa độ
...
// TN 6
#lt-tn(num: 6, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Chu kì tuần hoàn của hàm số $y = 3 tan(2x - pi/3)$ là],
    (
        [$2pi$],
        [$pi$],
        True([$pi/2$]),
...
// TN 7
#lt-tn(num: 7, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Một con lắc lò xo dao động điều hòa với phương trình li độ $x = A cos(omega t + phi)$. Tại thời điểm $t = 0$, vật đi qua vị trí cân bằng theo chiều dương. Pha ban đầu $phi$ của dao động là],
    (
        [$pi/2$],
        True([$-pi/2$]),
        [$pi$],
...
// TN 8
#lt-tn(num: 8, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Tập xác định của hàm số $y = 1 / (sin x + 1)$ là],
    (
...
// TN 9
#lt-tn(num: 9, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Một tấm pin năng lượng mặt trời đạt công suất tối đa $P(t) = 150 + 150 sin((pi)/12 (t - 6))$ (Watt) vào thời điểm $t$ giờ trong ngày ($0 <= t <= 24$). Thời điểm tấm pin đạt công suất lớn nhất trong ngày là lúc],
    (
        [$6$ giờ],
        True([$12$ giờ]),
        [$18$ giờ],
...
// TN 10
#lt-tn(num: 10, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Số nghiệm của phương trình $cos x = sqrt(3)/2$ trên đoạn $[0; 3pi]$ là],
    (
        [$1$],
        [$2$],
        True([$3$]),
...
// TN 11
#lt-tn(num: 11, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Trong mô hình cung - cầu kinh tế, sự chênh lệch doanh thu theo mùa vụ của một mặt hàng được ước tính bằng $f(x) = 200 + 50 cos((pi x)/6)$, với $x$ là tháng trong năm ($1 <= x <= 12$). Doanh thu đạt mức thấp nhất vào tháng nào?],
    (
        [$3$],
        True([$6$]),
        [$9$],
...
// TN 12
#lt-tn(num: 12, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Biểu thức rút gọn của $P = sin(x + pi/2) + cos(x + pi)$ là],
    (
        [$2 sin x$],
        [$2 cos x$],
        True([$0$]),
...
// TLN 1
#lt-tln(num: 17, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Một kim phút của đồng hồ dài $15 " cm"$. Tính quãng đường (làm tròn đến hàng phần mười của cm) mà đầu kim phút di chuyển được trong $20$ phút.],
    [31,4],
    loigiai: [
...
// TLN 2
#lt-tln(num: 18, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Huyết áp của một người được đo đạc và có thể mô hình hoá bởi hàm số $p(t) = 120 + 20 cos(150pi t)$, trong đó $p(t)$ là huyết áp tính bằng mmHg và $t$ là thời gian tính bằng phút. Biết nhịp tim của người này được tính là số chu kì của hàm số trong một phút. Tính nhịp tim của người đó (nhịp/phút).],
    [75],
    loigiai: [
...
// TLN 3
#lt-tln(num: 19, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Mực nước cao nhất tại một cảng biển được mô phỏng bởi $h(t) = 2 cos((pi t)/6) + 12$ (mét), với $t$ là thời gian tính bằng giờ sau nửa đêm ($0 <= t <= 24$). Tổng số giờ trong một ngày đêm mà mực nước cao hơn $13$ mét là bao nhiêu?],
    [8],
    loigiai: [
        Yêu cầu bài toán: 
        $
...
// TLN 5
#lt-tln(num: 21, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Có bao nhiêu giá trị nguyên của tham số $m$ để phương trình $5 cos x - m = 0$ có nghiệm?],
    [11],
    loigiai: [
...
// TLN 6
#lt-tln(num: 22, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Biết tập nghiệm của phương trình $cos(2x) - sin x = 0$ trên đoạn $[0; pi]$ là tập $S$. Tính tổng các phần tử của tập hợp $S$ (làm tròn đến $2$ chữ số thập phân, lấy $pi approx 3.14$).],
    [3,14],
    loigiai: [
...
#slide(title: none)[
  #align(center + horizon)[
    #block(
      ...
      #text(weight: "bold", fill: rgb("#a5b4fc"), size: 22pt)[🎉 CHÚC MỪNG HOÀN THÀNH CHƯƠNG 1. LƯỢNG GIÁC (ĐỀ SỐ 02)!]      #v(0.6em)
