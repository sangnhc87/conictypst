// ═══════════════════════════════════════════════════════════════════════════
// BEAMER ĐỀ THI: CHƯƠNG V. GIỚI HẠN. HÀM SỐ LIÊN TỤC (ĐỀ SỐ 02)
...
#show: lecture-theme.with(
  title: "CHƯƠNG V. GIỚI HẠN. HÀM SỐ LIÊN TỤC (ĐỀ SỐ 02)",
...
// TN 1
#lt-tn(num: 1, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Số lượng tế bào của một loài sinh vật sinh trưởng trong điều kiện dinh dưỡng hạn chế được mô tả bởi dãy số $v_n = 5000 - 4500 dot (0.9)^n$ (tế bào), với $n$ là số ngày. Khi thời gian kéo dài vô hạn ($n -> +oo$), số lượng tế bào sinh vật tiến sát dần về giá trị nào?],
    (
        [$500$ tế bào.],
        [$4500$ tế bào.],
        True([$5000$ tế bào.]),
...
// TN 2
#lt-tn(num: 2, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Một con lắc lò xo dao động tắt dần trên mặt phẳng ngang. Biên độ dao động của lần thứ $n$ bằng $80\%$ biên độ dao động của lần thứ $n-1$ ngay trước đó. Biết biên độ lần dao động đầu tiên là $10$ cm. Tính tổng biên độ của tất cả các lần dao động cho đến khi con lắc dừng lại hẳn (tính bằng xăng-ti-mét).],
    (
        [$40$ cm.],
        True([$50$ cm.]),
        [$80$ cm.],
...
// TN 3
#lt-tn(num: 3, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Một dự án khởi nghiệp dự kiến doanh thu đạt được mỗi năm sẽ giảm dần do cạnh tranh. Doanh thu năm đầu tiên là $100$ triệu đồng. Kể từ năm thứ hai, doanh thu mỗi năm giảm $20\%$ so với năm ngay trước đó. Tính tổng doanh thu tích lũy tối đa của dự án khi số năm vận hành kéo dài vô hạn (tính bằng triệu đồng).],
    (
        [$400$ triệu đồng.],
        True([$500$ triệu đồng.]),
        [$800$ triệu đồng.],
...
// TN 4
#lt-tn(num: 4, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Cho tam giác đều $A_1 B_1 C_1$ có diện tích bằng $S_1 = 16$. Dựng tam giác đều $A_2 B_2 C_2$ bằng cách nối các trung điểm của các cạnh tam giác $A_1 B_1 C_1$. Tiếp tục quá trình này vô hạn lần để được dãy tam giác đều. Tính tổng diện tích của tất cả các tam giác trong dãy đó.],
    (
        [$32$],
        [$24$],
        True([$(64)/(3)$]),
...
// TN 5
#lt-tn(num: 5, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Nồng độ thuốc $C(t)$ (đơn vị: mg/l) trong máu của bệnh nhân sau $t$ giờ tiêm được xác định bởi công thức $C(t) = (3t + 2)/(t^2 + 1)$. Tính giới hạn nồng độ thuốc trong máu của bệnh nhân khi thời gian kéo dài vô hạn ($t -> +oo$).],
    (
        True([$0$ mg/l.]),
        [$3$ mg/l.],
        [$2$ mg/l.],
...
// TN 6
#lt-tn(num: 6, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Chi phí trung bình để sản xuất $x$ sản phẩm của một xí nghiệp được cho bởi công thức $A(x) = (50x + 200)/(x)$ (nghìn đồng/sản phẩm). Khi số lượng sản phẩm sản xuất $x -> +oo$, chi phí sản xuất trung bình tiến sát dần về giá trị tối ưu nào?],
    (
        [$200$ nghìn đồng.],
        True([$50$ nghìn đồng.]),
        [$250$ nghìn đồng.],
...
// TN 7
#lt-tn(num: 7, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Vị trí của một hạt chuyển động trên trục số được xác định bởi hàm số tọa độ $s(t) = (t^2 - t + 2)/(2t^2 + 1)$ (mét), với $t >= 0$ là thời gian tính bằng giây. Xác định vị trí giới hạn của hạt khi thời gian $t -> +oo$.],
    (
        [$2$ m.],
        [$1$ m.],
        True([$(1)/(2)$ m.]),
...
// TN 8
#lt-tn(num: 8, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Nồng độ chất ô nhiễm sau màng lọc của một nhà máy hóa chất được đo bằng công thức $P(t) = (t^2 - 1)/(t - 1)$ (ppm) khi thời gian vận hành $t > 1$ giờ. Tính giới hạn nồng độ $P(t)$ khi thời gian tiến dần về sát mốc khởi điểm $1$ giờ ($t -> 1^+$).],
    (
        [$1$ ppm.],
        True([$2$ ppm.]),
        [$0$ ppm.],
...
// TN 9
#lt-tn(num: 9, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Một cánh tay robot chuyển động có quỹ đạo được mô tả bởi hàm số $f(t) = cases(t^2 - 4 "khi" t < 2, a t - 4 "khi" t >= 2)$. Tìm giá trị của tham số $a$ để quỹ đạo chuyển động là một đường liên tục tại thời điểm bàn giao $t = 2$.],
    (
        [$a = 0$],
        [$a = 1$],
        True([$a = 2$]),
...
// TN 10
#lt-tn(num: 10, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Cường độ dòng điện qua một mạch điện thay đổi theo thời gian $t$ (giây) bởi hàm số $I(t) = cases((t^2 - 3t + 2)/(t - 1) "khi" t > 1, k t + 1 "khi" t <= 1)$. Tìm giá trị của $k$ để cường độ dòng điện liên tục tại thời điểm $t = 1$.],
    (
        [$k = 0$],
        [$k = -1$],
        True([$k = -2$]),
...
// TN 11
#lt-tn(num: 11, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Cảm biến nhiệt độ trong một lò nung ghi nhận sự thay đổi nhiệt độ $T(t)$ ($""^o C$) liên tục theo thời gian $t$ (phút). Biết rằng ban đầu $T(0) = 20 ""^o C$ và sau $10$ phút $T(10) = 1500 ""^o C$. Theo định lý giá trị trung gian, khẳng định nào sau đây là chắc chắn đúng?],
    (
...
// TN 12
#lt-tn(num: 12, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Hàm số nào sau đây mô tả một hiện tượng biến thiên liên tục trong thực tế đời sống?],
    (
...
// DS 1
#lt-ds(num: 13, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Một doanh nghiệp dự kiến doanh thu mỗi năm sẽ giảm dần do cạnh tranh khốc liệt. Doanh thu năm đầu tiên là $10$ tỷ đồng. Kể từ năm thứ hai, doanh thu mỗi năm bằng $90\%$ doanh thu của năm trước đó. Các phát biểu sau đúng hay sai?], (
...
// DS 2
#lt-ds(num: 14, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Sau khi một bệnh nhân uống một liều thuốc, nồng độ thuốc trong máu của bệnh nhân $C(t)$ (mg/l) sau $t$ giờ được xác định bởi công thức: $C(t) = (15t)/(t^2 + 4)$. Các phát biểu sau đúng hay sai?], (
...
// DS 3
#lt-ds(num: 15, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Cước phí gửi bưu phẩm của một bưu điện được tính theo trọng lượng $x$ (gam) bởi hàm số:
...
// DS 4
#lt-ds(num: 16, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Một kỹ sư giao thông thiết kế bề mặt độ cao của một đoạn dốc cầu nối tiếp. Độ cao mặt đường $h(x)$ (mét) theo khoảng cách ngang $x$ (mét) được mô tả bởi hàm số:
...
// TL 1
#lt-tln(num: 17, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Một khách hàng tham gia chương trình tích lũy tài chính dài hạn. Đầu mỗi tháng người đó gửi tiết kiệm cố định $10$ triệu đồng vào tài khoản ngân hàng chịu lãi suất $0.5\%$/tháng. Giá trị hiện tại $P V$ của chuỗi tiền gửi vô hạn này quy về thời điểm hiện tại được tính bằng tổng cấp số nhân lùi vô hạn: $P V = (10)/(1 + 0.005) + (10)/((1 + 0.005)^2) + (10)/((1 + 0.005)^3) + ...$ Tính tổng giá trị hiện tại $P V$ này (làm tròn đến hàng đơn vị, tính bằng triệu đồng).],
    [2000],
    loigiai: [
...
// TL 2
#lt-tln(num: 18, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Một bình chứa $100$ lít cồn nguyên chất. Người ta rút ra $20$ lít cồn rồi đổ vào $20$ lít nước, khuấy đều. Sau đó lại rút ra $20$ lít dung dịch rồi đổ vào $20$ lít nước. Quá trình này lặp lại liên tục $n$ lần. Khi $n -> +oo$, lượng cồn nguyên chất còn lại trong bình tiến dần về bao nhiêu lít?],
    [0],
    loigiai: [
...
// TL 3
#lt-tln(num: 19, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Cường độ ánh sáng $I(x)$ truyền qua một tấm kính dày $x$ (cm) được xác định bởi công thức $I(x) = I_0 dot e^(-0.12 x)$, với $I_0$ là cường độ ban đầu. Khi độ dày kính tăng lên vô hạn ($x -> +oo$), cường độ ánh sáng truyền qua kính tiến về giá trị bằng bao nhiêu phần trăm (%) so với cường độ ánh sáng ban đầu $I_0$?],
    [0],
    loigiai: [
...
// TL 4
#lt-tln(num: 20, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Một doanh nghiệp ước tính doanh thu đạt được (tính bằng tỷ đồng) sau $x$ tháng chạy chương trình quảng bá sản phẩm mới là $R(x) = (30x^2 - x + 5)/(x^2 + 1)$. Tính giới hạn doanh thu tối đa của doanh nghiệp khi thời gian chạy chiến dịch quảng bá này kéo dài vô hạn ($x -> +oo$).],
    [30],
    loigiai: [
...
// TL 5
#lt-tln(num: 21, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Nồng độ bụi mịn PM2.5 trong không khí tại một trạm quan trắc tự động (đơn vị: $mu g / m^3$) thay đổi theo thời gian $t$ (giờ) trong ngày được mô tả bởi hàm số:
...
// TL 6
#lt-tln(num: 22, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Lãi suất của một khoản vay mua nhà ưu đãi được tính theo thời gian $t$ (năm) bởi hàm số liên tục:
...
#slide(title: none)[
  #align(center + horizon)[
    #block(
      ...
      #text(weight: "bold", fill: rgb("#a5b4fc"), size: 22pt)[🎉 CHÚC MỪNG HOÀN THÀNH CHƯƠNG V. GIỚI HẠN. HÀM SỐ LIÊN TỤC (ĐỀ SỐ 02)!]      #v(0.6em)
