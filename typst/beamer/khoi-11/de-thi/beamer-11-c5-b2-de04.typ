// ═══════════════════════════════════════════════════════════════════════════
// BEAMER ĐỀ THI: CHƯƠNG V. GIỚI HẠN. HÀM SỐ LIÊN TỤC (ĐỀ SỐ 04)
...
#show: lecture-theme.with(
  title: "CHƯƠNG V. GIỚI HẠN. HÀM SỐ LIÊN TỤC (ĐỀ SỐ 04)",
...
// TN 1
#lt-tn(num: 1, de: [Đề 1 (de04) — Mã 101], back-to: "sec-exercise-hub",[Số lượng loài sinh vật gây hại trong một nông trại được kiểm soát sinh học. Mỗi tuần số lượng của chúng giảm $30\%$ so với tuần trước, đồng thời nông trại ghi nhận có thêm $3000$ cá thể mới sinh ra từ các ổ bọ gậy. Gọi $u_n$ là số lượng sinh vật sau $n$ tuần. Tìm số lượng cá thể ổn định lâu dài của loài này khi $n -> +oo$.],
    (
        [$3000$ cá thể.],
        [$7000$ cá thể.],
        True([$10000$ cá thể.]),
...
// TN 3
#lt-tn(num: 3, de: [Đề 1 (de04) — Mã 101], back-to: "sec-exercise-hub",[Cho một hình vuông có cạnh bằng $4$. Người ta dựng hình vuông thứ hai bằng cách nối trung điểm các cạnh của hình vuông thứ nhất, hình vuông thứ ba bằng cách nối trung điểm các cạnh của hình vuông thứ hai, và cứ tiếp tục như vậy vô hạn lần. Tính tổng diện tích của tất cả các hình vuông trong dãy đó.],
    (
        [$16$],
        [$24$],
        True([$32$]),
...
// TN 4
#lt-tn(num: 4, de: [Đề 1 (de04) — Mã 101], back-to: "sec-exercise-hub",[Cho giới hạn $limits(lim)_(x -> 0) (sqrt(1 + 2x) - 1)/(x) = a$ và $limits(lim)_(x -> +oo) (b x^2 - x + 1)/(2x^2 + 3) = 2$ với $a, b in RR$. Tính giá trị của biểu thức $P = a + b$.],
    (
        [$P = 3$],
        [$P = 4$],
        True([$P = 5$]),
...
// TN 5
#lt-tn(num: 5, de: [Đề 1 (de04) — Mã 101], back-to: "sec-exercise-hub",[Chi phí thiết lập ban đầu cho một dây chuyền sản xuất của một doanh nghiệp là $500$ triệu đồng. Chi phí nguyên vật liệu để sản xuất mỗi sản phẩm tiếp theo là $2$ triệu đồng. Gọi $C(x)$ (triệu đồng) là chi phí trung bình để sản xuất $x$ sản phẩm. Tính giới hạn của chi phí trung bình này khi số lượng sản phẩm sản xuất ra tiến tới vô hạn ($x -> +oo$).],
    (
        [$500$ triệu đồng.],
        [$250$ triệu đồng.],
        True([$2$ triệu đồng.]),
...
// TN 6
#lt-tn(num: 6, de: [Đề 1 (de04) — Mã 101], back-to: "sec-exercise-hub",[Nồng độ của một chất kích thích sinh trưởng trong đất sau khi phun thuốc được tính bởi công thức $A(t) = (t^2 - 4t + 3)/(t - 3)$ (ppm) với $t > 3$ (ngày). Tính giới hạn nồng độ chất kích thích khi thời gian $t$ tiến sát về ngày thứ $3$ ($t -> 3^+$).],
    (
        [$0$ ppm.],
        [$1$ ppm.],
        True([$2$ ppm.]),
...
// TN 7
#lt-tn(num: 7, de: [Đề 1 (de04) — Mã 101], back-to: "sec-exercise-hub",[Tốc độ truyền tin của một chiến dịch truyền thông trực tuyến sau $t$ ngày được mô tả bởi hàm số $V(t) = 150 - (120)/(t^2 + 1)$ (lượt chia sẻ/phút). Tính tốc độ truyền tin tối đa đạt được khi chiến dịch kéo dài vô hạn ($t -> +oo$).],
    (
        [$120$ lượt/phút.],
        True([$150$ lượt/phút.]),
        [$30$ lượt/phút.],
...
// TN 8
#lt-tn(num: 8, de: [Đề 1 (de04) — Mã 101], back-to: "sec-exercise-hub",[Cho hàm số $f(x)$ thỏa mãn giới hạn $limits(lim)_(x -> -1) (f(x) - 3)/(x + 1) = 2$. Tính giới hạn $L = limits(lim)_(x -> -1) (f^2(x) - 9)/(x^2 - 1)$.],
    (
        [$L = 6$],
        [$L = 2$],
        [$L = -3$],
...
// TN 9
#lt-tn(num: 9, de: [Đề 1 (de04) — Mã 101], back-to: "sec-exercise-hub",[Cường độ sáng của một đèn LED thông minh tự động thay đổi theo thời gian $t$ (giây) bởi hàm số $L(t) = cases(t^2 + 2 "khi" 0 <= t < 2, a t - 2 "khi" t >= 2)$. Tìm giá trị của $a$ để đèn thay đổi cường độ sáng liên tục tại thời điểm chuyển chế độ $t = 2$.],
    (
        [$a = 2$],
        [$a = 3$],
        True([$a = 4$]),
...
// TN 11
#lt-tn(num: 11, de: [Đề 1 (de04) — Mã 101], back-to: "sec-exercise-hub",[Quỹ đạo bay của một tên lửa khí tượng được mô tả bởi một hàm số liên tục theo thời gian $t$ (phút). Lúc phóng ($t = 0$), tên lửa ở độ cao $0$ m. Sau $5$ phút, tên lửa đạt độ cao tối đa là $8000$ m. Nhận xét nào sau đây chắc chắn đúng?],
    (
...
// TN 12
#lt-tn(num: 12, de: [Đề 1 (de04) — Mã 101], back-to: "sec-exercise-hub",[Hàm số nào sau đây mô tả một đại lượng thay đổi liên tục (không có bước nhảy) trong đời sống tự nhiên?],
    (
...
// DS 1
#lt-ds(num: 13, de: [Đề 1 (de04) — Mã 101], back-to: "sec-exercise-hub",[Một quần thể cá trong hồ bảo tồn ban đầu có số lượng là $1000$ con. Mỗi năm, do sinh sản tự nhiên, số cá tăng $10\%$ so với năm trước đó, đồng thời cơ quan bảo tồn đánh bắt giảm thiểu định kỳ $200$ con cá để cân bằng sinh thái. Gọi $u_n$ là số lượng cá sau $n$ năm. Các phát biểu sau đúng hay sai?], (
...
// DS 2
#lt-ds(num: 14, de: [Đề 1 (de04) — Mã 101], back-to: "sec-exercise-hub",[Chi phí trung bình để sản xuất $x$ tấn sản phẩm hóa chất của một nhà máy được tính bởi công thức: $C(x) = (10x + 20)/(x + 1)$ (triệu đồng/tấn) với sản lượng $x > 0$. Các phát biểu sau đúng hay sai?], (
...
// DS 3
#lt-ds(num: 15, de: [Đề 1 (de04) — Mã 101], back-to: "sec-exercise-hub",[Một đập ngăn nước thủy điện có mặt cắt đứng được thiết kế dạng đường biên là hàm số $y = f(x)$ (mét) theo chiều ngang $x$ (mét):
...
// DS 4
#lt-ds(num: 16, de: [Đề 1 (de04) — Mã 101], back-to: "sec-exercise-hub",[Một chiến dịch quảng cáo trực tuyến được bắt đầu bằng việc gửi tin nhắn đến $100$ khách hàng đầu tiên. Cứ sau mỗi giờ, lượng khách hàng mới nhận được quảng cáo bằng $50\%$ lượng người mới nhận ở giờ ngay trước đó (tin nhắn lan truyền giảm dần theo cấp số nhân lùi vô hạn). Các phát biểu sau đúng hay sai?], (
...
// TL 2
#lt-tln(num: 18, de: [Đề 1 (de04) — Mã 101], back-to: "sec-exercise-hub",[Biết giới hạn $limits(lim)_(x -> 3) (a sqrt(x + 1) + b)/(x - 3) = 1$ với $a, b in RR$. Tính giá trị của biểu thức $P = a^2 + b^2$.],
    [80],
    loigiai: [
...
// TL 3
#lt-tln(num: 19, de: [Đề 1 (de04) — Mã 101], back-to: "sec-exercise-hub",[Cho hàm số $y = f(x)$ liên tục tại mọi điểm và thỏa mãn giới hạn $limits(lim)_(x -> 2) (f(x) - 6)/(x - 2) = 5$. Tính giới hạn $H = limits(lim)_(x -> 2) (f^2(x) - 36)/(x^2 - 4)$.],
    [15],
    loigiai: [
...
// TL 4
#lt-tln(num: 20, de: [Đề 1 (de04) — Mã 101], back-to: "sec-exercise-hub",[Một loại vi sinh vật gây bệnh được tiêm vào cơ thể động vật thí nghiệm. Nồng độ kháng thể tự nhiên $A(t)$ (mg/dl) tạo ra để chống lại vi khuẩn sau $t$ ngày được mô tả bởi công thức $A(t) = (40t^2 + 10)/(t^2 + 2)$. Tính giới hạn nồng độ kháng thể tối đa đạt được trong cơ thể khi thời gian $t -> +oo$.],
    [40],
    loigiai: [
...
// TL 5
#lt-tln(num: 21, de: [Đề 1 (de04) — Mã 101], back-to: "sec-exercise-hub",[Tìm giá trị dương của tham số $m$ để hàm số $f(x) = cases((x^2 - 3x + 2)/(x - 2) "khi" x < 2, m^2 x - 7 "khi" x >= 2)$ liên tục tại điểm $x = 2$.],
    [2],
    loigiai: [
...
// TL 6
#lt-tln(num: 22, de: [Đề 1 (de04) — Mã 101], back-to: "sec-exercise-hub",[Một máng nước bằng bê tông được đúc nối tiếp hai phần dốc và ngang. Bề mặt đáy máng nước có độ cao so với mặt đất phẳng là $h(t)$ (mét) theo chiều dài máng $t$ (mét) được mô tả bởi hàm số liên tục:
...
#slide(title: none)[
  #align(center + horizon)[
    #block(
      ...
      #text(weight: "bold", fill: rgb("#a5b4fc"), size: 22pt)[🎉 CHÚC MỪNG HOÀN THÀNH CHƯƠNG V. GIỚI HẠN. HÀM SỐ LIÊN TỤC (ĐỀ SỐ 04)!]      #v(0.6em)
