// ═══════════════════════════════════════════════════════════════════════════
// BEAMER ĐỀ THI: CHƯƠNG V. GIỚI HẠN. HÀM SỐ LIÊN TỤC (ĐỀ SỐ 09)
...
#show: lecture-theme.with(
  title: "CHƯƠNG V. GIỚI HẠN. HÀM SỐ LIÊN TỤC (ĐỀ SỐ 09)",
...
// TN 1
#lt-tn(num: 1, de: [Đề 3 (de09) — Mã 101], back-to: "sec-exercise-hub",[Một quần thể cá trong hồ nước được mô tả sinh trưởng theo mô hình tăng trưởng giới hạn (mô hình tăng trưởng Logistic): $P(t) = (2000)/(1 + 3 e^(-0.2 t))$ (con) sau $t$ tháng nuôi thả. Số lượng cá tối đa mà hồ nước có thể duy trì nuôi dưỡng lâu dài (giới hạn sức chứa $K$ khi $t -> +oo$) là bao nhiêu?],
    (
        [$500$ con.],
        [$1000$ con.],
        True([$2000$ con.]),
...
// TN 2
#lt-tn(num: 2, de: [Đề 3 (de09) — Mã 101], back-to: "sec-exercise-hub",[Vận tốc $v(t)$ (m/s) của một người nhảy dù rơi tự do trong không khí có lực cản sau $t$ giây được cho bởi công thức: $v(t) = 50 (1 - e^(-0.15 t))$. Tính vận tốc rơi cực hạn (giới hạn vận tốc khi thời gian rơi kéo dài vô hạn) của người nhảy dù đó.],
    (
        [$35$ m/s.],
        True([$50$ m/s.]),
        [$75$ m/s.],
...
// TN 3
#lt-tn(num: 3, de: [Đề 3 (de09) — Mã 101], back-to: "sec-exercise-hub",[Một tụ điện phẳng có điện dung $C$ được nạp điện bằng nguồn điện xoay chiều thông qua mạch RC (như sơ đồ vẽ bên dưới). Điện lượng tích tụ trên bản tụ điện tăng dần theo thời gian $t$ (giây) theo công thức $Q(t) = 0.05 (1 - e^(-2 t))$ (coulomb). Tính giới hạn điện tích tối đa mà tụ điện có thể tích lũy được khi $t -> +oo$.],
    (
        [$0.01$ coulomb.],
        [$0.025$ coulomb.],
        True([$0.05$ coulomb.]),
...
// TN 4
#lt-tn(num: 4, de: [Đề 3 (de09) — Mã 101], back-to: "sec-exercise-hub",[Nồng độ $C(t)$ (mol/l) của một chất hóa học sản phẩm trong một phản ứng hóa học sau $t$ phút được xác định bởi công thức: $C(t) = (0.8 t)/(2 t + 3)$. Tính nồng độ tối đa của sản phẩm hóa học thu được khi thời gian phản ứng kéo dài vô hạn.],
    (
        [$0.2$ mol/l.],
        True([$0.4$ mol/l.]),
        [$0.8$ mol/l.],
...
// TN 5
#lt-tn(num: 5, de: [Đề 3 (de09) — Mã 101], back-to: "sec-exercise-hub",[Chi phí tổng cộng để doanh nghiệp cơ khí sản xuất $x$ sản phẩm được mô tả bởi hàm số $C(x) = 1500 + 4x$ (nghìn đồng). Hàm chi phí trung bình để sản xuất một sản phẩm là $A(x) = (C(x))/(x)$. Tìm giới hạn của hàm chi phí trung bình khi sản lượng $x$ tiến tới vô hạn.],
    (
        [$0$ nghìn đồng.],
        [$1.5$ nghìn đồng.],
        True([$4$ nghìn đồng.]),
...
// TN 7
#lt-tn(num: 7, de: [Đề 3 (de09) — Mã 101], back-to: "sec-exercise-hub",[Một kỹ sư giao thông thiết kế đường cong chuyển tiếp kết nối một đoạn đường thẳng nằm ngang $y = 1$ (khi $x < 0$) với một đoạn đường cong dạng parabol $y = a x^2 + b x + c$ (khi $x >= 0$) (như hình vẽ bên dưới). Để mặt đường liên tục tại vị trí nối $x = 0$, giá trị của tham số thiết kế $c$ phải bằng bao nhiêu?],
    (
        [$0$],
        True([$1$]),
        [$2$],
...
// TN 8
#lt-tn(num: 8, de: [Đề 3 (de09) — Mã 101], back-to: "sec-exercise-hub",[Nhiệt độ $T(h)$ °C của tầng khí quyển theo độ cao $h$ (km) từ mặt đất lên đến tầng bình lưu (độ cao dưới $20$ km) được mô hình hóa bởi một hàm liên tục có công thức (như hình vẽ):
...
// TN 9
#lt-tn(num: 9, de: [Đề 3 (de09) — Mã 101], back-to: "sec-exercise-hub",[Nhiệt độ của một thanh kim loại nung nóng được đưa vào phòng làm nguội có nhiệt độ môi trường cố định là $25°$C. Sau $t$ phút, nhiệt độ của thanh kim loại được mô tả bởi hàm số $T(t) = 25 + 120 e^(-0.05 t)$ (như đồ thị bên dưới). Tìm nhiệt độ ổn định cuối cùng của thanh kim loại khi thời gian làm nguội kéo dài vô hạn ($t -> +oo$).],
    (
        [$0°$C.],
        True([$25°$C.]),
        [$120°$C.],
...
// TN 10
#lt-tn(num: 10, de: [Đề 3 (de09) — Mã 101], back-to: "sec-exercise-hub",[Theo định luật Beer-Lambert về sự hấp thụ ánh sáng trong môi trường đồng chất, cường độ ánh sáng $I(d)$ tại độ sâu $d$ (mét) dưới mặt nước biển giảm dần theo hàm số mũ $I(d) = I_0 dot (0.85)^d$ (với $I_0$ là cường độ ánh sáng tại mặt nước). Tìm giới hạn của cường độ ánh sáng khi độ sâu tiến ra vô hạn ($d -> +oo$).],
    (
        [$I_0$],
        [$0.85 I_0$],
        True([$0$]),
...
// TN 11
#lt-tn(num: 11, de: [Đề 3 (de09) — Mã 101], back-to: "sec-exercise-hub",[Năng suất làm việc của một nhân viên mới sau $t$ ngày đào tạo được mô tả bởi đường cong học tập (Learning Curve) có hàm số: $N(t) = (40 t + 10)/(t + 2)$ (sản phẩm/ngày) (như đồ thị bên dưới). Tìm năng suất tối đa giới hạn mà nhân viên đó có thể đạt được khi thời gian đào tạo kéo dài vô hạn.],
    (
        [$10$ sản phẩm/ngày.],
        [$20$ sản phẩm/ngày.],
...
// TN 12
#lt-tn(num: 12, de: [Đề 3 (de09) — Mã 101], back-to: "sec-exercise-hub",[Sự lan truyền một tin đồn trong một trường học có $1000$ học sinh được mô hình hóa bởi hàm số: $S(t) = (1000)/(1 + 99 e^(-0.5 t))$ (học sinh) sau $t$ giờ phát tán. Tìm số lượng học sinh tối đa biết tin đồn sau một thời gian rất dài ($t -> +oo$).],
    (
        [$10$ học sinh.],
        [$100$ học sinh.],
        [$990$ học sinh.],
...
// DS 1
#lt-ds(num: 13, de: [Đề 3 (de09) — Mã 101], back-to: "sec-exercise-hub",[Cho mô hình tăng trưởng của loài tảo sinh học biển trong hồ nuôi cấy thí nghiệm: $A(t) = (500)/(1 + 9 e^(-0.1 t))$ (gram) sau $t$ ngày nuôi cấy. Các phát biểu sau đúng hay sai?], (
...
// DS 2
#lt-ds(num: 14, de: [Đề 3 (de09) — Mã 101], back-to: "sec-exercise-hub",[Một dòng điện nạp vào tụ điện phẳng trong một mạch tích điện có điện lượng biến thiên theo thời gian: $q(t) = 3 (1 - e^(-0.5 t))$ (mili-coulomb, viết tắt mC) sau $t$ giây. Các phát biểu sau đúng hay sai?], (
...
// DS 3
#lt-ds(num: 15, de: [Đề 3 (de09) — Mã 101], back-to: "sec-exercise-hub",[Nhiệt độ khí quyển $T(h)$ °C theo độ cao $h$ (km) từ mặt đất lên đến sát tầng bình lưu được mô tả bởi một hàm số ghép liên tục như sau:
...
// DS 4
#lt-ds(num: 16, de: [Đề 3 (de09) — Mã 101], back-to: "sec-exercise-hub",[Kỹ sư cầu đường thiết kế bề mặt dốc cầu trượt trơn nối tiếp từ mặt đường nằm ngang lên một đoạn nghiêng dạng parabol:
...
// TL 1
#lt-tln(num: 17, de: [Đề 3 (de09) — Mã 101], back-to: "sec-exercise-hub",[Nồng độ của một loại dược chất kháng sinh đào thải dần trong máu của người bệnh sau $t$ giờ uống thuốc được cho bởi công thức nồng độ: $C(t) = (180 t + 50)/(2 t + 1)$ (mg/l) (như hình vẽ tương tự). Tìm giới hạn nồng độ ổn định lâu dài của dược chất đó khi thời gian $t -> +oo$ (tính bằng mg/l).],
    [90],
    fig: fig-cooling,
    fig-pos: "center",
    fig-width: 30%,
...
// TL 2
#lt-tln(num: 18, de: [Đề 3 (de09) — Mã 101], back-to: "sec-exercise-hub",[Một xưởng cơ khí sản xuất linh kiện có chi phí cố định ban đầu là $80$ triệu đồng, và chi phí phát sinh trực tiếp cho mỗi sản phẩm là $0.5$ triệu đồng. Gọi $A(x)$ là chi phí trung bình (triệu đồng/sản phẩm) để sản xuất được $x$ sản phẩm. Tính giới hạn chi phí trung bình tối thiểu để sản xuất một sản phẩm khi sản lượng sản xuất tiến tới vô hạn ($x -> +oo$), dưới dạng số thập phân.],
    [0.5],
    fig: fig-average-cost,
    fig-pos: "center",
    fig-width: 30%,
...
// TL 4
#lt-tln(num: 20, de: [Đề 3 (de09) — Mã 101], back-to: "sec-exercise-hub",[Thiết kế hệ thống cảm biến nhiệt đo hai phạm vi hoạt động khác nhau liên tục (hàm số nhiệt độ liên tục) trên dải thời gian:
...
// TL 5
#lt-tln(num: 21, de: [Đề 3 (de09) — Mã 101], back-to: "sec-exercise-hub",[Kỹ sư thiết kế đường cong chuyển tiếp kết nối hai làn đường cao tốc có cao độ được mô hình hóa bởi hàm số:
...
#slide(title: none)[
  #align(center + horizon)[
    #block(
      ...
      #text(weight: "bold", fill: rgb("#a5b4fc"), size: 22pt)[🎉 CHÚC MỪNG HOÀN THÀNH CHƯƠNG V. GIỚI HẠN. HÀM SỐ LIÊN TỤC (ĐỀ SỐ 09)!]      #v(0.6em)
