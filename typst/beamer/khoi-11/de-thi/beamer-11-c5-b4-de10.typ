// ═══════════════════════════════════════════════════════════════════════════
// BEAMER ĐỀ THI: CHƯƠNG V. GIỚI HẠN. HÀM SỐ LIÊN TỤC (ĐỀ SỐ 10)
...
#show: lecture-theme.with(
  title: "CHƯƠNG V. GIỚI HẠN. HÀM SỐ LIÊN TỤC (ĐỀ SỐ 10)",
...
// TN 1
#lt-tn(num: 1, de: [Đề 1 (de10) — Mã 101], back-to: "sec-exercise-hub",[Một quả bóng cao su được thả từ độ cao $9$ m xuống một nền đất phẳng chịu lực. Mỗi lần chạm đất, quả bóng nảy ngược lên độ cao bằng $(2)/(3)$ độ cao của lần rơi ngay trước đó (như hình vẽ mô tả). Tính tổng quãng đường quả bóng di chuyển được (từ lúc thả cho đến khi dừng hẳn, tính bằng mét).],
    (
        [$27$ m.],
        [$36$ m.],
        True([$45$ m.]),
...
// TN 6
#lt-tn(num: 6, de: [Đề 1 (de10) — Mã 101], back-to: "sec-exercise-hub",[Một bình chứa khí gas công nghiệp bị rò rỉ van xả áp suất (như hình vẽ). Mỗi giờ, lượng gas thoát ra ngoài bằng $5\%$ lượng gas còn lại trong bình chứa. Giả sử lượng gas ban đầu là $V_0$. Tìm tổng lượng gas đã thoát ra khỏi bình chứa sau thời gian vô hạn.],
    (
        [$0.05 V_0$],
        [$0.5 V_0$],
        True([$V_0$]),
...
// TN 7
#lt-tn(num: 7, de: [Đề 1 (de10) — Mã 101], back-to: "sec-exercise-hub",[Một quần thể động vật quý hiếm trong khu bảo tồn sinh thái bị đánh bắt có mức sinh trưởng sau $t$ năm được mô phỏng theo hàm số: $P(t) = (3000 t + 500)/(t + 1)$ (con) (như đồ thị minh họa bên dưới). Hãy tìm giới hạn số lượng cá thể tối đa mà quần thể này có thể đạt được và duy trì ổn định dài hạn khi $t -> +oo$.],
    (
        [$500$ con.],
        [$1500$ con.],
        True([$3000$ con.]),
...
// TN 8
#lt-tn(num: 8, de: [Đề 1 (de10) — Mã 101], back-to: "sec-exercise-hub",[Kỹ sư thiết kế đường cong chuyển tiếp kết nối một mặt phẳng ngang cao độ $y = 3$ (khi $x <= 0$) với một đoạn đường cong dốc dạng parabol $y = a x^2 + b x + c$ (khi $x > 0$) (như hình vẽ tương tự). Để mặt đường liên tục tại vị trí nối $x = 0$, giá trị của tham số thiết kế $c$ phải bằng bao nhiêu?],
    (
        [$0$],
        [$1$],
        [$2$],
...
// TN 10
#lt-tn(num: 10, de: [Đề 1 (de10) — Mã 101], back-to: "sec-exercise-hub",[Mật độ vi khuẩn trong đĩa thí nghiệm sinh học phát triển theo thời gian $t$ (giờ) tuân theo mô hình giới hạn sức chứa (hàm số Logistic): $P(t) = (5000)/(1 + 4 e^(-0.3 t))$ (đơn vị/ml) (như đồ thị bên dưới). Tìm giới hạn mật độ vi khuẩn tối đa đạt được khi thời gian nuôi cấy kéo dài vô hạn ($t -> +oo$).],
    (
        [$1000$ đơn vị/ml.],
        [$1250$ đơn vị/ml.],
        True([$5000$ đơn vị/ml.]),
...
// TN 12
#lt-tn(num: 12, de: [Đề 1 (de10) — Mã 101], back-to: "sec-exercise-hub",[Thiết kế đoạn đường ray tàu hỏa nối tiếp giữa hai khu vực địa hình có cao độ được mô phỏng bởi hàm số liên tục:
...
// DS 1
#lt-ds(num: 13, de: [Đề 1 (de10) — Mã 101], back-to: "sec-exercise-hub",[Một quả bóng bàn được thả rơi tự do từ độ cao $6$ m xuống mặt bàn phẳng. Mỗi lần chạm mặt bàn, quả bóng bàn nảy ngược lên độ cao bằng $(1)/(2)$ độ cao rơi của lần trước đó (hình vẽ tương tự). Các phát biểu sau đúng hay sai?], (
...
// DS 2
#lt-ds(num: 14, de: [Đề 1 (de10) — Mã 101], back-to: "sec-exercise-hub",[Một bệnh nhân ngoại trú uống một liều kháng sinh điều trị định kỳ $80$ mg sau mỗi 24 giờ. Cơ thể hấp thụ và tự đào thải thuốc: sau mỗi chu kỳ 24 giờ lượng thuốc còn lưu lại trong máu giảm xuống còn một nửa ($50\%$) lượng thuốc hiện có (như mô hình vẽ bên dưới). Các phát biểu sau đúng hay sai?], (
...
// DS 4
#lt-ds(num: 16, de: [Đề 1 (de10) — Mã 101], back-to: "sec-exercise-hub",[Một dốc chuyển tiếp giao thông có thiết kế dạng hàm số ghép liên tục như sau:
...
// TL 1
#lt-tln(num: 17, de: [Đề 1 (de10) — Mã 101], back-to: "sec-exercise-hub",[Một quả bóng được thả rơi từ độ cao $12$ m xuống đất. Mỗi lần chạm đất, quả bóng nảy ngược lên độ cao bằng $(1)/(3)$ độ cao của lần rơi trước đó (như hình vẽ tương tự). Tính tổng quãng đường quả bóng di chuyển được cho đến khi dừng hẳn (tính bằng mét).],
    [24],
    fig: fig-bouncing-ball,
    fig-pos: "center",
    fig-width: 30%,
...
// TL 3
#lt-tln(num: 19, de: [Đề 1 (de10) — Mã 101], back-to: "sec-exercise-hub",[Một con lắc vật lý dao động giảm dần từ vị trí ban đầu (hình vẽ tương tự). Vòng dao động đầu tiên có chiều dài $50$ cm. Mỗi vòng dao động tiếp theo có chiều dài bằng $80\%$ vòng dao động ngay trước đó. Tính tổng quãng đường con lắc di chuyển được cho đến khi dừng hẳn (tính bằng xăng-ti-mét).],
    [250],
    fig: fig-pendulum,
    fig-pos: "center",
    fig-width: 30%,
...
// TL 5
#lt-tln(num: 21, de: [Đề 1 (de10) — Mã 101], back-to: "sec-exercise-hub",[Thiết kế đoạn dốc cầu parabol kết nối mặt phẳng ngang cao độ $y = 4$ có công thức cao độ liên tục:
...
// TL 6
#lt-tln(num: 22, de: [Đề 1 (de10) — Mã 101], back-to: "sec-exercise-hub",[Một quần thể cá hồi di cư phát triển theo thời gian $t$ (năm) dưới tác động đánh bắt được mô phỏng bởi hàm số: $P(t) = (1600 t + 300)/(2 t + 1)$ (con) (hình vẽ đồ thị tương tự). Tìm giới hạn số lượng cá hồi tối đa mà quần thể đạt được khi thời gian $t -> +oo$.],
    [800],
    fig: fig-growth-harvest,
    fig-pos: "center",
    fig-width: 30%,
...
#slide(title: none)[
  #align(center + horizon)[
    #block(
      ...
      #text(weight: "bold", fill: rgb("#a5b4fc"), size: 22pt)[🎉 CHÚC MỪNG HOÀN THÀNH CHƯƠNG V. GIỚI HẠN. HÀM SỐ LIÊN TỤC (ĐỀ SỐ 10)!]      #v(0.6em)
