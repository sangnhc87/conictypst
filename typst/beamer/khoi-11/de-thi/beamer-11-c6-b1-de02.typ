// ═══════════════════════════════════════════════════════════════════════════
// BEAMER ĐỀ THI: CHƯƠNG 2. MŨ VÀ LOGARIT (ĐỀ SỐ 02)
...
#show: lecture-theme.with(
  title: "CHƯƠNG 2. MŨ VÀ LOGARIT (ĐỀ SỐ 02)",
...
// TN 1
#lt-tn(num: 1, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Áp suất khí quyển $P$ (tính bằng milibar) giảm dần theo độ cao $h$ (tính bằng km) so với mực nước biển theo công thức $P(h) = 1013 e^(-0.12 h)$. Áp suất khí quyển ở độ cao $5$ km xấp xỉ bằng bao nhiêu milibar?],
    (
        [$250$ mb],
        [$345$ mb],
        True([$556$ mb]),
...
// TN 2
#lt-tn(num: 2, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Một bệnh nhân được tiêm $20$ mg một loại thuốc. Lượng thuốc $M(t)$ (mg) còn lại trong máu sau $t$ giờ tiêm tuân theo hàm phân rã $M(t) = 20 (0.75)^t$. Hỏi sau bao lâu thì lượng thuốc trong máu giảm xuống chỉ còn $5$ mg? (Làm tròn đến 1 chữ số thập phân).],
    (
        [$3.0$ giờ],
        [$4.2$ giờ],
        [$2.8$ giờ],
...
// TN 3
#lt-tn(num: 3, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Để diệt khuẩn trong một căn phòng, người ta sử dụng khí ozone. Biết rằng lượng vi khuẩn $N(t)$ sau $t$ phút phun khí ozone được mô hình hóa bởi $N(t) = N_0 dot 10^(-0.05 t)$, với $N_0$ là số lượng vi khuẩn ban đầu. Phải mất ít nhất bao nhiêu phút để tiêu diệt được $99%$ lượng vi khuẩn trong phòng?],
    (
        [$10$ phút],
        [$20$ phút],
        True([$40$ phút]),
...
// TN 4
#lt-tn(num: 4, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Cường độ sáng biểu kiến của một ngôi sao được đặc trưng bởi "cấp sao" $m$. Công thức Pogson cho biết hiệu cấp sao của hai ngôi sao tỉ lệ thuận với logarit cơ số $10$ của tỉ số độ sáng (quang thông) của chúng: $m_2 - m_1 = 2.5 log(I_1 / I_2)$. Nếu ngôi sao $A$ sáng gấp $100$ lần ngôi sao $B$, thì hiệu cấp sao $m_B - m_A$ là bao nhiêu?],
    (
        [$2$],
        True([$5$]),
        [$10$],
...
// TN 5
#lt-tn(num: 5, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Sự phát triển của một trang mạng xã hội được mô phỏng bởi hàm logistic $S(t) = 100 / (1 + 9 e^(-0.8 t))$ (triệu người dùng), với $t$ là số năm kể từ lúc ra mắt. Theo mô hình này, số lượng người dùng tối đa (khi $t -> +oo$) mà trang mạng này có thể đạt được là bao nhiêu?],
    (
        [$9$ triệu],
        True([$100$ triệu]),
        [$10$ triệu],
...
// TN 6
#lt-tn(num: 6, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Trong một nhà máy, mức ồn $L$ đo được tại khu vực sản xuất là $90$ dB. Nếu quản đốc cho bật thêm một hệ thống máy móc có cường độ âm (đo bằng $W/m^2$) đúng bằng hệ thống hiện tại, mức ồn mới sẽ xấp xỉ bao nhiêu? (Biết $log 2 approx 0.3$).],
    (
        [$180$ dB],
        [$90.3$ dB],
        True([$93$ dB]),
...
// TN 7
#lt-tn(num: 7, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Bác An đầu tư $500$ triệu đồng vào một chứng chỉ quỹ với lãi suất dự kiến $8%$/năm, tính lãi kép liên tục (công thức $A = P e^(r t)$). Hỏi sau 10 năm, bác An nhận được tổng số tiền xấp xỉ bao nhiêu?],
    (
        [$1079$ triệu đồng],
...
// TN 8
#lt-tn(num: 8, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Phương trình $log_3 (x^2 - 8x) = 2$ có tổng các nghiệm bằng bao nhiêu?],
    (
        True([$8$]),
        [$-8$],
        [$9$],
...
// TN 9
#lt-tn(num: 9, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Chu kì bán rã của chất phóng xạ Polonium-210 là $138$ ngày. Ban đầu có $10$ gam Polonium-210. Khối lượng còn lại sau $276$ ngày là],
    (
        [$5$ gam],
        True([$2.5$ gam]),
        [$1.25$ gam],
...
// TN 10
#lt-tn(num: 10, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Khảo sát một hồ nước, người ta thấy độ sâu tối đa ánh sáng xuyên tới để thực vật thủy sinh có thể quang hợp là nơi có cường độ sáng bằng $1%$ cường độ sáng mặt hồ. Giả sử cường độ ánh sáng $I$ tại độ sâu $d$ (mét) là $I(d) = I_0 e^(-0.2 d)$. Thực vật thủy sinh có thể sống ở độ sâu tối đa bao nhiêu mét?],
    (
        [$15$ m],
        [$10$ m],
        True([$23$ m]),
...
// TN 11
#lt-tn(num: 11, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",[Năng lượng $E$ (tính bằng Erg) giải phóng ra từ một trận động đất có độ Richter là $M$ được xác định theo công thức $log E = 11.4 + 1.5 M$. Trận động đất lịch sử ở Nhật Bản năm 2011 (9.0 độ Richter) tỏa ra năng lượng gấp bao nhiêu lần trận động đất ở Haiti năm 2010 (7.0 độ Richter)?],
    (
        [$2$ lần],
        [$3$ lần],
        True([$1000$ lần]),
...
// DS 1
#lt-ds(num: 13, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",
...
// DS 2
#lt-ds(num: 14, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",
...
// DS 3
#lt-ds(num: 15, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",
...
// DS 4
#lt-ds(num: 16, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",
...
// TL 1
#lt-tln(num: 17, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",
...
// TL 2
#lt-tln(num: 18, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",
...
// TL 3
#lt-tln(num: 19, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",
...
// TL 4
#lt-tln(num: 20, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",
...
// TL 5
#lt-tln(num: 21, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",
...
// TL 6
#lt-tln(num: 22, de: [Đề 2 (de02) — Mã 101], back-to: "sec-exercise-hub",
...
#slide(title: none)[
  #align(center + horizon)[
    #block(
      ...
      #text(weight: "bold", fill: rgb("#a5b4fc"), size: 22pt)[🎉 CHÚC MỪNG HOÀN THÀNH CHƯƠNG 2. MŨ VÀ LOGARIT (ĐỀ SỐ 02)!]      #v(0.6em)
