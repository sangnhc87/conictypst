// ═══════════════════════════════════════════════════════════════════════════
// BEAMER ĐỀ THI: CHỦ ĐỀ 1B: TỐI ƯU HÓA HÌNH HỌC KHÔNG GIAN 3D & KHỐI TRÒN XOAY (VD - VDC)
// Lớp: Khối 12  ·  Mã đề: 502  ·  GV: Nguyễn Văn Sang
...
#show: lecture-theme.with(
  title: "CHỦ ĐỀ 1B: TỐI ƯU HÓA HÌNH HỌC KHÔNG GIAN 3D & KHỐI TRÒN XOAY (VD - VDC)",
  subtitle: "CHUYÊN ĐỀ 1: BÀI TOÁN TỐI ƯU HÓA TRONG THỰC TIỄN — MÃ ĐỀ: 502",
...
#slide(title: none)[
  ...
  #context {
    ...
    grid(
      ...
      pad(left: 8pt)[#text(size: 9pt, fill: s.muted, weight: "bold")[MÃ ĐỀ: 502]],
...
// TN 1
#lt-tn(num: 1, de: [Đề cd01B — Mã 502], back-to: "sec-exercise-hub",[Một công ty sản xuất thùng chứa hóa chất hình trụ có nắp đậy bằng tôn với dung tích cố định $V = 54 pi "m"^3$. Để tiết kiệm vật liệu nhất (diện tích toàn phần của hình trụ là nhỏ nhất) thì chiều cao $h$ của thùng chứa phải bằng bao nhiêu mét?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 2
#lt-tn(num: 2, de: [Đề cd01B — Mã 502], back-to: "sec-exercise-hub",[Trong tất cả các khối trụ nội tiếp trong một mặt cầu bán kính $R = 3 sqrt(3)$ cm, khối trụ có thể tích lớn nhất bằng bao nhiêu xăng-ti-mét khối?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 3
#lt-tn(num: 3, de: [Đề cd01B — Mã 502], back-to: "sec-exercise-hub",[Trong các khối nón nội tiếp trong một mặt cầu bán kính $R = 9$ cm, thể tích lớn nhất của khối nón bằng bao nhiêu?],
    (
        True([$256 pi "cm"^3$]),
        [$128 pi "cm"^3$],
        [$512 pi "cm"^3$],
...
// TN 4
#lt-tn(num: 4, de: [Đề cd01B — Mã 502], back-to: "sec-exercise-hub",[Một khối nón tròn xoay có đường sinh cố định $l = 6$ cm. Thể tích lớn nhất của khối nón này bằng bao nhiêu?],
    (
...
// TN 5
#lt-tn(num: 5, de: [Đề cd01B — Mã 502], back-to: "sec-exercise-hub",[Người ta cần thiết kế một chiếc bồn chứa nước hình trụ không có nắp đậy với dung tích $V = 27 pi "m"^3$. Chi phí làm đáy bồn là 1,5 triệu đồng/m#super[2], chi phí làm thành bồn xung quanh là 1,0 triệu đồng/m#super[2]. Chi phí tối thiểu để sản xuất chiếc bồn này bằng bao nhiêu triệu đồng?],
    (
...
// TN 6
#lt-tn(num: 6, de: [Đề cd01B — Mã 502], back-to: "sec-exercise-hub",[Một xưởng làm mô hình muốn cắt một hình quạt tròn từ một tấm bìa hình tròn bán kính $R = 10$ cm để cuộn lại thành một chiếc phễu hình nón có thể tích lớn nhất. Góc ở tâm $alpha$ của hình quạt tròn đó bằng bao nhiêu radian?],
    (
...
// TN 7
#lt-tn(num: 7, de: [Đề cd01B — Mã 502], back-to: "sec-exercise-hub",[Cho một hình nón có chiều cao $H = 12$ cm và bán kính đáy $R = 6$ cm. Người ta đặt vào bên trong hình nón một khối trụ có một đáy nằm trên đáy của hình nón và đường tròn đáy còn lại nằm trên mặt xung quanh của hình nón. Thể tích lớn nhất của khối trụ này bằng bao nhiêu?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 8
#lt-tn(num: 8, de: [Đề cd01B — Mã 502], back-to: "sec-exercise-hub",[Một khối hộp chữ nhật có đáy là hình vuông và tổng diện tích tất cả các mặt là $150 "cm"^2$. Thể tích lớn nhất của khối hộp chữ nhật đó bằng bao nhiêu?],
    (
        True([$125 "cm"^3$]),
        [$100 "cm"^3$],
        [$150 "cm"^3$],
...
// TN 9
#lt-tn(num: 9, de: [Đề cd01B — Mã 502], back-to: "sec-exercise-hub",[Một lều cắm trại có dạng hình chóp tứ giác đều $S.A B C D$ có tất cả các cạnh bên bằng $a = 3$ m. Thể tích không gian bên trong lều lớn nhất khi chiều cao của lều bằng bao nhiêu mét?],
    (
        True([$sqrt(3)$ m]),
        [$1","5$ m],
        [$2$ m],
...
// TN 10
#lt-tn(num: 10, de: [Đề cd01B — Mã 502], back-to: "sec-exercise-hub",[Một bồn chứa khí hóa lỏng có dạng một khối trụ ở giữa và hai nửa khối cầu có cùng bán kính gắn khít vào hai đầu. Biết thể tích toàn bộ bồn chứa là $V = 36 pi "m"^3$. Để chi phí chế tạo vỏ bồn chứa là thấp nhất (diện tích toàn phần nhỏ nhất) thì bán kính của phần bán cầu bằng bao nhiêu mét?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 11
#lt-tn(num: 11, de: [Đề cd01B — Mã 502], back-to: "sec-exercise-hub",[Một chiếc cốc giấy có dạng hình nón không nắp có thể tích $V = 9 sqrt(3) pi "cm"^3$. Để lượng giấy dùng làm cốc là ít nhất (diện tích xung quanh nhỏ nhất) thì góc ở đỉnh của hình nón bằng bao nhiêu?],
    (
...
// TN 12
#lt-tn(num: 12, de: [Đề cd01B — Mã 502], back-to: "sec-exercise-hub",[Trong không gian $O x y z$, cho hai điểm $A(1; 2; 3)$ và $B(5; 4; 7)$. Điểm $M(x; y; 0)$ di động trên mặt phẳng tọa độ $(O x y)$. Giá trị nhỏ nhất của tổng khoảng cách $M A + M B$ bằng bao nhiêu?],
    (
        True([$2 sqrt(30)$]),
        [$2 sqrt(38)$],
        [$6 sqrt(2)$],
...
// DS 1
#lt-ds(num: 13, de: [Đề cd01B — Mã 502], back-to: "sec-exercise-hub",[Một công ty bao bì thực phẩm cần sản xuất các lon nước ngọt bằng nhôm có dạng hình trụ tròn xoay với thể tích chuẩn $V = 330$ ml ($330 "cm"^3$). Do yêu cầu kỹ thuật, nắp và đáy lon được dập từ nhôm dày hơn có giá thành 300 đồng/cm#super[2], trong khi thân lon (mặt xung quanh) được làm từ lá nhôm mỏng hơn có giá thành 100 đồng/cm#super[2].
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// DS 2
#lt-ds(num: 14, de: [Đề cd01B — Mã 502], back-to: "sec-exercise-hub",[Một bể bơi có dạng hình lăng trụ đứng đáy là tam giác vuông cân có cạnh góc vuông bằng $x$ (m) và chiều sâu của bể là $h$ (m). Biết dung tích của bể bơi là $V = 32 "m"^3$. Người ta cần lát gạch toàn bộ mặt đáy và các mặt thành bên trong của bể bơi.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// DS 3
#lt-ds(num: 15, de: [Đề cd01B — Mã 502], back-to: "sec-exercise-hub",[Cho hình nón tròn xoay có bán kính đáy $R$ và chiều cao $H$. Một khối cầu bán kính $r$ nội tiếp trong hình nón (tiếp xúc với mặt đáy và tất cả các đường sinh của hình nón).
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TLN 1
#lt-tln(num: 17, de: [Đề cd01B — Mã 502], back-to: "sec-exercise-hub",[Một thùng chứa nước bằng tôn có dạng hình trụ không có nắp với dung tích $V = 16 pi "m"^3$. Để tiết kiệm tôn nhất (diện tích toàn phần gồm diện tích xung quanh và một đáy là nhỏ nhất), bán kính đáy của thùng nước bằng bao nhiêu mét?],
    [2],
    loigiai: [
        #step([Lời giải chi tiết])
...
// TLN 2
#lt-tln(num: 18, de: [Đề cd01B — Mã 502], back-to: "sec-exercise-hub",[Một khối trụ có bán kính đáy $R = 4$ cm và chiều cao $h = 6$ cm. Người ta khoét bỏ ở hai đầu khối trụ hai khối nón có đáy trùng với hai đáy của khối trụ và đỉnh của hai khối nón trùng với trung điểm của đoạn nối tâm hai đáy khối trụ. Thể tích của phần khối trụ còn lại bằng bao nhiêu $pi "cm"^3$? (Điền hệ số trước $pi$).],
    [64],
    loigiai: [
        #step([Lời giải chi tiết])
...
// TLN 3
#lt-tln(num: 19, de: [Đề cd01B — Mã 502], back-to: "sec-exercise-hub",[Một khối cầu bằng kim loại có bán kính $R = 6$ cm. Người thợ tiện gọt khối cầu đó để tạo thành một khối nón có thể tích lớn nhất nội tiếp trong khối cầu. Chiều cao của khối nón tiện được bằng bao nhiêu cm?],
    [8],
    loigiai: [
        #step([Lời giải chi tiết])
...
// TLN 5
#lt-tln(num: 21, de: [Đề cd01B — Mã 502], back-to: "sec-exercise-hub",[Cho một khối chóp tứ giác đều $S.A B C D$ nội tiếp trong một mặt cầu bán kính $R = 9$ cm. Thể tích lớn nhất của khối chóp này bằng bao nhiêu xăng-ti-mét khối?],
    [576],
    loigiai: [
        #step([Lời giải chi tiết])
...
// TLN 6
#lt-tln(num: 22, de: [Đề cd01B — Mã 502], back-to: "sec-exercise-hub",[Một hình trụ có bán kính đáy $r = 3$ cm và chiều cao $h = 8$ cm. Một mặt phẳng $(P)$ song song với trục của hình trụ và cách trục một khoảng bằng $sqrt(5)$ cm. Thiết diện của hình trụ cắt bởi mặt phẳng $(P)$ là một hình chữ nhật có diện tích bằng bao nhiêu xăng-ti-mét vuông?],
    [32],
    loigiai: [
        #step([Lời giải chi tiết])
...
#slide(title: none)[
  #align(center + horizon)[
    #block(
      ...
      #text(weight: "bold", fill: rgb("#a5b4fc"), size: 22pt)[🎉 CHÚC MỪNG HOÀN THÀNH CHỦ ĐỀ 1B: TỐI ƯU HÓA HÌNH HỌC KHÔNG GIAN 3D & KHỐI TRÒN XOAY (VD - VDC)!]      #v(0.6em)
