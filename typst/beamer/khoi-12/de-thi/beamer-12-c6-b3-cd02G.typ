// ═══════════════════════════════════════════════════════════════════════════
// BEAMER ĐỀ THI: CHỦ ĐỀ 2G: THUẾ THU NHẬP CÁ NHÂN & TỐI ƯU HÓA THU NHẬP (VD - VDC)
// Lớp: Khối 12  ·  Mã đề: 517  ·  GV: Nguyễn Văn Sang
...
#show: lecture-theme.with(
  title: "CHỦ ĐỀ 2G: THUẾ THU NHẬP CÁ NHÂN & TỐI ƯU HÓA THU NHẬP (VD - VDC)",
  subtitle: "CHUYÊN ĐỀ 2: ỨNG DỤNG TOÁN HỌC TRONG TÀI CHÍNH — MÃ ĐỀ: 517",
...
#slide(title: none)[
  ...
  #context {
    ...
    grid(
      ...
      pad(left: 8pt)[#text(size: 9pt, fill: s.muted, weight: "bold")[MÃ ĐỀ: 517]],
...
// TN 1
#lt-tn(num: 1, de: [Đề cd02G — Mã 517], back-to: "sec-exercise-hub",[Một kỹ sư có thu nhập tính thuế sau khi đã trừ đi các khoản đóng bảo hiểm bắt buộc và các khoản giảm trừ gia cảnh là $"TNTT" = 15$ triệu đồng mỗi tháng. Áp dụng Biểu thuế lũy tiến từng phần theo quy định của Luật Thuế thu nhập cá nhân hiện hành:
...
// TN 4
#lt-tn(num: 4, de: [Đề cd02G — Mã 517], back-to: "sec-exercise-hub",[Trong kinh tế học tài chính, hiện tượng "Leo bậc thuế" (Bracket Creep) do lạm phát xảy ra khi
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 5
#lt-tn(num: 5, de: [Đề cd02G — Mã 517], back-to: "sec-exercise-hub",[Một doanh nghiệp xem xét hai phương án thưởng định kỳ hàng tháng cho một cán bộ quản lý đang có mức thu nhập tính thuế nằm ở Bậc 4 (thuế suất biên $20\%$):
...
// TN 7
#lt-tn(num: 7, de: [Đề cd02G — Mã 517], back-to: "sec-exercise-hub",[Theo Luật Thuế TNCN và các quy định hướng dẫn, người lao động tham gia đóng góp vào Quỹ hưu trí tự nguyện được trừ tối đa $1$ triệu đồng mỗi tháng ($12$ triệu đồng mỗi năm) vào thu nhập tính thuế. Một nhân viên quản lý cấp cao có thu nhập tính thuế nằm ở Bậc 5 (thuế suất biên $25\%$). Nếu nhân viên này trích đủ $12$ triệu đồng mỗi năm để đóng vào quỹ hưu trí tự nguyện, thì số tiền thuế TNCN thực tế mà nhân viên này tiết kiệm được trong năm là bao nhiêu?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 9
#lt-tn(num: 9, de: [Đề cd02G — Mã 517], back-to: "sec-exercise-hub",[Xét hàm thu nhập sau thuế $N(x) = x - T(x)$, trong đó $x$ là thu nhập tính thuế và $T(x)$ là tiền thuế TNCN tính theo biểu thuế lũy tiến từng phần với thuế suất biên $t_k in [5\%, 35\%]$. Nhận định nào sau đây là đúng về mặt toán học đối với hàm thu nhập sau thuế $N(x)$?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 10
#lt-tn(num: 10, de: [Đề cd02G — Mã 517], back-to: "sec-exercise-hub",[Một cá nhân chuyển nhượng một căn nhà gắn liền với quyền sử dụng đất với giá trị ghi trên hợp đồng công chứng là $3$ tỷ đồng ($3000$ triệu đồng). Theo quy định của Luật Thuế TNCN, thuế suất đối với hoạt động chuyển nhượng bất động sản là $2\%$ tính trên giá trị chuyển nhượng từng lần. Số tiền thuế TNCN mà cá nhân này phải nộp khi thực hiện chuyển nhượng bất động sản là
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 11
#lt-tn(num: 11, de: [Đề cd02G — Mã 517], back-to: "sec-exercise-hub",[Một khách hàng may mắn trúng giải độc đắc xổ số điện toán với tổng giá trị giải thưởng là $2$ tỷ đồng ($2000$ triệu đồng). Theo Luật Thuế TNCN, thu nhập từ trúng thưởng chịu thuế suất $10\%$ tính trên phần giá trị giải thưởng vượt trên $10$ triệu đồng cho mỗi vé trúng thưởng. Số tiền thực nhận sau thuế mà khách hàng này được nhận về là
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 12
#lt-tn(num: 12, de: [Đề cd02G — Mã 517], back-to: "sec-exercise-hub",[Trong một gia đình, hai vợ chồng có mức thu nhập khác nhau:
...
// DS 1
#lt-ds(num: 13, de: [Đề cd02G — Mã 517], back-to: "sec-exercise-hub",[Xét hàm thuế thu nhập cá nhân $T(x)$ tính theo biểu thuế lũy tiến từng phần với $x >= 0$ là thu nhập tính thuế hàng tháng (đơn vị: triệu đồng). Biểu thuế quy định $7$ bậc với các ngưỡng thu nhập $5; 10; 18; 32; 52; 80$ triệu đồng và các mức thuế suất tương ứng $5\%, 10\%, 15\%, 20\%, 25\%, 30\%, 35\%$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// DS 2
#lt-ds(num: 14, de: [Đề cd02G — Mã 517], back-to: "sec-exercise-hub",[Một ứng viên quản lý cấp trung được công ty mời vào làm việc với hai phương án đề xuất hợp đồng lao động:
...
// DS 3
#lt-ds(num: 15, de: [Đề cd02G — Mã 517], back-to: "sec-exercise-hub",[Một doanh nghiệp thuê một chuyên gia nước ngoài sang làm việc với mức lương chịu thuế cố định là $100$ triệu đồng mỗi tháng. Ngoài tiền lương, công ty thuê cho chuyên gia một căn hộ chung cư cao cấp với chi phí thuê nhà thực tế là $20$ triệu đồng/tháng do công ty thanh toán trực tiếp cho chủ nhà.
...
// DS 4
#lt-ds(num: 16, de: [Đề cd02G — Mã 517], back-to: "sec-exercise-hub",[Một kỹ sư có thu nhập tính thuế bình thường ổn định là $30$ triệu đồng/tháng (đang chịu thuế suất biên $20\%$, Bậc 4). Do yêu cầu tiến độ dự án, kỹ sư được công ty đề nghị làm thêm giờ vào các ngày nghỉ cuối tuần với tổng tiền công làm thêm giờ là $10$ triệu đồng.
...
// TLN 1
#lt-tln(num: 17, de: [Đề cd02G — Mã 517], back-to: "sec-exercise-hub",[Một người lao động có thu nhập tính thuế sau khi giảm trừ là $"TNTT" = 8$ triệu đồng mỗi tháng (nằm ở Bậc 2 của biểu thuế lũy tiến: $5$ triệu đầu chịu $5\%$, phần trên $5$ triệu đến $10$ triệu chịu $10\%$). Số tiền thuế TNCN mà người này phải nộp hàng tháng là bao nhiêu nghìn đồng?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TLN 3
#lt-tln(num: 19, de: [Đề cd02G — Mã 517], back-to: "sec-exercise-hub",[Một chuyên gia có thu nhập tính thuế hàng tháng là $"TNTT" = 45$ triệu đồng. Theo Biểu thuế lũy tiến từng phần, Bậc 5 áp dụng cho phần thu nhập trên $32$ đến $52$ triệu đồng với thuế suất $25\%$ và công thức tính nhanh số thuế phải nộp là:
...
// TLN 4
#lt-tln(num: 20, de: [Đề cd02G — Mã 517], back-to: "sec-exercise-hub",[Một cá nhân trúng thưởng giải nhất xổ số truyền thống với giá trị giải thưởng là $1","5$ tỷ đồng ($1500$ triệu đồng). Biết thuế suất thuế TNCN đối với thu nhập từ trúng thưởng là $10\%$ tính trên phần giá trị giải thưởng vượt trên $10$ triệu đồng. Số tiền thuế TNCN mà cá nhân này phải nộp là bao nhiêu triệu đồng?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TLN 5
#lt-tln(num: 21, de: [Đề cd02G — Mã 517], back-to: "sec-exercise-hub",[Một người bán một lô đất với giá chuyển nhượng trên hợp đồng công chứng là $4","5$ tỷ đồng ($4500$ triệu đồng). Thuế suất thuế TNCN đối với hoạt động chuyển nhượng bất động sản là $2\%$ tính trên giá trị chuyển nhượng. Số tiền thuế TNCN người này phải nộp là bao nhiêu triệu đồng?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TLN 6
#lt-tln(num: 22, de: [Đề cd02G — Mã 517], back-to: "sec-exercise-hub",[Trong Biểu thuế thu nhập cá nhân lũy tiến từng phần hiện hành của Việt Nam áp dụng đối với thu nhập từ tiền lương, tiền công, mức thuế suất biên cao nhất (ở Bậc 7 - áp dụng cho phần thu nhập tính thuế trên $80$ triệu đồng/tháng) là bao nhiêu phần trăm?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#slide(title: none)[
  #align(center + horizon)[
    #block(
      ...
      #text(weight: "bold", fill: rgb("#a5b4fc"), size: 22pt)[🎉 CHÚC MỪNG HOÀN THÀNH CHỦ ĐỀ 2G: THUẾ THU NHẬP CÁ NHÂN & TỐI ƯU HÓA THU NHẬP (VD - VDC)!]      #v(0.6em)
