// ═══════════════════════════════════════════════════════════════════════════
// BEAMER ĐỀ THI: ĐỀ ÔN TẬP CHUYÊN ĐỀ 3A: KHÁI NIỆM BIẾN NGẪU NHIÊN RỜI RẠC & BẢNG PHÂN BỐ XÁC SUẤT
// Lớp: Khối 12  ·  Mã đề: 521  ·  GV: Nguyễn Văn Sang
...
#show: lecture-theme.with(
  title: "ĐỀ ÔN TẬP CHUYÊN ĐỀ 3A: KHÁI NIỆM BIẾN NGẪU NHIÊN RỜI RẠC & BẢNG PHÂN BỐ XÁC SUẤT",
  subtitle: "CHUYÊN ĐỀ 3: BIẾN NGẪU NHIÊN RỜI RẠC & CÁC SỐ ĐẶC TRƯNG — MÃ ĐỀ: 521",
...
#slide(title: none)[
  ...
  #context {
    ...
    grid(
      ...
      pad(left: 8pt)[#text(size: 9pt, fill: s.muted, weight: "bold")[MÃ ĐỀ: 521]],
...
// TN 1
#lt-tn(num: 1, de: [Đề cd03A — Mã 521], back-to: "sec-exercise-hub",[Trong các đại lượng sau đây trong thực tiễn đời sống và khoa học kỹ thuật, đại lượng nào là một ví dụ về biến ngẫu nhiên rời rạc?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 2
#lt-tn(num: 2, de: [Đề cd03A — Mã 521], back-to: "sec-exercise-hub",[Một biến ngẫu nhiên rời rạc $X$ có bảng phân bố xác suất như sau:
#align(center)[
  #table(
    columns: 5,
    align: center,
...
// TN 3
#lt-tn(num: 3, de: [Đề cd03A — Mã 521], back-to: "sec-exercise-hub",[Một trung tâm chăm sóc khách hàng thống kê số lượng cuộc gọi khiếu nại $X$ nhận được trong một khung giờ cao điểm với bảng phân bố xác suất:
#align(center)[
  #table(
    columns: 5,
    align: center,
...
// TN 4
#lt-tn(num: 4, de: [Đề cd03A — Mã 521], back-to: "sec-exercise-hub",[Tung đồng thời $3$ đồng xu cân đối và đồng chất. Gọi $X$ là biến ngẫu nhiên chỉ số lần xuất hiện mặt ngửa ($N$). Bảng phân bố xác suất của biến ngẫu nhiên $X$ là:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 5
#lt-tn(num: 5, de: [Đề cd03A — Mã 521], back-to: "sec-exercise-hub",[Một hộp chứa $5$ quả cầu màu đỏ và $3$ quả cầu màu xanh có cùng kích thước. Lấy ngẫu nhiên đồng thời $2$ quả cầu từ hộp. Gọi $X$ là số quả cầu màu đỏ lấy được. Xác suất $P(X = 1)$ bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 6
#lt-tn(num: 6, de: [Đề cd03A — Mã 521], back-to: "sec-exercise-hub",[Hai xạ thủ A và B độc lập cùng bắn mỗi người $1$ viên đạn vào một bia mục tiêu. Xác suất bắn trúng bia của A và B lần lượt là $0","7$ và $0","8$. Gọi $X$ là tổng số viên đạn bắn trúng bia. Xác suất để $X = 1$ là:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 7
#lt-tn(num: 7, de: [Đề cd03A — Mã 521], back-to: "sec-exercise-hub",[Cho biến ngẫu nhiên rời rạc $X$ có bảng phân bố xác suất như sau:
#align(center)[
  #table(
    columns: 5,
    align: center,
...
// TN 8
#lt-tn(num: 8, de: [Đề cd03A — Mã 521], back-to: "sec-exercise-hub",[Biến ngẫu nhiên $X$ có bảng phân bố xác suất:
#align(center)[
  #table(
    columns: 4,
    align: center,
...
// TN 9
#lt-tn(num: 9, de: [Đề cd03A — Mã 521], back-to: "sec-exercise-hub",[Một nhân viên tiếp thị gọi điện chào hàng cho $3$ khách hàng độc lập. Xác suất để mỗi khách hàng đồng ý mua hàng là $0","2$. Gọi $X$ là số khách hàng đồng ý mua hàng. Xác suất để có ít nhất một khách hàng đồng ý mua hàng bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    for i in range(3) {
...
// TN 10
#lt-tn(num: 10, de: [Đề cd03A — Mã 521], back-to: "sec-exercise-hub",[Một biến ngẫu nhiên rời rạc $X$ nhận các giá trị thuộc tập $\{1, 2, 3, 4\}$ với quy luật xác suất $P(X = k) = c / 2^k$ (với $c$ là một hằng số thực dương). Giá trị của $c$ bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    line((-0.5, 0), (4.5, 0), stroke: 0.8pt + gray)
...
// TN 11
#lt-tn(num: 11, de: [Đề cd03A — Mã 521], back-to: "sec-exercise-hub",[Một đề kiểm tra có $4$ câu hỏi trắc nghiệm khách quan độc lập, mỗi câu có $4$ phương án lựa chọn trong đó chỉ có $1$ phương án đúng. Một học sinh không học bài nên khoanh ngẫu nhiên hoàn toàn cả $4$ câu. Gọi $X$ là số câu học sinh trả lời đúng. Xác suất để học sinh trả lời đúng đúng $2$ câu là:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    for i in range(4) {
...
// TN 12
#lt-tn(num: 12, de: [Đề cd03A — Mã 521], back-to: "sec-exercise-hub",[Gieo hai con xúc xắc cân đối và đồng chất. Gọi $X$ là giá trị lớn nhất trong hai mặt xuất hiện của hai con xúc xắc, tức là $X = max(X_1, X_2)$ với $X_1, X_2 in {1, 2, 3, 4, 5, 6}$. Xác suất $P(X = 4)$ bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    let s = 0.35
...
// DS 1
#lt-ds(num: 13, de: [Đề cd03A — Mã 521], back-to: "sec-exercise-hub",[Một hộp quà gồm $10$ phong bao lì xì bề ngoài hoàn toàn giống nhau, bên trong chứa các tờ tiền mệnh giá khác nhau:
...
// DS 2
#lt-ds(num: 14, de: [Đề cd03A — Mã 521], back-to: "sec-exercise-hub",[Một hộp có chứa $4$ viên bi trắng và $6$ viên bi đen. Lấy ngẫu nhiên không hoàn lại lần lượt từng viên cho đến khi lấy được viên bi trắng thì dừng lại. Gọi $X$ là số lần lấy bi.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// DS 3
#lt-ds(num: 15, de: [Đề cd03A — Mã 521], back-to: "sec-exercise-hub",[Một dây chuyền sản xuất vi mạch điện tử hoạt động tự động. Xác suất để dây chuyền tạo ra một vi mạch bị lỗi là $p = 0","05$. Để kiểm tra chất lượng định kỳ, kỹ sư lấy ngẫu nhiên độc lập $3$ vi mạch từ dây chuyền. Gọi $X$ là số vi mạch bị lỗi trong $3$ vi mạch được lấy ra.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    for i in range(3) {
...
// DS 4
#lt-ds(num: 16, de: [Đề cd03A — Mã 521], back-to: "sec-exercise-hub",[Cho biến ngẫu nhiên rời rạc $X$ có hàm phân bố xác suất tích lũy $F(x) = P(X <= x)$ được xác định bởi công thức:
$ F(x) = cases(
  0 & "khi" x < 1,
  0","2 & "khi" 1 <= x < 3,
  0","6 & "khi" 3 <= x < 5,
...
// TLN 1
#lt-tln(num: 17, de: [Đề cd03A — Mã 521], back-to: "sec-exercise-hub",[Một biến ngẫu nhiên rời rạc $X$ có bảng phân bố xác suất như sau:
#align(center)[
  #table(
    columns: 5,
    align: center,
...
// TLN 2
#lt-tln(num: 18, de: [Đề cd03A — Mã 521], back-to: "sec-exercise-hub",[Một xạ thủ bắn $3$ phát đạn độc lập vào mục tiêu. Xác suất bắn trúng mục tiêu của từng phát lần lượt là $p_1 = 0","6$; $p_2 = 0","7$ và $p_3 = 0","8$. Gọi $X$ là số viên đạn bắn trúng mục tiêu. Tính xác suất để xạ thủ bắn trúng đúng $2$ phát đạn (viết kết quả dưới dạng số thập phân).
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    for i in range(3) {
...
// TLN 3
#lt-tln(num: 19, de: [Đề cd03A — Mã 521], back-to: "sec-exercise-hub",[Một hộp chứa $6$ quả cầu đỏ và $4$ quả cầu xanh. Lấy ngẫu nhiên đồng thời $3$ quả cầu. Gọi $X$ là số quả cầu đỏ lấy được. Xác suất để lấy được ít nhất $2$ quả cầu đỏ bằng bao nhiêu? (Viết kết quả dưới dạng phân số tối giản $a/b$, nhập vào ô đáp án giá trị của tổng $a + b$).
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TLN 4
#lt-tln(num: 20, de: [Đề cd03A — Mã 521], back-to: "sec-exercise-hub",[Một biến ngẫu nhiên rời rạc $X$ có bảng phân bố xác suất thỏa mãn:
...
// TLN 5
#lt-tln(num: 21, de: [Đề cd03A — Mã 521], back-to: "sec-exercise-hub",[Một trò chơi quay bánh xe trúng thưởng gồm $10$ ô có diện tích bằng nhau. Trên các ô có ghi số điểm như sau:
- $5$ ô ghi số $0$ điểm.
- $3$ ô ghi số $10$ điểm.
- $2$ ô ghi số $50$ điểm.
...
// TLN 6
#lt-tln(num: 22, de: [Đề cd03A — Mã 521], back-to: "sec-exercise-hub",[Gieo đồng thời hai con xúc xắc cân đối và đồng chất. Gọi $X$ là tổng số chấm xuất hiện trên hai con xúc xắc. Tính xác suất để $X$ là một số nguyên tố (viết kết quả dưới dạng phân số tối giản $a/b$, điền vào ô đáp án giá trị của tử số $a$).
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
#slide(title: none)[
  #align(center + horizon)[
    #block(
      ...
      #text(weight: "bold", fill: rgb("#a5b4fc"), size: 22pt)[🎉 CHÚC MỪNG HOÀN THÀNH ĐỀ ÔN TẬP CHUYÊN ĐỀ 3A: KHÁI NIỆM BIẾN NGẪU NHIÊN RỜI RẠC & BẢNG PHÂN BỐ XÁC SUẤT!]      #v(0.6em)
