// ═══════════════════════════════════════════════════════════════════════════
// BEAMER ĐỀ THI: ĐỀ ÔN TẬP CHUYÊN ĐỀ 3D: PHÉP THỬ BERNOULLI & PHÂN BỐ XÁC SUẤT NHỊ THỨC B(n, p)
// Lớp: Khối 12  ·  Mã đề: 524  ·  GV: Nguyễn Văn Sang
...
#show: lecture-theme.with(
  title: "ĐỀ ÔN TẬP CHUYÊN ĐỀ 3D: PHÉP THỬ BERNOULLI & PHÂN BỐ XÁC SUẤT NHỊ THỨC B(n, p)",
  subtitle: "CHUYÊN ĐỀ 3: BIẾN NGẪU NHIÊN RỜI RẠC & CÁC SỐ ĐẶC TRƯNG — MÃ ĐỀ: 524",
...
#slide(title: none)[
  ...
  #context {
    ...
    grid(
      ...
      pad(left: 8pt)[#text(size: 9pt, fill: s.muted, weight: "bold")[MÃ ĐỀ: 524]],
...
// TN 1
#lt-tn(num: 1, de: [Đề cd03D — Mã 524], back-to: "sec-exercise-hub",[Gieo một con xúc xắc cân đối và đồng chất $5$ lần độc lập. Gọi $X$ là số lần xuất hiện mặt $6$ chấm trong $5$ lần gieo đó. Biến ngẫu nhiên $X$ tuân theo quy luật phân bố xác suất nào và xác suất để mặt $6$ chấm xuất hiện đúng $2$ lần bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 2
#lt-tn(num: 2, de: [Đề cd03D — Mã 524], back-to: "sec-exercise-hub",[Một xạ thủ bắn súng thể thao bắn độc lập $4$ phát đạn vào bia. Xác suất bắn trúng mục tiêu của xạ thủ trong mỗi phát bắn đều bằng $p = 0","7$. Xác suất để xạ thủ bắn trúng mục tiêu đúng $3$ phát bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 3
#lt-tn(num: 3, de: [Đề cd03D — Mã 524], back-to: "sec-exercise-hub",[Thực hiện $n = 20$ phép thử Bernoulli độc lập với xác suất thành công trong mỗi phép thử là $p = 0","35$. Giá trị tin chắc nhất (số lần thành công có xác suất xuất hiện lớn nhất) $k_0$ của biến ngẫu nhiên $X ~ B(20; 0","35)$ bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 4
#lt-tn(num: 4, de: [Đề cd03D — Mã 524], back-to: "sec-exercise-hub",[Thực hiện $n = 19$ phép thử Bernoulli độc lập với xác suất thành công mỗi phép thử là $p = 0","4$. Khẳng định nào sau đây là đúng về các giá trị tin chắc nhất của biến ngẫu nhiên $X ~ B(19; 0","4)$?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 5
#lt-tn(num: 5, de: [Đề cd03D — Mã 524], back-to: "sec-exercise-hub",[Xác suất trúng thưởng của một vé số cào điện tử là $p = 0","05$ ($5%$). Một người chơi mua $n$ vé độc lập. Số lượng vé tối thiểu người đó cần mua để xác suất có ít nhất một vé trúng thưởng không nhỏ hơn $95%$ ($0","95$) bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 6
#lt-tn(num: 6, de: [Đề cd03D — Mã 524], back-to: "sec-exercise-hub",[Một lô linh kiện điện tử lớn có tỷ lệ sản phẩm lỗi là $p = 0","02$ ($2%$). Chọn ngẫu nhiên có hoàn lại $100$ linh kiện để kiểm tra chất lượng. Xác suất để trong mẫu kiểm tra có không quá $1$ sản phẩm lỗi xấp xỉ bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 7
#lt-tn(num: 7, de: [Đề cd03D — Mã 524], back-to: "sec-exercise-hub",[Trong một kênh truyền tin nhị phân đối xứng (Binary Symmetric Channel - BSC), mỗi bit dữ liệu ($0$ hoặc $1$) được truyền đi với xác suất bị lỗi (bị đảo bit từ $0 -> 1$ hoặc $1 -> 0$) là $p = 0","01$. Để truyền một bit thông tin, bên phát gửi đi chuỗi $5$ bit lặp lại (ví dụ muốn gửi bit $1$ thì phát $11111$). Bên nhận giải mã theo nguyên tắc biểu quyết đa số: nếu nhận được từ $3$ bit $1$ trở lên thì quyết định bit gửi là $1$. Xác suất truyền thành công bit $1$ qua kênh truyền có mã lặp này xấp xỉ bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 8
#lt-tn(num: 8, de: [Đề cd03D — Mã 524], back-to: "sec-exercise-hub",[Cho biến ngẫu nhiên nhị thức $X ~ B(n, p)$ với $0 < p < 1$ và đặt $q = 1 - p$. Với mỗi số nguyên $k$ thỏa mãn $0 <= k < n$, tỷ số giữa hai xác suất liên tiếp $frac(P(X = k + 1), P(X = k))$ được rút gọn thành:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 10
#lt-tn(num: 10, de: [Đề cd03D — Mã 524], back-to: "sec-exercise-hub",[Gieo một con xúc xắc cân đối liên tiếp nhiều lần độc lập. Trò chơi dừng lại ngay khi xuất hiện mặt $6$ chấm lần thứ hai, hoặc dừng lại sau khi đã gieo tối đa $4$ lần. Xác suất để trò chơi kết thúc ở đúng lần gieo thứ $3$ bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 11
#lt-tn(num: 11, de: [Đề cd03D — Mã 524], back-to: "sec-exercise-hub",[Cho biến ngẫu nhiên nhị thức $X ~ B(10; 0","3)$. Giá trị tin chắc nhất của biến ngẫu nhiên là $k_0 = 3$. So sánh ba xác suất $P(X = 2)$, $P(X = 3)$ và $P(X = 4)$, khẳng định nào sau đây đúng?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 12
#lt-tn(num: 12, de: [Đề cd03D — Mã 524], back-to: "sec-exercise-hub",[Một biến ngẫu nhiên $X$ tuân theo quy luật phân bố nhị thức $X ~ B(4, p)$ với $0 < p < 1$. Biết rằng xác suất có đúng $2$ lần thành công gấp $1","5$ lần xác suất có đúng $1$ lần thành công, tức là $P(X = 2) = 1","5 dot P(X = 1)$. Xác suất thành công $p$ trong mỗi phép thử bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// DS 1 (Câu 13)
#lt-ds(num: 13, de: [Đề cd03D — Mã 524], back-to: "sec-exercise-hub",[Một dây chuyền công nghệ cao sản xuất màn hình điện thoại OLED với tỷ lệ sản phẩm bị lỗi là $p = 0","05$ ($5%$). Mỗi hộp sản phẩm xuất xưởng chứa $n = 10$ màn hình được chọn ngẫu nhiên độc lập từ dây chuyền. Gọi $X$ là số màn hình lỗi trong một hộp ($X ~ B(10; 0","05)$).
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// DS 3 (Câu 15)
#lt-ds(num: 15, de: [Đề cd03D — Mã 524], back-to: "sec-exercise-hub",[Một vận động viên bắn súng thể thao bắn $n = 15$ phát đạn độc lập vào bia. Xác suất bắn trúng hồng tâm trong mỗi phát bắn là $p = 0","8$. Gọi $X$ là số phát bắn trúng hồng tâm ($X ~ B(15; 0","8)$).
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// DS 4 (Câu 16)
#lt-ds(num: 16, de: [Đề cd03D — Mã 524], back-to: "sec-exercise-hub",[Một loại thuốc kháng sinh mới được thử nghiệm lâm sàng trên $n = 12$ bệnh nhân nhiễm khuẩn nặng độc lập nhau. Xác suất một bệnh nhân được chữa khỏi hoàn toàn khi điều trị bằng loại thuốc này là $p = 0","75$. Gọi $X$ là số bệnh nhân được chữa khỏi ($X ~ B(12; 0","75)$).
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TLN 1 (Câu 17)
#lt-tln(num: 17, de: [Đề cd03D — Mã 524], back-to: "sec-exercise-hub",[Một dây chuyền kiểm định chất lượng sản phẩm thực hiện kiểm tra ngẫu nhiên $n = 45$ linh kiện độc lập. Xác suất để mỗi linh kiện đạt tiêu chuẩn chất lượng là $p = 0","6$. Tìm số linh kiện đạt chuẩn có khả năng xuất hiện cao nhất (giá trị tin chắc nhất $k_0$).
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TLN 2 (Câu 18)
#lt-tln(num: 18, de: [Đề cd03D — Mã 524], back-to: "sec-exercise-hub",[Xác suất để một phần mềm an ninh phát hiện một lỗ hổng bảo mật trong một lượt quét tự động là $p = 0","2$. Cần thực hiện tối thiểu bao nhiêu lượt quét độc lập để xác suất phát hiện được lỗ hổng ít nhất một lần không nhỏ hơn $90%$?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TLN 3 (Câu 19)
#lt-tln(num: 19, de: [Đề cd03D — Mã 524], back-to: "sec-exercise-hub",[Cho biến ngẫu nhiên $X ~ B(4, p)$ với $0 < p < 1$. Biết rằng xác suất có đúng $2$ lần thành công gấp $1","5$ lần xác suất có đúng $1$ lần thành công, tức $P(X = 2) = 1","5 dot P(X = 1)$. Giá trị của tham số $p$ bằng bao nhiêu? (Nhập kết quả dưới dạng số thập phân, ví dụ 0.5).
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TLN 4 (Câu 20)
#lt-tln(num: 20, de: [Đề cd03D — Mã 524], back-to: "sec-exercise-hub",[Một hệ thống truyền dẫn gửi đi $n = 5$ gói tin độc lập qua mạng. Xác suất để mỗi gói tin bị thất lạc trong quá trình truyền là $p = 0","1$. Xác suất để hệ thống bị thất lạc đúng $1$ gói tin bằng bao nhiêu? (Làm tròn kết quả đến hàng phần nghìn, ví dụ 0.328).
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TLN 6 (Câu 22)
#lt-tln(num: 22, de: [Đề cd03D — Mã 524], back-to: "sec-exercise-hub",[Một tàu vũ trụ đổ bộ lên Mặt Trăng được trang bị cụm $4$ động cơ đẩy phản lực hoạt động hoàn toàn độc lập. Xác suất để mỗi động cơ khởi động và vận hành thành công là $p = 0","9$. Tàu đổ bộ hạ cánh an toàn nếu có ít nhất $3$ động cơ hoạt động thành công. Xác suất để tàu vũ trụ hạ cánh an toàn bằng bao nhiêu? (Nhập kết quả dưới dạng số thập phân, ví dụ 0.9477).
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
#slide(title: none)[
  #align(center + horizon)[
    #block(
      ...
      #text(weight: "bold", fill: rgb("#a5b4fc"), size: 22pt)[🎉 CHÚC MỪNG HOÀN THÀNH ĐỀ ÔN TẬP CHUYÊN ĐỀ 3D: PHÉP THỬ BERNOULLI & PHÂN BỐ XÁC SUẤT NHỊ THỨC B(N, P)!]      #v(0.6em)
