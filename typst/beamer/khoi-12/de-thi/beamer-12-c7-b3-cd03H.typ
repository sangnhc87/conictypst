// ═══════════════════════════════════════════════════════════════════════════
// BEAMER ĐỀ THI: ĐỀ ÔN TẬP CHUYÊN ĐỀ 3H: MÔ HÌNH HÀNG ĐỢI RỜI RẠC, TIẾP NHẬN DỊCH VỤ & KIỂM SOÁT CHẤT LƯỢNG
// Lớp: Khối 12  ·  Mã đề: 528  ·  GV: Nguyễn Văn Sang
...
#show: lecture-theme.with(
  title: "ĐỀ ÔN TẬP CHUYÊN ĐỀ 3H: MÔ HÌNH HÀNG ĐỢI RỜI RẠC, TIẾP NHẬN DỊCH VỤ & KIỂM SOÁT CHẤT LƯỢNG",
  subtitle: "CHUYÊN ĐỀ 3: BIẾN NGẪU NHIÊN RỜI RẠC & CÁC SỐ ĐẶC TRƯNG — MÃ ĐỀ: 528",
...
#slide(title: none)[
  ...
  #context {
    ...
    grid(
      ...
      pad(left: 8pt)[#text(size: 9pt, fill: s.muted, weight: "bold")[MÃ ĐỀ: 528]],
...
// TN 1
#lt-tn(num: 1, de: [Đề cd03H — Mã 528], back-to: "sec-exercise-hub",[Số lượng khách hàng đến một quầy giao dịch bưu điện trong khoảng thời gian $15$ phút là biến ngẫu nhiên rời rạc $X$ có bảng phân bố xác suất như sau:
#align(center)[
  #table(
...
// TN 2
#lt-tn(num: 2, de: [Đề cd03H — Mã 528], back-to: "sec-exercise-hub",[Một tổng đài cứu hộ giao thông ghi nhận số cuộc gọi khẩn cấp trong ca trực đêm là biến ngẫu nhiên $X$ có bảng phân bố xác suất:
#align(center)[
  #table(
    columns: (1.2fr, 1fr, 1fr, 1fr, 1fr),
    align: center + horizon,
...
// TN 3
#lt-tn(num: 3, de: [Đề cd03H — Mã 528], back-to: "sec-exercise-hub",[Trên một bản mạch in điện tử (PCB), robot tự động hàn $n = 200$ mối hàn độc lập. Xác suất để một mối hàn bị khuyết tật (lỗi hàn lạnh hoặc cầu chì) là $p = 0","01$. Số mối hàn bị lỗi trên bản mạch $X$ tuân theo phân bố nhị thức $X ~ B(200; 0","01)$. Độ lệch chuẩn của số mối hàn bị lỗi bằng (làm tròn đến hàng phần trăm):
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 4
#lt-tn(num: 4, de: [Đề cd03H — Mã 528], back-to: "sec-exercise-hub",[Một hộp linh kiện gồm $N = 30$ chiếc, trong đó có $M = 6$ linh kiện bị lỗi kỹ thuật. Người ta lấy ngẫu nhiên không hoàn lại $n = 4$ linh kiện để kiểm tra. Xác suất để trong $4$ linh kiện lấy ra không có linh kiện nào bị lỗi bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 5
#lt-tn(num: 5, de: [Đề cd03H — Mã 528], back-to: "sec-exercise-hub",[Khi kiểm định chất lượng lô sản phẩm có quy mô $N$ rất lớn và lấy ngẫu nhiên không hoàn lại một mẫu kích thước $n$, người ta có thể xấp xỉ phân bố siêu bội bằng phân bố nhị thức $B(n, p)$ (với $p = frac(M, N)$ là tỷ lệ phế phẩm) nếu thỏa mãn điều kiện quy chuẩn nào sau đây?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 6
#lt-tn(num: 6, de: [Đề cd03H — Mã 528], back-to: "sec-exercise-hub",[Một quầy phục vụ khách hàng tại một chi nhánh ngân hàng có thể phục vụ tối đa $K = 4$ khách hàng trong mỗi khung giờ $30$ phút. Số lượng khách hàng đến quầy trong khung giờ đó là biến ngẫu nhiên $X$ có bảng phân bố xác suất:
#align(center)[
  #table(
...
// TN 7
#lt-tn(num: 7, de: [Đề cd03H — Mã 528], back-to: "sec-exercise-hub",[Số lỗi in ấn trên mỗi trang của một bản thảo sách giáo khoa là biến ngẫu nhiên rời rạc $X$ có bảng phân bố xác suất:
#align(center)[
  #table(
...
// TN 8
#lt-tn(num: 8, de: [Đề cd03H — Mã 528], back-to: "sec-exercise-hub",[Một trạm thu phí cao tốc có $3$ làn thu phí tự động ETC hoạt động độc lập. Xác suất để mỗi làn gặp trục trặc kỹ thuật đọc thẻ trong một giờ cao điểm đều bằng $p = 0","05$. Xác suất để trong giờ cao điểm có đúng $1$ làn gặp trục trặc kỹ thuật bằng (làm tròn đến hàng phần nghìn):
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    // 3 làn thu phí ETC
...
// TN 11
#lt-tn(num: 11, de: [Đề cd03H — Mã 528], back-to: "sec-exercise-hub",[Trong một kênh truyền thông tin quang học, một chuỗi gồm $n = 1000$ gói dữ liệu được gửi đi độc lập. Xác suất để mỗi gói dữ liệu bị lỗi do nhiễu đường truyền là $p = 0","003$. Kỳ vọng số gói dữ liệu được truyền đi thành công (không bị lỗi) bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// DS 1
#lt-ds(num: 13, de: [Đề cd03H — Mã 528], back-to: "sec-exercise-hub",[Số bệnh nhân đến khám bệnh tại một phòng khám chuyên khoa tim mạch trong khung giờ cao điểm từ 8h đến 9h sáng là biến ngẫu nhiên rời rạc $X$ có bảng phân bố xác suất như sau:
#align(center)[
  #table(
...
// DS 2
#lt-ds(num: 14, de: [Đề cd03H — Mã 528], back-to: "sec-exercise-hub",[Một nhà máy sản xuất chip vi xử lý đóng gói các lô hàng xuất khẩu gồm $N = 2000$ chip với tỷ lệ chip bị khuyết tật là $p = 0","02$. Ban kiểm định chất lượng (QA) lấy ngẫu nhiên không hoàn lại $n = 100$ chip trong mỗi lô để kiểm tra trước khi xuất xưởng.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// DS 3
#lt-ds(num: 15, de: [Đề cd03H — Mã 528], back-to: "sec-exercise-hub",[Một trạm sạc xe điện thông minh tại trạm dừng nghỉ cao tốc có $2$ trụ sạc nhanh độc lập. Trong mỗi khoảng thời gian $30$ phút, số xe điện đến có nhu cầu sạc là biến ngẫu nhiên $X$ có bảng phân bố xác suất:
#align(center)[
  #table(
    columns: (1.2fr, 1fr, 1fr, 1fr, 1fr),
    align: center + horizon,
...
// DS 4
#lt-ds(num: 16, de: [Đề cd03H — Mã 528], back-to: "sec-exercise-hub",[Một xưởng quang học sản xuất thấu kính máy ảnh cao cấp. Mỗi chiếc thấu kính sau khi chế tác có thể gặp hai loại lỗi độc lập nhau:
...
// TLN 1
#lt-tln(num: 17, de: [Đề cd03H — Mã 528], back-to: "sec-exercise-hub",[Một phòng vé rạp chiếu phim thống kê số vé đặt trước trong khung giờ $15$ phút là biến ngẫu nhiên $X$ có bảng phân bố xác suất:
...
// TLN 2
#lt-tln(num: 18, de: [Đề cd03H — Mã 528], back-to: "sec-exercise-hub",[Một dây chuyền dập nắp lon bia có tỷ lệ nắp bị lỗi mép là $p = 0","008$. Người ta kiểm tra ngẫu nhiên một mẫu gồm $n = 500$ nắp lon độc lập. Tính phương sai $V(X)$ của số nắp lon bị lỗi trong mẫu.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TLN 3
#lt-tln(num: 19, de: [Đề cd03H — Mã 528], back-to: "sec-exercise-hub",[Một tổng đài viễn thông ghi nhận số lần nghẽn mạng trong một tuần là biến ngẫu nhiên rời rạc $X$ có $E(X) = 6$ và $V(X) = 4$. Xét biến ngẫu nhiên chi phí xử lý sự cố $Y = 3 X^2 - 5 X + 10$ (đơn vị: triệu đồng). Tính kỳ vọng $E(Y)$ của chi phí xử lý sự cố trong tuần (đơn vị: triệu đồng).
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TLN 4
#lt-tln(num: 20, de: [Đề cd03H — Mã 528], back-to: "sec-exercise-hub",[Một nhà máy sản xuất bóng đèn led kiểm tra chất lượng một mẫu gồm $n = 200$ bóng đèn. Xác suất để mỗi bóng đèn bị hỏng không sáng là $p = 0","04$ (các bóng đèn độc lập nhau). Tính độ lệch chuẩn $sigma(X)$ của số bóng đèn bị hỏng trong mẫu (làm tròn kết quả đến hàng phần mười).
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TLN 6
#lt-tln(num: 22, de: [Đề cd03H — Mã 528], back-to: "sec-exercise-hub",[Một lô hàng gồm $N = 100$ sản phẩm trong đó có $M = 10$ phế phẩm. Người ta lấy ngẫu nhiên không hoàn lại $n = 5$ sản phẩm để kiểm định chất lượng. Xác suất để trong $5$ sản phẩm lấy ra không có phế phẩm nào bằng bao nhiêu? (Làm tròn kết quả đến hàng phần trăm, ví dụ $0","58$).
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
#slide(title: none)[
  #align(center + horizon)[
    #block(
      ...
      #text(weight: "bold", fill: rgb("#a5b4fc"), size: 22pt)[🎉 CHÚC MỪNG HOÀN THÀNH ĐỀ ÔN TẬP CHUYÊN ĐỀ 3H: MÔ HÌNH HÀNG ĐỢI RỜI RẠC, TIẾP NHẬN DỊCH VỤ & KIỂM SOÁT CHẤT LƯỢNG!]      #v(0.6em)
