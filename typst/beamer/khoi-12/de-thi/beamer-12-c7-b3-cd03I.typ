// ═══════════════════════════════════════════════════════════════════════════
// BEAMER ĐỀ THI: ĐỀ ÔN TẬP CHUYÊN ĐỀ 3I: TỔNG HỢP VD - VDC: MÔ HÌNH NGẪU NHIÊN TRONG Y HỌC, KỸ THUẬT & KHOA HỌC DỮ LIỆU
// Lớp: Khối 12  ·  Mã đề: 529  ·  GV: Nguyễn Văn Sang
...
#show: lecture-theme.with(
  title: "ĐỀ ÔN TẬP CHUYÊN ĐỀ 3I: TỔNG HỢP VD - VDC: MÔ HÌNH NGẪU NHIÊN TRONG Y HỌC, KỸ THUẬT & KHOA HỌC DỮ LIỆU",
  subtitle: "CHUYÊN ĐỀ 3: BIẾN NGẪU NHIÊN RỜI RẠC & CÁC SỐ ĐẶC TRƯNG — MÃ ĐỀ: 529",
...
#slide(title: none)[
  ...
  #context {
    ...
    grid(
      ...
      pad(left: 8pt)[#text(size: 9pt, fill: s.muted, weight: "bold")[MÃ ĐỀ: 529]],
...
// TN 3
#lt-tn(num: 3, de: [Đề cd03I — Mã 529], back-to: "sec-exercise-hub",[Một máy bay vận tải cỡ lớn có $4$ động cơ phản lực hoạt động độc lập nhau. Chuyến bay được đảm bảo an toàn nếu có ít nhất $2$ trong số $4$ động cơ hoạt động bình thường. Biết rằng xác suất xảy ra sự cố hỏng hóc trong một chuyến bay của mỗi động cơ là $q = 0","01$ (xác suất hoạt động tốt là $p = 0","99$). Xác suất để chuyến bay an toàn bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    // Máy bay 4 động cơ
...
// TN 5
#lt-tn(num: 5, de: [Đề cd03I — Mã 529], back-to: "sec-exercise-hub",[Trong phương pháp xét nghiệm gộp của Dorfman: Nếu mẫu gộp $k = 5$ người có kết quả âm tính, chỉ cần $1$ xét nghiệm cho cả nhóm. Nếu mẫu gộp dương tính, người ta phải xét nghiệm đơn lẻ lại cho từng người trong nhóm (tổng cộng tốn $1 + 5 = 6$ xét nghiệm cho nhóm đó). Với tỷ lệ nhiễm $p = 0","02$, kỳ vọng số lượt xét nghiệm cần thực hiện cho một nhóm $5$ người bằng (làm tròn đến hàng phần trăm):
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 7
#lt-tn(num: 7, de: [Đề cd03I — Mã 529], back-to: "sec-exercise-hub",[Theo Luật số lớn (Luật yếu), xét dãy biến ngẫu nhiên độc lập cùng phân bố có phương sai $sigma^2 = 36$. Biến ngẫu nhiên trung bình mẫu là $bar(X)_n = frac(1, n) sum_(i=1)^n X_i$. Cỡ mẫu $n$ tối thiểu bằng bao nhiêu để độ lệch chuẩn của trung bình mẫu $sigma(bar(X)_n)$ không vượt quá $0","3$?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 9
#lt-tn(num: 9, de: [Đề cd03I — Mã 529], back-to: "sec-exercise-hub",[Để tiêu diệt một mục tiêu bay của đối phương, một tổ hợp tên lửa phòng không phóng độc lập các quả tên lửa. Xác suất tiêu diệt mục tiêu của mỗi quả tên lửa là $p = 0","75$. Cần phóng tối thiểu bao nhiêu quả tên lửa để xác suất tiêu diệt được mục tiêu đạt từ $99%$ trở lên?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 12
#lt-tn(num: 12, de: [Đề cd03I — Mã 529], back-to: "sec-exercise-hub",[Theo Bất đẳng thức Chebyshev, với một biến ngẫu nhiên $X$ bất kỳ có kỳ vọng $mu$ và độ lệch chuẩn $sigma$, xác suất để giá trị của $X$ lệch khỏi kỳ vọng từ $2","5 sigma$ trở lên thỏa mãn bất đẳng thức:
...
// DS 2
#lt-ds(num: 14, de: [Đề cd03I — Mã 529], back-to: "sec-exercise-hub",[Một hệ thống cụm máy chủ trung tâm dữ liệu được thiết kế gồm hai trung tâm dự phòng độc lập: Trung tâm A và Trung tâm B.
...
// DS 3
#lt-ds(num: 15, de: [Đề cd03I — Mã 529], back-to: "sec-exercise-hub",[Một hãng hàng không thương mại quản lý đội bay khai thác $n = 1000$ chuyến bay trong một tháng. Thống kê lịch sử cho thấy xác suất một chuyến bay cần thay thế lốp máy bay khẩn cấp sau khi hạ cánh là $p = 0","005$ (các chuyến bay độc lập nhau). Gọi $X$ là số lốp máy bay cần thay thế khẩn cấp trong tháng.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// DS 4
#lt-ds(num: 16, de: [Đề cd03I — Mã 529], back-to: "sec-exercise-hub",[Một thuật toán ngẫu nhiên kiểm tra tính nguyên tố kiểm tra một hợp số lẻ lớn $n$. Trong mỗi lượt kiểm tra độc lập, thuật toán có xác suất kết luận nhầm "$n$ là số nguyên tố" tối đa là $p = 0","25$. Thuật toán được lặp lại $k$ lượt độc lập, và thuật toán chỉ kết luận nhầm nếu *tất cả* $k$ lượt thử đều kết luận nhầm.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TLN 1
#lt-tln(num: 17, de: [Đề cd03I — Mã 529], back-to: "sec-exercise-hub",[Một vệ tinh nhân tạo sử dụng $4$ tấm pin quang điện độc lập để cấp điện. Xác suất để mỗi tấm pin hoạt động bình thường sau $5$ năm trong không gian là $p = 0","95$. Vệ tinh duy trì được chức năng nếu có ít nhất $3$ tấm pin hoạt động tốt. Tính xác suất để vệ tinh duy trì hoạt động tốt sau $5$ năm (làm tròn kết quả đến hàng phần nghìn).
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    // 4 tấm pin vệ tinh
...
// TLN 2
#lt-tln(num: 18, de: [Đề cd03I — Mã 529], back-to: "sec-exercise-hub",[Trong xét nghiệm tầm soát y tế gộp nhóm gồm $k = 8$ cá nhân độc lập với tỷ lệ nhiễm bệnh là $p = 0","015$. Tính xác suất để một mẫu gộp của nhóm đó có kết quả âm tính (làm tròn kết quả đến hàng phần trăm, ví dụ $0","89$).
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    // Mẫu gộp 8 người
...
// TLN 3
#lt-tln(num: 19, de: [Đề cd03I — Mã 529], back-to: "sec-exercise-hub",[Một hệ thống ngân hàng số sử dụng mô hình học máy (Machine Learning) để phát hiện giao dịch gian lận. Tỷ lệ cảnh báo nhầm đối với các giao dịch hợp lệ là $p = 0","02$. Trong một ca làm việc có $n = 1500$ giao dịch hợp lệ độc lập được xử lý. Tính phương sai $V(X)$ của số giao dịch bị cảnh báo nhầm trong ca làm việc đó.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TLN 4
#lt-tln(num: 20, de: [Đề cd03I — Mã 529], back-to: "sec-exercise-hub",[Một hệ thống phòng thủ ven biển bố trí các bệ phóng tên lửa tự động bắn độc lập vào một tàu đổ bộ xâm nhập. Xác suất tiêu diệt mục tiêu của mỗi quả tên lửa là $p = 0","60$. Cần bắn tối thiểu bao nhiêu quả tên lửa để xác suất tiêu diệt mục tiêu đạt ít nhất $99%$?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TLN 5
#lt-tln(num: 21, de: [Đề cd03I — Mã 529], back-to: "sec-exercise-hub",[Cho biến ngẫu nhiên $X$ có kỳ vọng $E(X) = 50$ và phương sai $V(X) = 25$. Theo Bất đẳng thức Chebyshev, cận trên của xác suất để $X$ lệch khỏi kỳ vọng từ $15$ đơn vị trở lên: $P(|X - 50| >= 15) <= frac(1, k^2)$. Giá trị của cận trên $frac(1, k^2)$ đó bằng bao nhiêu? (Làm tròn kết quả đến hàng phần trăm, ví dụ $0","11$).
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    // Sơ đồ Chebyshev k = 3
...
#slide(title: none)[
  #align(center + horizon)[
    #block(
      ...
      #text(weight: "bold", fill: rgb("#a5b4fc"), size: 22pt)[🎉 CHÚC MỪNG HOÀN THÀNH ĐỀ ÔN TẬP CHUYÊN ĐỀ 3I: TỔNG HỢP VD - VDC: MÔ HÌNH NGẪU NHIÊN TRONG Y HỌC, KỸ THUẬT & KHOA HỌC DỮ LIỆU!]      #v(0.6em)
