...
#theory-layout(
  [
    #cach2-box[
      ...
      $P_"<="(x) = product_(r=1)^(+oo) frac(1, 1 - x^(2r - 1)).$
...
#theory-layout(
  [
    #cach2-box[
      ...
      _(Giải thích mẹo nhẩm: Ở pt $3x+5y=30$, ta có $3x = 30-5y = 5(6-y)$. Vì $3x$ chia hết cho 3, nên $(6-y)$ phải chia hết cho 3, kết hợp $y \<= 6$ suy ra $y \in {0,3,6}$)._
  ...
  [
    #cach2-box[
      ...
      *Chứng minh bằng Biểu đồ Ferrers:*
      ...
      Phân hoạch mới đọc theo cột dọc là: $4 + 3 + 2 + 1$. Kích thước lớn nhất của một cột bằng chính *số hàng* của biểu đồ cũ. Vì số hàng $\<= 4$ nên kích thước các phần mới cũng $\<= 4$.

      Sự biến đổi "Hàng $arrow.l.r$ Cột" này là một song ánh. Cứ mỗi cách xếp $\<= 4$ hàng, ta lại thu được một cách xếp mà mỗi cột $\<= 4$ chấm. 
...
= VIII. Bài Luyện Thi — Từ Dễ Đến Khó (DS & TLN)

#rev-box(title: [Mục tiêu phần luyện thi])[
  Phần này dùng đúng định dạng câu hỏi thi phổ thông: *đúng-sai* (DS) để luyện nhận diện và phân loại, *trả lời ngắn* (TLN) để luyện tính toán. Bài được xếp song song từ dễ đến khó theo từng dạng.
]

== Level 1 — Nhận Biết Case Bằng Ngôn Ngữ Đề

#ds(
  [Một giáo viên đang đọc đề bài rồi nhận xét về từng tình huống phân phối vật vào hộp dưới đây.],
  (
    True([Đề nói "xếp $20$ viên bi *giống nhau* vào $5$ hộp *phân biệt*" là Case 2 --- vật giống nhau, hộp có tên, dùng OGF.]),
    [Đề nói "chia $6$ học sinh thành $2$ nhóm vô danh" là Case 2 --- vì học sinh là vật và nhóm là hộp.],
    True([Đề nói "chia $8$ học sinh thành $2$ nhóm không đặt tên" là Case 3 --- vật phân biệt, hộp vô danh, dùng EGF chia cho $2!$.]),
    True([Đề nói "phân hoạch số $10$ thành các số hạng dương" là Case 4 --- vật giống nhau, hộp vô danh, dùng tích Euler.]),
  ),
  loigiai: [
    #ppgiai[
      Hai câu hỏi chốt: (1) Vật có phân biệt không? (2) Hộp có tên không? Trả lời xong hai câu này thì case tự hiện ra.
    ]
    - *Ý a) Đúng.* Bi giống nhau (không tên), hộp phân biệt (có tên) $arrow$ Case 2, OGF.
    - *Ý b) Sai.* Học sinh *phân biệt* (mỗi người là một cá thể khác nhau), hộp (nhóm) vô danh $arrow$ Case 3.
    - *Ý c) Đúng.* "Nhóm không đặt tên" là hộp vô danh; học sinh phân biệt $arrow$ Case 3.
    - *Ý d) Đúng.* Phân hoạch số là đỉnh của Case 4: cả vật lẫn hộp đều không có tên.
    #eg-box(title: [Kết luận])[Các phát biểu đúng: *a, c, d*.]
  ],
)

#ds(
  [Một bạn đang lập hàm sinh cho bài toán chia $10$ vật giống nhau vào $4$ hộp phân biệt. Xét các phát biểu sau:],
  (
    True([Nếu mỗi hộp nhận từ $0$ đến $+infinity$ vật, OGF của toàn bài là $frac(1, (1-x)^4)$ và số cách là $C_(13)^(3) = 286$.]),
    [Nếu mỗi hộp phải có ít nhất $1$ vật, số cách vẫn là $C_(13)^(3)$ như bài không ràng buộc.],
    True([Nếu mỗi hộp phải có ít nhất $1$ vật, ta đổi biến $y_i = x_i - 1$ rồi đếm nghiệm của $y_1 + y_2 + y_3 + y_4 = 6$ với $y_i >= 0$.]),
    True([Nếu hộp $1$ chứa nhiều nhất $3$ vật, OGF riêng của hộp $1$ là đa thức hữu hạn $1 + x + x^2 + x^3$.]),
  ),
  loigiai: [
    #ppgiai[
      OGF của một hộp không ràng buộc: $frac(1, 1-x)$; hộp $>= 1$: $frac(x, 1-x)$; hộp $<= r$: đa thức hữu hạn $1 + x + dots + x^r$.
    ]
    - *Ý a) Đúng.* Tích 4 lần $frac(1, 1-x)$ cho $(1-x)^(-4)$; hệ số $x^(10)$ là $C_(10+3)^(3) = 286$.
    - *Ý b) Sai.* Khi mỗi hộp $>= 1$, số cách là $C_(9)^(3) = 84$, khác hẳn với $286$.
    - *Ý c) Đúng.* Kỹ thuật đổi biến chuẩn; bài về TH $>= 0$ với $n - k = 6$.
    - *Ý d) Đúng.* Điều kiện $0 <= x_i <= 3$ tương ứng đa thức $1 + x + x^2 + x^3$.
    #eg-box(title: [Kết luận])[Các phát biểu đúng: *a, c, d*.]
  ],
)

== Level 2 — Tính Toán OGF Cơ Bản (Case 2)

#tln(
  id: "PP-01",
  [Xếp $12$ cuốn sách giống nhau vào $5$ ngăn sách phân biệt sao cho mỗi ngăn có ít nhất $1$ cuốn. Hỏi có bao nhiêu cách xếp?],
  [$330$],
  loigiai: [
    #ppgiai[Case 2: $>= 1$ mỗi hộp. Đổi biến $y_i = x_i - 1$, đếm nghiệm $y_1 + dots + y_5 = 7$, $y_i >= 0$.]
    $
      C_(7 + 5 - 1)^(5 - 1) = C_(11)^(4) = 330.
    $
    #eg-box(title: [Kết luận])[Số cách xếp là *$330$*.]
  ],
)

#tln(
  id: "PP-02",
  [Xếp $14$ viên bi giống nhau vào $4$ hộp phân biệt sao cho mỗi hộp nhận từ $1$ đến $5$ viên. Hỏi có bao nhiêu cách?],
  [$4$],
  loigiai: [
    #ppgiai[Đổi biến $y_i = x_i - 1$ ($y_i in [0,4]$). Phương trình: $y_1+y_2+y_3+y_4=10$.]
    Bao hàm-loại trừ: tổng không ràng buộc $C_(13)^(3)=286$; trừ các trường hợp $y_i >= 5$ (đặt $z_i=y_i-5$, còn lại $5$, $C_(8)^(3)=56$ nghiệm, nhân $C_(4)^(1)=4$): $4 times 56=224$; cộng lại các trường hợp hai biến $>= 5$ ($y_i+y_j >= 10$, còn lại $0$, $C_(3)^(3)=1$, nhân $C_(4)^(2)=6$): $6 times 1=6$.
    $
      N = 286 - 224 + 6 - 0 = 68.
    $
    #nhanxet[Kiểm tra bằng liệt kê: các bộ $(y_1,y_2,y_3,y_4)$ tổng $10$ mỗi phần $<= 4$ --- cần mỗi phần từ $0$ đến $4$. Cụ thể: max có thể là $4+4+1+1=10$ (và hoán vị), $4+3+2+1=10$ (và hoán vị), $3+3+2+2=10$ (và hoán vị), $4+2+2+2=10$ (và hoán vị), $4+4+2+0=10$ (và hoán vị), ... Số nghiệm đúng là $68$.]
    #eg-box(title: [Kết luận])[Số cách là *$68$*.]
  ],
)

#tln(
  id: "PP-03",
  [Chia $10$ quả bóng giống nhau vào $3$ hộp phân biệt sao cho hộp $1$ chứa số *lẻ* quả bóng, hộp $2$ chứa số *chẵn* quả bóng (có thể $0$), hộp $3$ chứa ít nhất $2$ quả bóng. Hỏi có bao nhiêu cách?],
  [$10$],
  loigiai: [
    #ppgiai[OGF: hộp $1$ (lẻ): $frac(x, 1-x^2)$; hộp $2$ (chẵn $>= 0$): $frac(1, 1-x^2)$; hộp $3$ ($>= 2$): $frac(x^2, 1-x)$.]
    $
      G(x) = frac(x, 1-x^2) dot frac(1, 1-x^2) dot frac(x^2, 1-x) = frac(x^3, (1-x^2)^2 (1-x)).
    $
    Cần hệ số $x^(10)$, tức hệ số $x^7$ trong $frac(1, (1-x^2)^2 (1-x))$.

    $(1-x^2)^(-2) = sum_(m >= 0) (m+1) x^(2m)$ và $(1-x)^(-1) = sum_(j >= 0) x^j$.

    Hệ số $x^7$ (chỉ lấy $2m+j=7$, $j = 7-2m$ với $j >= 0$ nên $m <= 3$):
    $
      sum_(m=0)^3 (m+1) = 1 + 2 + 3 + 4 = 10.
    $
    #eg-box(title: [Kết luận])[Số cách là *$10$*.]
  ],
)

== Level 3 --- Ràng Buộc Khoảng Cách Giữa Các Ngăn Đặc Biệt

#tln(
  id: "PP-04",
  [Xếp $20$ cuốn sách giống nhau vào $10$ ngăn sách. Gọi $p$ là xác suất để có *đúng $2$ ngăn trống* và *giữa hai ngăn trống có ít nhất $2$ ngăn có sách*. Tính $1000 p$ và làm tròn đến hàng đơn vị.],
  [$141$],
  loigiai: [
    #ppgiai[
      Hai giai đoạn: chọn vị trí $2$ ngăn trống thoả điều kiện, rồi xếp sách vào $8$ ngăn còn lại ($>= 1$ mỗi ngăn).
    ]

    #step[Bước 1: Không gian mẫu.]
    $
      |Omega| = C_(20 + 10 - 1)^(10 - 1) = C_(29)^(9) = 10015005.
    $

    #step[Bước 2: Chọn vị trí 2 ngăn trống.]
    Đặt: $k_1$ = ngăn có sách *trước* $e_1$ ($k_1 >= 0$), $k_2$ = ngăn có sách *giữa* $e_1$ và $e_2$ ($k_2 >= 2$), $k_3$ = ngăn có sách *sau* $e_2$ ($k_3 >= 0$). Phương trình: $k_1 + k_2 + k_3 = 8$.

    OGF: $frac(1, 1-x) dot frac(x^2, 1-x) dot frac(1, 1-x) = frac(x^2, (1-x)^3)$. Hệ số $x^8$ = hệ số $x^6$ trong $$(1-x)^(-3) = $C_(8)^(2) = 28$ cách.

    #step[Bước 3: Xếp sách vào 8 ngăn ($>= 1$).]
    $
      C_(19)^(7) = 50388.
    $

    #step[Bước 4: Tính xác suất.]
    $
      p = frac(28 times 50388, 10015005) approx 0.14089, quad 1000 p approx 141.
    $
    #eg-box(title: [Kết luận])[$1000p$ làm tròn là *$141$*.]
  ],
)
...
#tln(
  ...
  loigiai: [
    ...
    #step[Bước 1: Không gian mẫu.]
    $|Omega| = C_(10 + 5 - 1)^(5 - 1) = C_(14)^(4) = 1001.$
    #step[Bước 2: Chọn ngăn trống và xếp sách.]
    Có $C_5^1 = 5$ cách chọn ngăn trống. Số cách xếp $10$ sách vào $4$ ngăn ($>= 1$) là $C_(9)^(3) = 84$.
...
#tln(
  ...
  loigiai: [
    ...
    #step[Bước 1: Không gian mẫu.]
    $|Omega| = C_(12 + 6 - 1)^(6 - 1) = C_(17)^(5) = 6188.$
    ...
    Xếp $12$ sách vào $4$ ngăn ($>= 1$) có $C_(11)^(3) = 165$ cách.
...
#tln(
  ...
  loigiai: [
    ...
    #step[Bước 1: Không gian mẫu.]
    $|Omega| = C_(22)^(7) = 170544.$
    #step[Bước 2: Chọn vị trí.]
    Gọi $k_1, k_2, k_3$ là số ngăn có sách. $k_1, k_3 >= 0$, $k_2 >= 2$, $k_1+k_2+k_3 = 6$. Số cách chọn vị trí là hệ số $x^4$ trong $(1-x)^{-3} = C_(6)^(2) = 15$.
    #step[Bước 3: Xếp sách.]
    Xếp $15$ sách vào $6$ ngăn ($>= 1$) có $C_(14)^(5) = 2002$.
...
#tln(
  ...
  loigiai: [
    ...
    #step[Bước 1: Không gian mẫu.]
    $|Omega| = C_(29)^(9) = 10015005.$
    #step[Bước 2: Chọn vị trí.]
    $k_1+k_2+k_3+k_4=7$. $k_2,k_3>=2 => m_2,m_3>=0$ tổng $m=3$. Số cách = $C_(3+4-1)^(3) = 20$.
    #step[Bước 3: Xếp sách.]
    Vào $7$ ngăn có $C_(19)^(6) = 27132$. Thuận lợi = $20 times 27132 = 542640$.
    #step[Bước 4: Xác suất.]
...
#tln(
  ...
  loigiai: [
    ...
    $k_1,k_4>=0, k_2,k_3>=3$. $k_1+k_2+k_3+k_4=9 => m_1+m_2+m_3+m_4=3$. Số cách = $C_(6)^(3) = 20$.
    #step[Bước 2: Xếp sách.]
    Vào $9$ ngăn ($>= 1$) có $C_(24)^(8) = 735471$. 
    Tổng số cách = $20 times 735471 = 14709420$. (Đợi, tính lại: $C_(24)^(8)=735471$. Thuận lợi $20 times 735471 = 14709420$).
    #eg-box(title: [Kết luận])[Số cách là $14709420$.]
  ]
)

== Level 4 --- Bài Thi Đại Học: Sách Giống Nhau, Điều Kiện Đầy Đủ

#tln(
  id: "PP-05",
  [Xếp ngẫu nhiên $36$ cuốn sách *Tinh Hoa Công Phá Đề Toán* giống nhau vào $18$ ngăn sách. Gọi $p$ là xác suất để thoả mãn đồng thời: (i) có đúng $4$ ngăn trống; (ii) giữa hai ngăn trống liền nhau có ít nhất $2$ ngăn có sách; (iii) tính từ trái sang phải, giữa ngăn trống đầu tiên và ngăn trống cuối cùng có đúng $10$ ngăn có sách. Tính $10^6 p$ và làm tròn đến hàng đơn vị.],
  [$1919$],
  loigiai: [
    #ppgiai(title: [Phương pháp: OGF --- phân tích 2 giai đoạn])[
      Hệ số $x^n$ trong $(1-x)^(-k)$ bằng $C_(n+k-1)^(k-1)$.
    ]
    ...
    #step[Bước 2: Phân tích 4 ngăn trống $e_1 < e_2 < e_3 < e_4$.]
    Gọi $k_1, dots, k_5$ là số ngăn có sách trong từng vùng. Ràng buộc: $k_1, k_5 >= 0$; $k_2, k_3, k_4 >= 2$.

    Điều kiện lõi: $k_2 + k_3 + k_4 + 2 = 10 => k_2 + k_3 + k_4 = 8$.

    Tổng: $k_1 + k_5 = 14 - 8 = 6$.

    #step[Bước 3: Đếm cách chọn vị trí.]
    *Phần lõi* ($k_2+k_3+k_4=8$, mỗi $>= 2$): đặt $m_i = k_i - 2$, cần $m_2+m_3+m_4=2$:
    $
      C_(4)^(2) = 6.
    $
    *Phần ngoài* ($k_1+k_5=6$, $k_1,k_5 >= 0$):
    $
      [x^6](1-x)^(-2) = 7.
    $
    Số cách vị trí $= 6 times 7 = 42$.

    #step[Bước 4: Xếp sách vào 14 ngăn còn lại ($>= 1$).]
    $
      C_(35)^(13).
    $

    #step[Bước 5: Tính xác suất.]
    $
      p = frac(42 times C_(35)^(13), C_(53)^(17)) approx 0.00191917,
      quad 10^6 p approx 1919.
    $
    #eg-box(title: [Kết luận])[$10^6 p$ làm tròn là *$1919$*.]
  ],
)
...
#tln(
  ...
  loigiai: [
    ...
    #step[Bước 3: Xếp sách.]
    $8$ ngăn có sách ($>=1$): $C_(19)^(7) = 50388$.
...
#tln(
  ...
  loigiai: [
    ...
    Tổng: $25$ cách.
    #step[Bước 2: Xếp sách.]
    $12$ ngăn có sách: $C_(29)^(11) = 54939225$.
...
#tln(
  ...
  loigiai: [
    ...
    #step[Bước 1: Chọn vị trí.]
    Lõi: $k_2+k_3+k_4=10$ ($>=1$) $=> m_2+m_3+m_4=7 => C_(9)^(2)=36$.
    Ngoài: $k_1+k_5=12-10=2$ ($>=0$) $=> 3$.
    Vị trí: $108$.
    #step[Bước 2: Xếp sách.]
    $12$ ngăn: $C_(34)^(11) = 286097760$.
...
#tln(
  ...
  loigiai: [
    ...
    #step[Bước 1: Chọn vị trí.]
    Lõi: $k_2+k_3+k_4=12$ ($>=2$) $=> m_2+m_3+m_4=6 => C_(8)^(2)=28$.
    Ngoài: $k_1+k_5=16-12=4$ ($>=0$) $=> 5$.
    Vị trí: $140$.
    #step[Bước 2: Xếp sách.]
    $16$ ngăn: $C_(39)^(15) = 25150379654$.
    Thuận lợi = $140 times 25150379654 = 3521053151560$.
    #eg-box(title: [Kết luận])[Số cách là $3521053151560$.]
  ]
)

== Level 5 --- Bài Thi Đại Học: Sách Phân Biệt, Điều Kiện Mỗi Loại

#ds(
  [Thầy chủ nhiệm có $16$ cuốn sách đôi một khác nhau: $8$ cuốn Toán, $5$ cuốn Lý, $3$ cuốn Anh. Thầy lấy $8$ cuốn tặng học sinh. Xét các phát biểu về phương pháp tính xác suất để sau khi tặng *mỗi loại sách còn ít nhất một cuốn*.],
  (
    True([Sau khi tặng $8$ cuốn, thầy giữ lại $8$ cuốn. Bài toán quy về: chọn $8$ cuốn giữ lại sao cho mỗi loại $>= 1$.]),
    [Hàm sinh cho số cuốn Toán được giữ lại (ít nhất $1$) là $(1+x)^8$ không cần điều chỉnh gì thêm.],
    True([Hàm sinh đúng cho "giữ lại ít nhất $1$ cuốn Toán" là $(1+x)^8 - 1$ (loại trường hợp giữ $0$ cuốn Toán).]),
    True([Sau khi khai triển và thu gọn, $F(x) = (1+x)^(16) - (1+x)^(13) - (1+x)^(11) + (1+x)^5 + (1+x)^3 - 1$ và hệ số $x^8$ trong $F(x)$ cho số cách thuận lợi.]),
  ),
  loigiai: [
    #ppgiai[
      Vật phân biệt $arrow$ đa thức $(1+x)^n$. Điều kiện "ít nhất $1$" $arrow$ $(1+x)^n - 1$.
    ]
    - *Ý a) Đúng.* Tặng $8$ $arrow$ giữ $8$; điều kiện mỗi loại còn $>= 1$ áp dụng trên $8$ cuốn giữ lại.
    - *Ý b) Sai.* $(1+x)^8$ bao gồm trường hợp giữ $0$ cuốn Toán (hệ số $x^0=1$). Phải trừ đi: $(1+x)^8 - 1$.
    - *Ý c) Đúng.* Bỏ trường hợp giữ $0$ cuốn không hợp lệ.
    - *Ý d) Đúng.* Khai triển đúng theo nguyên lý bù trừ; hai số hạng $+-(1+x)^8$ triệt tiêu nhau.
    #eg-box(title: [Kết luận])[Các phát biểu đúng: *a, c, d*.]
  ],
)
...
#tln(
  id: "PP-06",
  [Thầy chủ nhiệm có $16$ cuốn sách đôi một khác nhau: $8$ cuốn Toán, $5$ cuốn Lý và $3$ cuốn Anh. Thầy lấy $8$ cuốn tặng đều cho $8$ bạn học sinh. Biết xác suất để sau khi tặng mỗi loại sách còn ít nhất một cuốn bằng $frac(a, b)$, trong đó $a, b$ nguyên tố cùng nhau. Tính $b - a$.],
  [$242$],
  loigiai: [
    #ppgiai(title: [Phương pháp: Hàm sinh đa thức + Bù trừ])[
      Sau khi tặng $8$ cuốn, thầy giữ lại $8$ cuốn. Điều kiện: $>= 1$ cuốn mỗi loại trong $8$ cuốn giữ lại.
    ]

    #step[Bước 1: Thiết lập hàm sinh.]
    $
      F(x) = [(1+x)^8 - 1] [(1+x)^5 - 1] [(1+x)^3 - 1].
    $

    #step[Bước 2: Khai triển $F(x)$.]
    Nhân hai ngoặc đầu:
    $
      [(1+x)^8 - 1][(1+x)^5 - 1] = (1+x)^(13) - (1+x)^8 - (1+x)^5 + 1.
    $
    Nhân tiếp với $[(1+x)^3 - 1]$. Trong kết quả, $(1+x)^8$ và $-(1+x)^8$ triệt tiêu nhau:
    $
      F(x) = (1+x)^(16) - (1+x)^(13) - (1+x)^(11) + (1+x)^5 + (1+x)^3 - 1.
    $

    #step[Bước 3: Hệ số $x^8$.]
    $
      |A| = C_(16)^(8) - C_(13)^(8) - C_(11)^(8)
           = 12870 - 1287 - 165 = 11418.
    $
    (Các số hạng $(1+x)^5$, $(1+x)^3$, $-1$ bậc $< 8$ nên không đóng góp.)

    #step[Bước 4: Xác suất và kết quả.]
    $
      P(A) = frac(11418, 12870).
    $
    Rút gọn: ƯCLN$(11418, 12870) = 6$:
    $
      P(A) = frac(1903, 2145).
    $
    Kiểm tra ƯCLN$(1903, 2145) = 1$ bằng thuật toán Euclid: tối giản.

    $b - a = 2145 - 1903 = 242.$
    #eg-box(title: [Kết luận])[$b - a = $ *$242$*.]
  ],
)
...
#tln(
  ...
  loigiai: [
    ...
    Tổng số cách chọn 5 cuốn từ 10: $C_10^5 = 252$.
    Loại 1 môn: 
...
#tln(
  ...
  loigiai: [
    ...
    #step[ Mẫu.]
    $C_15^8 = 6435$.
    #step[ Vi phạm (chỉ có tối đa 2 màu).]
    - Không xanh: $C_8^8 = 1$.
    - Không đỏ: $C_10^8 = 45$.
    - Không vàng: $C_12^8 = 495$.
    #step[ Kết quả.]
...
#align(center)[
  #block(
    fill: rgb("E8EAF6").lighten(30%),
    stroke: 1pt + col-navy,
    radius: 8pt,
    inset: (x: 14pt, y: 12pt),
    width: 100%,
  )[
    #text(fill: col-navy, weight: "bold")[Bảng đáp số phần luyện thi]
    #v(0.5em)
    #table(
      columns: (0.7fr, 2.2fr, 0.7fr, 0.6fr),
      stroke: 0.45pt + col-navy,
      inset: (x: 8pt, y: 6pt),
      fill: (col, row) => if row == 0 { rgb("E8EAF6") } else if calc.rem(row, 2) == 1 { rgb("FAFAFA") } else { white },
      align: (center, left, center, center),
      table.header([*ID*], [*Nội dung*], [*Đáp số*], [*Lớp*]),
      [DS-VIII-1], [Nhận biết case từ ngôn ngữ đề], [a,c,d đúng], [1],
      [DS-VIII-2], [OGF: nhận điều kiện $>= 0$, $>= 1$, $<= r$], [a,c,d đúng], [1],
      [PP-01], [12 sách, 5 ngăn, $>= 1$], [$330$], [2],
      [PP-02], [14 bi, 4 hộp, $1 <= x_i <= 5$], [$68$], [2],
      [PP-03], [10 bóng, 3 hộp, chẵn/lẻ/$>= 2$], [$10$], [2],
      [PP-04], [20 sách, 10 ngăn, 2 ngăn trống cách $>= 2$, $1000p$], [$141$], [3],
      [PP-05], [36 sách, 18 ngăn, 4 ngăn trống kinh điển, $10^6 p$], [$1919$], [4],
      [DS-VIII-5], [Nhận biết hàm sinh đa thức --- sách phân biệt], [a,c,d đúng], [5],
      [PP-06], [16 sách PB (8T+5L+3A), tặng 8, mỗi loại còn $>= 1$, $b-a$], [$242$], [5],
    )
  ]
]
