...
#tln(
  [Để thi công một tuyến cáp treo đi qua khe núi, các kỹ sư mô hình hóa khe núi bằng một khối chóp tứ giác $S.A B C D$ có đáy $A B C D$ là hình thang (với $A D parallel B C$ và $A D = 2 B C$). Một tuyến cáp phụ nằm trên mặt phẳng $(P)$ đi qua trung điểm $M$ của sườn núi $S A$, đi qua điểm $N$ trên sườn $S B$ sao cho $S N = 1/3 S B$ và song song với cạnh núi $A D$. Hãy tính tỉ số $x = (S K) / (S D)$ với $K$ là điểm neo cáp trên sườn $S D$.],
  [$1/2$],
  ...
  - **Bước 2: Sử dụng định lý Thales trong tam giác**
    - Xét $Delta S A D$ có $M K parallel A D$ (vì $(P) parallel A D$).
  ...
  - **Bước 3: Mở rộng tìm thiết diện hoàn chỉnh (Tham khảo thêm)**
    - Nếu đề bài yêu cầu vẽ thiết diện cắt bởi $(P)$: Do $(P) parallel A D$, mà $A D parallel B C$ (đáy là hình thang) $=> (P) parallel B C$.
...
#tln(
  ...
  - **Bước 1: Dựng thiết diện của $(alpha)$**
    ...
    - Lưỡi cưa $(alpha)$ đi qua $G$ và song song với $(A B C D)$. Do $(A B C D)$ chứa $A B$ nên $(alpha) parallel A B$.
    ...
    - Tương tự, do $(alpha) parallel (A B C D)$ nên $(alpha)$ sẽ cắt các mặt bên còn lại theo các giao tuyến song song với các cạnh đáy: $N P parallel B C$, $P Q parallel C D$, $Q M parallel D A$.
