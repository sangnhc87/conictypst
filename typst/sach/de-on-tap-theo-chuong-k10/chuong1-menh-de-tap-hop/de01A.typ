...
#show: thpt-school-exam.with(
  ...
  exam-title: "BÀI 1: MỆNH ĐỀ TOÁN HỌC (ĐỀ SỐ 1)",
  subject: "TOÁN",
...
// TN 1
#tn([Trong các câu sau, câu nào không phải là mệnh đề toán học?],
    (
        [$pi$ là một số vô tỉ.],
...
// TN 2
#tn([Phủ định của mệnh đề "Mọi số nguyên đều là số hữu tỉ" là mệnh đề nào sau đây?],
    (
...
// TN 3
#tn([Cho hai mệnh đề $P$: "Số tự nhiên $a$ chia hết cho 3" và $Q$: "Số tự nhiên $a$ chia hết cho 9". Phát biểu mệnh đề $P => Q$ và xét tính đúng sai của nó.],
    (
...
// TN 4
#tn([Cho mệnh đề chứa biến $P(x)$: "$x^2 - 3 x + 2 = 0$". Mệnh đề nào sau đây đúng?],
    (
        [$forall x in RR, P(x)$],
...
// TN 5
#tn([Xét định lý: "Nếu hai tam giác bằng nhau thì diện tích của chúng bằng nhau". Phát biểu nào sau đây là đúng?],
    (
...
// TN 6
#tn([Khẳng định nào sau đây là *sai* khi nói về mệnh đề tương đương $P <=> Q$?],
    (
...
// TN 7
#tn([Biết mệnh đề kéo theo $P => Q$ là một mệnh đề sai. Khẳng định nào sau đây là đúng?],
    (
        True([$P$ đúng, $Q$ sai.]),
        [$P$ sai, $Q$ đúng.],
        [$P$ đúng, $Q$ đúng.],
...
#tn([Trong một vương quốc có hai loại người: hiệp sĩ luôn nói thật và kẻ gian luôn nói dối. Bạn gặp hai người dân A và B. A nói: "Ít nhất một trong hai chúng tôi là kẻ gian". Khẳng định nào sau đây là đúng?],
    (
...
// TN 9
#tn([Phủ định của mệnh đề "$forall x in RR, x^2 - x + 1 > 0$" là mệnh đề nào sau đây?],
    (
...
// TN 10
#tn([Trong các mệnh đề chứa biến sau đây, mệnh đề nào là mệnh đề *đúng*?],
    (
...
// TN 11
#tn([Có bao nhiêu số nguyên $n$ thỏa mãn $-5 <= n <= 5$ để mệnh đề chứa biến $P(n)$: "$n^2 - 4 < 0$" là một mệnh đề đúng?],
    (
        [2],
        True([3]),
        [4],
...
#tn([Cho các mệnh đề $P, Q, R$. Biết mệnh đề kéo theo $P => (Q text(" hay ") R)$ là mệnh đề sai. Khẳng định nào sau đây là đúng?],
    (
        [Cả $P, Q, R$ đều sai.],
...
// DS 1
#ds([Cho mệnh đề $P$: "Mọi học sinh trong lớp 10A đều thích học môn Toán".],
  (
...
// DS 2
#ds([Xét mệnh đề chứa biến $P(x)$ và $Q(x)$ với $x in RR$. Cho định lý: "$forall x in RR, P(x) => Q(x)$".],
  (
...
// DS 3
#ds([Xét tính đúng sai của các mệnh đề sau trên tập số thực $RR$:],
  (
...
// DS 4
#ds([Một khóa số an toàn có mật mã là một số tự nhiên $n$ có một chữ số ($0 <= n <= 9$). Ba người thợ mỗi người đưa ra hai gợi ý về mật mã như sau:
...
// TLN 2
#tln([Cho mệnh đề chứa biến $P(n)$: "$n^2 - n + 11$ là số nguyên tố", với $n$ là số tự nhiên. Tìm giá trị nguyên dương nhỏ nhất của $n$ để $P(n)$ là một mệnh đề SAI.],
    [11],
    loigiai: [
...
// TLN 3
#tln([Trong các mệnh đề dưới đây, có bao nhiêu mệnh đề là mệnh đề kéo theo ĐÚNG?
(1) Nếu $x = 2$ thì $x^2 = 4$.
(2) Nếu $x^2 = 4$ thì $x = 2$.
...
// TLN 4
#tln([Tìm giá trị thực của tham số $m$ để mệnh đề " $exists x in RR, (m - 1) x + 2 = 0$ " là một mệnh đề SAI.],
    [1],
    loigiai: [
...
// TLN 5
#tln([Cho hai mệnh đề $P$ và $Q$. Biết rằng mệnh đề kéo theo $P => Q$ là mệnh đề SAI. 
...
// TLN 6
#tln([Trong một kì thi, 3 bạn An, Bình, Cường đạt 3 mức điểm khác nhau và đều là số nguyên dương nhỏ hơn 10.
