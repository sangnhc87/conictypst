...
#show: thpt-school-exam.with(
  ...
  exam-title: "BÀI 1: MỆNH ĐỀ TOÁN HỌC (ĐỀ SỐ 3)",
  subject: "TOÁN",
...
// TN 1
#tn([Trong các câu sau, câu nào là mệnh đề Toán học?],
    (
        [Hôm nay là thứ mấy?],
...
// TN 2
#tn([Phủ định của mệnh đề $P$: "Tất cả các số tự nhiên đều là số chẵn" là mệnh đề nào sau đây?],
    (
...
// TN 3
#tn([Mệnh đề nào sau đây là mệnh đề ĐÚNG?],
    (
...
// TN 4
#tn([Cho hai mệnh đề $P$: "Tứ giác ABCD là hình thoi" và $Q$: "Tứ giác ABCD có hai đường chéo vuông góc với nhau". Khẳng định nào sau đây là ĐÚNG?],
    (
        [Mệnh đề $P => Q$ là sai.],
...
// TN 5
#tn([Mệnh đề "Nếu một tam giác cân có một góc bằng $60^o$ thì tam giác đó là tam giác đều" có thể được phát biểu dưới dạng điều kiện cần, điều kiện đủ như thế nào?],
    (
...
// TN 6
#tn([Khẳng định nào sau đây là mệnh đề *sai*?],
    (
...
// TN 7
#tn([Trong hình học phẳng, phát biểu nào sau đây là mệnh đề tương đương ĐÚNG?],
    (
...
// TN 8
#tn([Cho hai mệnh đề chứa biến $P(n)$: "$n$ là số nguyên tố" và $Q(n)$: "$n$ là số lẻ", với $n$ là số tự nhiên. Có bao nhiêu giá trị của $n$ thỏa mãn mệnh đề $P(n)$ đúng nhưng $Q(n)$ sai?],
    (
        [0],
        True([1]),
        [2],
...
// TN 9
#tn([Cho mệnh đề "Mọi số tự nhiên có tận cùng bằng 0 thì đều chia hết cho 5". Mệnh đề đảo của mệnh đề này là gì?],
    (
...
// TN 10
#tn([Trong các mệnh đề dưới đây, mệnh đề nào SAI?],
    (
...
// TN 11
#tn([Mệnh đề $exists x in ZZ, 2x^2 - 3x + 1 = 0$ là mệnh đề ĐÚNG hay SAI?],
    (
...
#tn([Ba bạn A, B, C tham gia một trò chơi vòng tròn. 
...
// DS 1
#ds([Xét các mệnh đề về tính chất hình học của tứ giác:],
  (
...
// DS 2
#ds([Cho các mệnh đề số học trên tập số tự nhiên $NN$:],
  (
...
// DS 3
#ds([Cho ba điểm $A, B, C$ phân biệt trên mặt phẳng. Xét các mệnh đề sau:],
  (
...
// DS 4
#ds([Trong vương quốc có hai loại người: Hiệp sĩ (luôn nói thật) và Kẻ gian (luôn nói dối). Bạn gặp hai người cư dân tên là X và Y. 
...
// TLN 1
#tln([Có bao nhiêu mệnh đề ĐÚNG trong các mệnh đề sau?
...
// TLN 2
#tln([Cho mệnh đề chứa biến $P(n)$: "$n^2 - 3n + 2 <= 0$". Có bao nhiêu số nguyên $n$ làm cho mệnh đề $P(n)$ đúng?],
    [2],
    loigiai: [
...
// TLN 3
#tln([Cho $P$: "Tam giác ABC vuông tại A", $Q$: "$A B^2 + B C^2 = A C^2$". 
...
// TLN 4
#tln([Biết $P$ là mệnh đề ĐÚNG, $Q$ là mệnh đề SAI, và $R$ là mệnh đề ĐÚNG.
...
// TLN 5
#tln([Tìm giá trị thực của tham số $m$ để mệnh đề "$forall x in RR, (m^2 - 1)x + m - 1 = 0$" là một mệnh đề ĐÚNG.],
    [1],
    loigiai: [
...
// TLN 6 (Tư duy)
#tln([Cho một số tự nhiên có 3 chữ số $n = overline(a b c)$. Đặt mệnh đề $P(n)$: "$a, b, c$ là ba số tự nhiên liên tiếp tăng dần", mệnh đề $Q(n)$: "$n$ chia hết cho 3".
