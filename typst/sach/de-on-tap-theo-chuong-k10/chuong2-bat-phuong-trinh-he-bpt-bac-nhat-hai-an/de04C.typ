...
#show: thpt-school-exam.with(
  ...
  exam-title: "BÀI 4: HỆ BẤT PHƯƠNG TRÌNH BẬC NHẤT HAI ẨN (ĐỀ SỐ 3)",
  subject: "TOÁN",
...
#tn([Tìm tất cả các giá trị của tham số $m$ để hệ $cases((m - 1)x + 2y <= 3, x + (m + 2)y >= 1)$ là hệ bất phương trình bậc nhất hai ẩn.],
    (
        [$m != 1$],
        [$m != -2$],
...
#tn([Điểm nào sau đây là một nghiệm của hệ bất phương trình $cases(x - 2y + 3 >= 0, 3x + y - 5 <= 0, x + y > 0)$?],
    (
        [$A(2; 1)$],
        True([$B(1; 1)$]),
        [$C(0; 6)$],
...
#tn([Miền nghiệm của hệ bất phương trình $cases(|x| <= 1, |y| <= 2)$ trên mặt phẳng tọa độ $O x y$ là một:],
    (
...
#tn([Diện tích của hình chữ nhật là miền nghiệm của hệ bất phương trình $cases(-1 <= x <= 3, -2 <= y <= 2)$ bằng:],
    (
        [$8$],
        [$12$],
        True([$16$]),
...
#tn([Hình vẽ sau đây biểu diễn miền nghiệm của hệ bất phương trình nào (phần ngũ giác không bị gạch, kể cả bờ)?
#draw_system_bpt()
],
    (
...
#tn([Cho miền tam giác $O A B$ có ba đỉnh là $O(0;0), A(4;0), B(2;4)$. Biểu thức $F(x, y) = 2x - y$ đạt giá trị LỚN NHẤT tại đỉnh nào?],
    (
        [$O(0;0)$],
        True([$A(4;0)$]),
        [$B(2;4)$],
...
#tn([Cho điểm $M(x_0; y_0)$ thuộc miền nghiệm của hệ bất phương trình $cases(x >= 0, y >= 0, x + y <= 4)$. Giá trị lớn nhất của tích $P = x_0 dot y_0$ bằng:],
    (
        [$2$],
        True([$4$]),
        [$8$],
...
#tn([Hệ bất phương trình nào sau đây có miền nghiệm là một hình vuông diện tích bằng $4$ nằm ở góc phần tư thứ nhất?],
    (
        [$cases(0 <= x <= 4, 0 <= y <= 4)$],
...
#tn([Miền nghiệm của hệ bất phương trình $cases(x - y >= 0, x + y <= 4, y >= 0)$ là một hình tam giác vuông tại đỉnh nào?],
    (
        [$O(0;0)$],
        True([$B(2;2)$]),
        [$A(4;0)$],
...
#tn([Tìm giá trị của tham số $m$ để hệ bất phương trình $cases(x + y <= 3, x >= 1, y >= m)$ có đúng MỘT nghiệm duy nhất.],
    (
        [$m = 0$],
        [$m = 1$],
        True([$m = 2$]),
...
#tn([Trọng tâm $G$ của hình tam giác tạo bởi miền nghiệm của hệ bất phương trình $cases(x >= 0, y >= 0, x/6 + y/3 <= 1)$ có tọa độ là:],
    (
        [$(3; 1.5)$],
        True([$(2; 1)$]),
        [$(1; 2)$],
...
#ds([Cho hệ bất phương trình $cases(x + y <= 5, x <= 4, y <= 3, x >= 0, y >= 0)$. Xét tính đúng sai của các mệnh đề sau:],
  (
...
#ds([Cho hình vẽ biểu diễn miền nghiệm của hệ bất phương trình $cases(x <= 3, y <= x, y >= -x)$ trên mặt phẳng tọa độ $O x y$ (phần tam giác không bị gạch, kể cả bờ):
#draw_system_bpt(
  lines: (
...
#ds([Một xưởng gốm sứ Bát Tràng sản xuất hai sản phẩm: lọ hoa cao cấp (cần $3$ kg đất sét, $2$ giờ nung lò, lãi $60$ nghìn đồng/chiếc) và bộ ấm chén (cần $2$ kg đất sét, $4$ giờ nung lò, lãi $80$ nghìn đồng/bộ). Mỗi ngày xưởng có tối đa $30$ kg đất sét và $40$ giờ nung lò. Gọi $x, y$ lần lượt là số lọ hoa và bộ ấm chén sản xuất trong ngày ($x, y in NN$).],
  (
...
// DS 4: Tham số m
#ds([Cho hệ bất phương trình $cases(0 <= x <= 3, 0 <= y <= 3, x + y >= m)$ (với $m$ là tham số). Xét tính đúng sai của các mệnh đề sau:],
  (
...
#tln([Cho hệ bất phương trình $cases(x >= 0, y >= 0, x <= 3, y <= 3, x + y <= 4)$. Tính diện tích của hình đa giác là miền nghiệm của hệ bất phương trình trên.],
    [7],
    loigiai: [
...
#tln([Tìm giá trị lớn nhất của biểu thức $F(x, y) = 5x + 4y$ trên miền nghiệm của hệ bất phương trình $cases(x >= 0, y >= 0, x <= 4, y <= 3, x + y <= 5)$.],
    [24],
    loigiai: [
        #step([Xác định 5 đỉnh của ngũ giác])
...
#tln([Một cơ sở sản xuất hai loại hạt đóng hộp: hạt điều (lãi $50$ nghìn đồng/hộp) và hạt macca (lãi $60$ nghìn đồng/hộp). Mỗi hộp hạt điều cần $2$ giờ sấy khô và $2$ giờ đóng gói. Mỗi hộp hạt macca cần $1$ giờ sấy khô và $3$ giờ đóng gói. Mỗi tuần cơ sở có tối đa $120$ giờ sấy khô và $140$ giờ đóng gói. Tính số tiền lãi lớn nhất mỗi tuần mà cơ sở có thể thu được (đơn vị: nghìn đồng).],
    [3350],
    loigiai: [
...
#tln([Có tất cả bao nhiêu điểm có tọa độ nguyên $(x; y)$ thuộc miền tam giác nghiệm của hệ bất phương trình $cases(x >= 0, y >= 0, 2x + 3y <= 12)$?],
    [19],
    loigiai: [
...
#tln([Cho hình vuông $O A B C$ có 4 đỉnh $O(0;0), A(4;0), B(4;4), C(0;4)$ là miền nghiệm của hệ $cases(0 <= x <= 4, 0 <= y <= 4)$. Tìm giá trị nhỏ nhất của biểu thức $T = (x - 6)^2 + (y - 5)^2$ với điểm $(x; y)$ thuộc hình vuông đó.],
    [5],
    loigiai: [
...
#tln([Tìm giá trị dương của tham số $m$ để giá trị lớn nhất của biểu thức $F(x, y) = 2x + y$ trên miền nghiệm của hệ $cases(x + y <= 4, x >= 0, y >= 0)$ bằng $m^2 - 1$.],
    [3],
    loigiai: [
