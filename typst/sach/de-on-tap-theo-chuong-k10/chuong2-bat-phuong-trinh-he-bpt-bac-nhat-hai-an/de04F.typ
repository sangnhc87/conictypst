...
#show: thpt-school-exam.with(
  ...
  exam-title: "BÀI 4: QUY HOẠCH TUYẾN TÍNH & TOÁN THỰC TẾ (ĐỀ SỐ 3 - VẬN TẢI & TÀI CHÍNH)",
  subject: "TOÁN",
...
#tn([Một công ty logistics cần điều động $x$ chuyến xe tải loại $5$ tấn và $y$ chuyến xe tải loại $10$ tấn để vận chuyển hàng hóa. Điều kiện chuẩn của hai biến số $x$ và $y$ trong bài toán thực tế là:],
    (
        [$x > 0, y > 0$],
        [$x in RR, y in RR$],
...
#tn([Cần vận chuyển ít nhất $140$ tấn hàng. Mỗi chuyến xe loại $A$ chở được $10$ tấn hàng, mỗi chuyến xe loại $B$ chở được $20$ tấn hàng. Gọi $x, y$ lần lượt là số chuyến xe loại $A$ và loại $B$. Bất phương trình rút gọn nào sau đây mô tả lượng hàng cần vận chuyển?],
    (
        [$x + 2y <= 14$],
        [$2x + y >= 14$],
        True([$x + 2y >= 14$]),
...
#tn([Một doanh nghiệp dự định phát sóng phóng sự quảng cáo trên truyền hình ($x$ lần, giá $30$ triệu đồng/lần) và trên đài phát thanh ($y$ lần, giá $10$ triệu đồng/lần). Ngân sách tối đa cho chiến dịch là $300$ triệu đồng. Bất phương trình rút gọn về ngân sách là:],
    (
        [$x + 3y <= 30$],
        True([$3x + y <= 30$]),
        [$3x + y >= 30$],
...
#tn([Mỗi bài quảng cáo trên mạng xã hội Facebook ($x$ bài) tiếp cận được $50$ nghìn lượt xem; mỗi video trên TikTok ($y$ video) tiếp cận được $70$ nghìn lượt xem. Hàm mục tiêu biểu diễn tổng số lượt tiếp cận $T(x, y)$ (đơn vị: nghìn lượt) là:],
    (
        [$T(x, y) = 70x + 50y$],
        True([$T(x, y) = 50x + 70y$]),
        [$T(x, y) = 120(x + y)$],
...
#tn([Một vận động viên cử tạ cần nạp ít nhất $1800" kcal"$ trong bữa ăn chính từ thịt bò ($x$ lạng) và cá hồi ($y$ lạng). Mỗi lạng thịt bò cung cấp $250" kcal"$, mỗi lạng cá hồi cung cấp $200" kcal"$. Bất phương trình rút gọn nào sau đây đúng?],
    (
        [$4x + 5y >= 36$],
        True([$5x + 4y >= 36$]),
        [$5x + 4y <= 36$],
...
#tn([Bác An có $800$ triệu đồng để gửi tiết kiệm ngân hàng ($x$ triệu đồng, lãi suất $6\%$/năm) và mua trái phiếu chính phủ ($y$ triệu đồng, lãi suất $8\%$/năm). Biểu thức tính tổng tiền lãi $L(x, y)$ (đơn vị: triệu đồng) mà bác An thu được sau một năm là:],
    (
        [$L(x, y) = 0.08x + 0.06y$],
        [$L(x, y) = 6x + 8y$],
...
#tn([Cần vận chuyển ít nhất $400$ hành khách ra đảo du lịch. Mỗi chuyến tàu cao tốc ($x$ chuyến) chở được $80$ người, mỗi chuyến tàu cánh ngầm ($y$ chuyến) chở được $40$ người. Bất phương trình rút gọn thể hiện số hành khách được chở là:],
    (
        [$x + 2y >= 10$],
        True([$2x + y >= 10$]),
        [$2x + y <= 10$],
...
#tn([Một kho lạnh bảo quản hai loại hàng hóa: kiện thủy sản ($x$ kiện, chiếm $1.5" m"^3$/kiện) và kiện hoa quả tươi ($y$ kiện, chiếm $2" m"^3$/kiện). Tổng thể tích khả dụng của kho là $120" m"^3$. Bất phương trình mô tả thể tích kho là:],
    (
        [$2x + 1.5y <= 120$],
        [$3x + 4y >= 240$],
        True([$3x + 4y <= 240$]),
...
#tn([Miền nghiệm của bài toán thuê xe tải là tam giác $A B C$ có tọa độ các đỉnh $A(0; 10), B(4; 4), C(8; 0)$. Hàm tổng chi phí thuê xe là $C(x, y) = 4x + 3y$ (triệu đồng). Chi phí thuê xe nhỏ nhất là:],
    (
        [$30$ triệu đồng],
        True([$28$ triệu đồng]),
        [$32$ triệu đồng],
...
#tn([Miền nghiệm của chiến dịch quảng cáo là hình tứ giác $O A B C$ với các đỉnh $O(0; 0), A(10; 0), B(6; 8), C(0; 10)$. Hàm số người tiếp cận là $F(x, y) = 20x + 25y$ (nghìn người). Số người tiếp cận lớn nhất là:],
    (
        [$200$ nghìn người],
        [$250$ nghìn người],
...
#tn([Một quỹ đầu tư yêu cầu số tiền mua cổ phiếu mạo hiểm ($x$ tỷ đồng) không được vượt quá số tiền gửi trái phiếu an toàn ($y$ tỷ đồng). Bất phương trình nào sau đây đúng?],
    (
        [$x + y <= 0$],
        True([$x - y <= 0$]),
        [$x - y >= 0$],
...
#tn([Một trạm vận hành $x$ trụ sạc nhanh (tiêu thụ $60" kWh"$/giờ) và $y$ trụ sạc tiêu chuẩn (tiêu thụ $20" kWh"$/giờ). Trạm cần cung ứng ít nhất $180" kWh"$ điện mỗi giờ. Bất phương trình mô tả sản lượng điện cung ứng là:],
    (
        [$x + 3y >= 9$],
        True([$3x + y >= 9$]),
        [$3x + y <= 9$],
...
#ds([Một doanh nghiệp khởi nghiệp lên kế hoạch truyền thông cho sản phẩm bao bì tự hủy. Doanh nghiệp sử dụng hai kênh: Phóng sự truyền hình ($x$ phóng sự) và Bài viết mạng xã hội ($y$ bài). Mỗi phóng sự truyền hình tốn chi phí $20$ triệu đồng, cần $3$ ngày chuẩn bị và tiếp cận $40$ nghìn người. Mỗi bài viết mạng xã hội tốn chi phí $10$ triệu đồng, cần $1$ ngày chuẩn bị và tiếp cận $30$ nghìn người. Doanh nghiệp có ngân sách tối đa $120$ triệu đồng và tổng thời gian chuẩn bị không quá $15$ ngày. Xét tính đúng sai của các mệnh đề sau:],
  (
...
#ds([Một nhà đầu tư cá nhân có $1$ tỷ đồng ($1000$ triệu đồng) dự định phân bổ vào hai kênh: Quỹ trái phiếu an toàn ($x$ triệu đồng, lợi nhuận $7\%$/năm) và Quỹ cổ phiếu tăng trưởng ($y$ triệu đồng, lợi nhuận $12\%$/năm). Để quản trị rủi ro, nhà đầu tư đưa ra các quy tắc: số tiền mua cổ phiếu không quá $400$ triệu đồng, và số tiền đầu tư trái phiếu phải ít nhất gấp đôi số tiền đầu tư cổ phiếu . Xét tính đúng sai của các mệnh đề sau:],
  (
...
#tln([Một trường học cần thuê xe đưa $320$ học sinh đi tham quan trải nghiệm. Công ty du lịch có hai loại xe: xe $45$ chỗ (giá thuê $4$ triệu đồng/chuyến, có $6$ xe) và xe $29$ chỗ (giá thuê $3$ triệu đồng/chuyến, có $8$ xe). Tìm chi phí thuê xe thấp nhất (đơn vị: triệu đồng) để chở hết số học sinh.],
    [30],
    loigiai: [
...
#tln([Một vận động viên cần nạp ít nhất $60" g"$ Protein và $800" kcal"$ mỗi bữa. Mỗi lạng ức gà ($x$ lạng) chứa $30" g"$ Protein và $150" kcal"$, giá $15$ nghìn đồng. Mỗi lạng cá hồi ($y$ lạng) chứa $20" g"$ Protein và $200" kcal"$, giá $35$ nghìn đồng. Để chi phí bữa ăn rẻ nhất, vận động viên cần ăn bao nhiêu lạng cá hồi?],
    [1],
    loigiai: [
...
#tln([Một thương hiệu thời trang thuê màn hình LED tại Ngã tư ($x$ tuần) và Trung tâm thương mại ($y$ tuần). Mỗi tuần ở Ngã tư tốn $10$ triệu đồng, mang lại $50$ nghìn lượt xem. Mỗi tuần ở TTTM tốn $15$ triệu đồng, mang lại $90$ nghìn lượt xem. Ngân sách có $90$ triệu đồng và tổng số tuần phát sóng tối đa là $8$ tuần. Tính số lượt xem lớn nhất (đơn vị: nghìn lượt) mà thương hiệu có thể đạt được.],
    [540],
    loigiai: [
...
#tln([Một công ty giao nhận vận tải liên tỉnh kết hợp hai phương thức: Toa tàu hỏa hàng rời ($x$ toa) và Xe đầu kéo container ($y$ xe).
