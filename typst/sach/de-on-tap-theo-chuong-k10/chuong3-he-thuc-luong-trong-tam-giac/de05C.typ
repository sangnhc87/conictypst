...
#show: thpt-school-exam.with(
  ...
  exam-title: "BÀI 5: GIÁ TRỊ LƯỢNG GIÁC CỦA MỘT GÓC TỪ 0° ĐẾN 180° (ĐỀ SỐ 3)",
  subject: "TOÁN",
...
#tn([Cho góc $alpha$ thỏa mãn $0^circ <= alpha <= 180^circ$ và $sin alpha = 0$. Khẳng định nào sau đây đúng?],
    (
        [$alpha = 90^circ$],
        [$alpha = 0^circ$],
        [$alpha = 180^circ$],
...
// TN 2: Cosin bằng -1
#tn([Cho góc $alpha$ ($0^circ <= alpha <= 180^circ$) thỏa mãn $cos alpha = - 1$. Số đo của góc $alpha$ là:],
    (
        [$alpha = 0^circ$],
        [$alpha = 90^circ$],
        True([$alpha = 180^circ$]),
...
#tn([Giá trị của biểu thức $A = sin 30^circ dot cos 60^circ + cos 30^circ dot sin 60^circ$ bằng:],
    (
        [$0$],
        [$1/2$],
        True([$1$]),
...
#tn([Đẳng thức nào sau đây luôn đúng với mọi góc $alpha$ ($0^circ <= alpha <= 180^circ$)?],
    (
...
#tn([Với mọi góc $alpha != 90^circ$ ($0^circ <= alpha <= 180^circ$), biểu thức $P = tan alpha dot cos alpha$ bằng biểu thức nào sau đây?],
    (
        [$cos alpha$],
        True([$sin alpha$]),
        [$1$],
...
#tn([Cho góc nhọn $alpha$ thỏa mãn $cos alpha = 3/5$. Giá trị của $cot alpha$ bằng:],
    (
        [$4/5$],
        [$4/3$],
        True([$3/4$]),
...
#tn([Rút gọn biểu thức $S = 1 - sin^2(180^circ - alpha)$ ta được kết quả là:],
    (
        [$sin^2 alpha$],
        [$- cos^2 alpha$],
        True([$cos^2 alpha$]),
...
#tn([Cho tam giác tù $A B C$ có góc $hat(A) > 90^circ$. Khẳng định nào sau đây về dấu của các giá trị lượng giác là đúng?],
    (
...
#tn([Giá trị của biểu thức $C = sin^2 10^circ + sin^2 20^circ + sin^2 30^circ + dots + sin^2 90^circ$ (gồm 9 số hạng) bằng:],
    (
        [$4$],
        [$4.5$],
        True([$5$]),
...
#tn([Cho góc $alpha$ thỏa mãn $tan alpha = 3$. Giá trị của $cos^2 alpha$ bằng:],
    (
        [$1/9$],
        True([$1/10$]),
        [$9/10$],
...
#tn([Cho tam giác đều $A B C$. Côsin của góc giữa hai cạnh $A B$ và $A C$ bằng bao nhiêu?],
    (
        [$sqrt(3)/2$],
        True([$1/2$]),
        [$sqrt(2)/2$],
...
#tn([Một máy bay cất cánh từ đường băng với góc nâng không đổi $alpha = 20^circ$ so với mặt đất nằm ngang. Sau khi máy bay bay được quãng đường thẳng dài $3000" m"$, độ cao của máy bay so với mặt đất xấp xỉ bằng bao nhiêu mét?],
    (
        [$1000" m"$],
        True([$1026" m"$]),
        [$1098" m"$],
...
#ds([Cho góc $alpha$ thỏa mãn $0^circ < alpha < 180^circ$ và $sin alpha = 1/3$. Xét tính đúng sai của các khẳng định sau:],
  (
...
#ds([Cho tam giác nhọn $A B C$. Xét tính đúng sai của các đẳng thức lượng giác sau:],
  (
    True([$sin(A + B) = sin C$.]),
    True([$cos(B + C) = - cos A$.]),
...
#ds([Xét biểu thức $A = (sin^2 alpha - cos^2 alpha + 1) / (sin^2 alpha)$ với $sin alpha != 0$. Xét tính đúng sai của các mệnh đề sau:],
  (
...
#tln([Cho góc $alpha$ thỏa mãn $tan alpha = 3$. Tính giá trị của biểu thức $P = (2 sin^2 alpha + 3 sin alpha cos alpha + cos^2 alpha) / (sin^2 alpha - cos^2 alpha)$.],
    [3.5],
    loigiai: [
...
#tln([Tính giá trị của biểu thức $T = (1 + cot 135^circ) dot (1 + tan 45^circ) + sin 90^circ$.],
    [1],
    loigiai: [
...
#tln([Cho góc $alpha$ ($0^circ <= alpha <= 180^circ$) thỏa mãn $sin alpha + cos alpha = sqrt(2)$. Tìm số đo của góc $alpha$ (theo đơn vị độ).],
    [45],
    loigiai: [
...
#tln([Cho góc $alpha$ thỏa mãn $tan alpha + cot alpha = 4$. Tính giá trị của biểu thức $M = (tan alpha - cot alpha)^2$.],
    [12],
    loigiai: [
...
#tln([Hai trạm quan sát $A$ và $B$ trên mặt đất cách nhau $1000" m"$ cùng nằm trên đường thẳng đi qua hình chiếu vuông góc $H$ của một quả bóng thám không trên mặt đất (với $A$ nằm giữa $H$ và $B$). Tại cùng một thời điểm, trạm $A$ nhìn thấy quả bóng dưới góc nâng $45^circ$, trạm $B$ nhìn thấy quả bóng dưới góc nâng $30^circ$. Tính độ cao của quả bóng so với mặt đất (làm tròn kết quả đến hàng mét).],
    [1366],
    loigiai: [
...
#tln([Cho góc nhọn $alpha$ ($0^circ < alpha < 90^circ$). Tìm giá trị nhỏ nhất của biểu thức $G = 1 / (sin^2 alpha) + 1 / (cos^2 alpha)$.],
    [4],
    loigiai: [
