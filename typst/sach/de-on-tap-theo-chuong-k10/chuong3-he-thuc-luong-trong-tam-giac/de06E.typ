...
#show: thpt-school-exam.with(
  ...
  exam-title: "BÀI 6: TOÁN THỰC TẾ & ĐO ĐẠC HÌNH HỌC (ĐỀ E: HÀNG HẢI, HÀNG KHÔNG & RADAR)",
  subject: "TOÁN",
...
#tn([Từ cảng biển $O$, một tàu tuần tra biên phòng chạy theo hướng Đông một quãng đường $40$ hải lý đến vị trí $A$, sau đó rẽ vuông góc chạy thẳng về hướng Bắc một đoạn $30$ hải lý đến vị trí $B$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
#tn([Một ngọn hải đăng $O$ quan sát thấy hai tàu hàng $A$ và $B$. Khoảng cách từ hải đăng đến tàu $A$ là $6" km"$, đến tàu $B$ là $8" km"$, và khoảng cách đo được giữa hai tàu là $10" km"$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#tn([Một máy bay phản lực đang ở độ cao $1200" m"$, khoảng cách theo phương nằm ngang từ máy bay đến đầu đường băng là $15" km"$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#tn([Một thiết bị thu phát tín hiệu di động $M$ nhận tín hiệu từ hai trạm viễn thông $A$ và $B$ cách nhau $10" km"$. Thiết bị xác định khoảng cách đến trạm $A$ là $6" km"$ và đến trạm $B$ là $8" km"$. Góc nhìn $hat(A M B)$ từ thiết bị đến hai trạm bằng:],
    (
        [$60^circ$],
        [$120^circ$],
        True([$90^circ$]),
...
#tn([Một tàu ngầm lặn xuống biển theo đường thẳng tạo với mặt nước một góc nghiêng $15^circ$. Sau khi di chuyển được quãng đường dài $800" m"$ theo phương lặn.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#tn([Khi máy bay ở độ cao $300" m"$, phi công nhìn thấy cụm đèn hiệu ở đầu đường băng dưới góc hạ $3^circ$. Khoảng cách đường chim bay trực tiếp từ máy bay đến cụm đèn hiệu xấp xỉ bằng:],
    (
        [$4500" m"$],
        [$5000" m"$],
        True([$5732" m"$]),
...
#tn([Trạm radar hàng hải ven bờ $O$ có bán kính phát hiện mục tiêu là $50" km"$. Hai tàu biển $A$ và $B$ cùng di chuyển trên đường biên của vùng quét sóng ($O A = O B = 50" km"$), góc giữa hai tàu nhìn từ trạm radar là $hat(A O B) = 60^circ$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#tn([Một máy bay xuất phát từ $A$ bay đến $B$. Do gặp vùng giông sét, máy bay bay lệch một góc $20^circ$ so với lộ trình thẳng một đoạn $A C = 350" km"$. Khi góc $hat(A B C) = 30^circ$, khoảng cách $B C$ từ máy bay đến đích $B$ xấp xỉ bằng:],
    (
        [$210.5" km"$],
        True([$239.4" km"$]),
        [$265.8" km"$],
...
#tn([Một đội cứu hộ trên biển khoanh vùng khu vực tìm kiếm là một tam giác $A B C$ có hai cạnh $A B = 20" km", A C = 30" km"$ và góc xen giữa $hat(A) = 30^circ$. Diện tích vùng biển tìm kiếm bằng:],
    (
        [$300" km"^2$],
        True([$150" km"^2$]),
        [$150 sqrt(3)" km"^2$],
...
#tn([Hai cano cùng rời bến $O$ vào cùng một thời điểm. Cano thứ nhất chạy về hướng Đông với vận tốc $30" km/h"$, cano thứ hai chạy về hướng Đông Bắc (lệch $60^circ$ so với hướng Đông) với vận tốc $50" km/h"$. Sau $1$ giờ, khoảng cách giữa hai cano bằng:],
    (
        [$40" km"$],
        [$60" km"$],
        True([$43.6" km"$]),
...
#tn([Từ đài chỉ huy của một tàu sân bay có độ cao $36" m"$ so với mặt nước biển, tầm nhìn tối đa ra đường chân trời (bỏ qua khúc xạ ánh sáng, bán kính Trái Đất $R approx 6400" km"$) xấp xỉ bằng:],
    (
        [$15.5" km"$],
        True([$21.5" km"$]),
        [$28.2" km"$],
...
#tn([Khi một máy bay thực hiện động tác nghiêng cánh lượn vòng cung tròn với vận tốc không đổi, lực nâng tạo với phương thẳng đứng góc nghiêng $theta = 30^circ$. Tỷ số giữa thành phần lực hướng tâm nằm ngang và trọng lượng máy bay bằng:],
    (
        [$1$],
        [$sqrt(3)$],
        True([$sqrt(3)/3$]),
...
#ds([Trạm radar ven biển $O$ phát hiện hai tàu chở hàng $A$ và $B$ trong vùng quét sóng của mình. Khoảng cách đo được từ trạm đến tàu $A$ là $O A = 30" km"$, đến tàu $B$ là $O B = 50" km"$. Góc ngắm radar giữa hai tàu là $hat(A O B) = 120^circ$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#ds([Một chiếc máy bay phản lực thương mại bắt đầu giai đoạn tiếp cận hạ cánh (Glide path) thẳng đều về phía ngưỡng đường băng $T$. Khi máy bay ở vị trí $P$, thiết bị dẫn đường báo độ cao là $600" m"$ và góc hạ nhìn về đầu đường băng $T$ là $3^circ$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#ds([Ba máy bay trinh sát không người lái (UAV) bay ở cùng một độ cao tạo thành đội hình tam giác $A B C$ có các cạnh $a = 15" km", b = 24" km", c = 21" km"$. Xét tính đúng sai của các khẳng định sau:],
  (
...
#tln([Hai chiếc tàu rời cảng $O$ cùng lúc, tàu thứ nhất chạy về hướng Đông với vận tốc $15" km/h"$, tàu thứ hai chạy về hướng Nam với vận tốc $20" km/h"$. Tính khoảng cách giữa hai tàu sau $2$ giờ hành trình (đơn vị: km).],
    [50],
    loigiai: [
...
#tln([Một đội tàu tuần duyên kiểm soát một ngư trường hình tam giác $A B C$ có hai cạnh $a = 15" km", b = 20" km"$ và góc $hat(C) = 30^circ$. Tính diện tích của vùng ngư trường đó (đơn vị: $"km"^2$).],
    [75],
    loigiai: [
...
#tln([Hai cano tuần tra xuất phát từ trạm $O$, cano $A$ chạy được $10$ hải lý, cano $B$ chạy được $16$ hải lý. Góc tạo bởi hai hướng chạy của hai cano là $60^circ$. Tính khoảng cách giữa hai cano (đơn vị: hải lý).],
    [14],
    loigiai: [
        #step([Áp dụng định lý côsin])
...
#tln([Hai trạm hải đăng ven bờ $A$ và $B$ cách nhau $12" km"$ cùng nhận được tín hiệu cấp cứu $S O S$ phát ra từ một chiếc phao $C$. Trạm $A$ đo được góc $hat(C A B) = 60^circ$, trạm $B$ đo được góc $hat(C B A) = 45^circ$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#tln([Hai trạm radar $A$ và $B$ trên mặt bằng ven biển cách nhau $20" km"$ cùng phát hiện một máy bay không người lái $M$ đang bay trên biển (trong cùng mặt phẳng thẳng đứng đi qua $A B$, máy bay nằm giữa $A$ và $B$). Góc nâng nhìn lên máy bay tại trạm $A$ là $30^circ$, tại trạm $B$ là $60^circ$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#tln([Một trạm radar cảnh báo bờ biển phát chùm sóng thẳng đứng có cửa sổ phát sóng từ độ cao $9" m"$ (điểm $B$) đến độ cao $25" m"$ (điểm $A$) so với mặt biển. Một tàu tuần tra cao tốc di chuyển trên mặt nước.
#align(center)[
    #cetz.canvas({
        import cetz.draw: *
        set-style(stroke: 0.8pt)
