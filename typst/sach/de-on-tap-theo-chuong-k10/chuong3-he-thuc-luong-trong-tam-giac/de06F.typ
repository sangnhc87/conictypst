...
// ═══════════════════════════════════════════════════════════
#let mode = "loigiai"
#let accent = rgb("d97706") // Amber / Golden Solar
...
#show: thpt-school-exam.with(
  ...
  exam-title: "BÀI 6: TOÁN THỰC TẾ & ĐO ĐẠC HÌNH HỌC (ĐỀ F: NĂNG LƯỢNG TÁI TẠO, QUANG HỌC & TỐI ƯU HÓA)",
  subject: "TOÁN",
...
#tn([Vào một thời điểm trong ngày, các tia nắng mặt trời chiếu xuống mặt đất tạo với phương nằm ngang một góc $60^circ$. Một tòa nhà cao tầng có chiều cao thẳng đứng $H = 45" m"$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
#tn([Để tấm pin năng lượng mặt trời đón ánh sáng mặt trời đạt hiệu suất cao nhất, mặt phẳng tấm pin cần vuông góc với tia sáng tới. Tại một địa phương vào giữa trưa, tia nắng hợp với phương thẳng đứng một góc $35^circ$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#tn([Một cột đèn cao áp chiếu sáng quảng trường có chiều cao $12" m"$. Đèn phát ra chùm sáng hình nón có góc mở $2 theta = 60^circ$ hướng đối xứng thẳng đứng xuống mặt đất.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#tn([Một camera an ninh góc rộng được gắn trên trần nhà cao $3" m"$ ngay trên mép cửa ra vào. Camera bao quát một dải sàn nhà phía trước từ vị trí $A$ (cách chân tường $2" m"$) đến vị trí $B$ (cách chân tường $8" m"$).
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#tn([Một cửa sổ kính cao $2" m"$. Để ngăn ánh nắng trưa chiếu trực tiếp vào phòng khi góc nâng của mặt trời là $60^circ$, người ta làm một mái đua che nắng nhô ra nằm ngang từ mép trên của cửa sổ. Chiều dài nhô ra tối thiểu của mái che xấp xỉ bằng:],
    (
        [$0.85" m"$],
        True([$1.15" m"$]),
        [$1.45" m"$],
...
#tn([Một tấm pin năng lượng mặt trời chuyên dụng có dạng tam giác cân $A B C$ với hai cạnh bên $A B = A C = 2" m"$ và góc ở đỉnh $hat(A) = 120^circ$. Diện tích mặt thu nhiệt của tấm pin bằng:],
    (
        [$2" m"^2$],
        [$2 sqrt(3)" m"^2$],
        True([$sqrt(3)" m"^2$]),
...
#tn([Từ đài kiểm soát $C$ của một nông trại điện gió, người ta đo được khoảng cách đến hai trụ tuabin $A$ và $B$ lần lượt là $C A = 300" m", C B = 500" m"$ và góc $hat(A C B) = 60^circ$. Khoảng cách giữa hai trụ tuabin gió $A$ và $B$ xấp xỉ bằng:],
    (
        [$400.0" m"$],
        True([$435.9" m"$]),
        [$465.2" m"$],
...
#tn([Một gương cầu gom nhiệt mặt trời có mặt cắt là một cung tròn ngoại tiếp tam giác $A B C$ tạo bởi ba chốt gắn khung. Biết khoảng cách $A B = 3" m"$ và góc đối diện $hat(A C B) = 30^circ$. Bán kính mặt cong $R$ của gương bằng:],
    (
        [$1.5" m"$],
        True([$3.0" m"$]),
        [$6.0" m"$],
...
#tn([Một cột đo gió của trạm điện gió cao $40" m"$. Người ta kéo một sợi dây thép neo giữ từ đỉnh cột xuống một mốc cố định trên mặt đất cách chân cột $30" m"$. Chiều dài sợi dây thép neo bằng:],
    (
        [$45" m"$],
        True([$50" m"$]),
        [$60" m"$],
...
#tn([Cường độ bức xạ mặt trời chiếu vuông góc vào mặt đất là $I_0 = 1000" W/m"^2$. Khi tia nắng tạo với mặt đất một góc nghiêng $30^circ$, cường độ bức xạ mặt trời hấp thụ trên mỗi mét vuông mặt đất bằng:],
    (
        [$1000" W/m"^2$],
        [$866" W/m"^2$],
        True([$500" W/m"^2$]),
...
#tn([Một trụ tuabin gió được giữ thăng bằng nhờ các sợi cáp thép tạo với mặt đất một góc $60^circ$. Vị trí cọc neo dưới đất cách chân trụ $25" m"$. Chiều dài mỗi sợi dây cáp néo bằng:],
    (
        [$25 sqrt(3)" m"$],
        [$35" m"$],
        True([$50" m"$]),
...
#tn([Một tuabin gió ba cánh đang quay, tại một thời điểm hai cánh quạt liên tiếp tạo với tâm trục quay một góc $120^circ$. Biết chiều dài mỗi cánh quạt là $30" m"$. Diện tích hình tam giác tạo bởi tâm quay và hai đầu mút cánh quạt bằng:],
    (
        [$450" m"^2$],
        True([$225 sqrt(3)" m"^2$]),
        [$450 sqrt(3)" m"^2$],
...
#ds([Một hệ thống pin quang điện mặt trời được lắp đặt trên một mái nhà dốc nghiêng $25^circ$ so với phương nằm ngang. Tại thời điểm nắng đỉnh điểm trong ngày, các tia nắng mặt trời chiếu xiên tạo với phương nằm ngang một góc $55^circ$. Mảng pin mặt trời có chiều dài theo mặt dốc là $L = 6" m"$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#ds([Trên dải phân cách của một đoạn đường cao tốc, người ta dựng hai cột đèn cao áp $A$ và $B$ cách nhau $A B = 40" m"$, mỗi cột có chiều cao $H = 15" m"$. Đèn phát chùm sáng hình nón rộng rọi đều về hai phía.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#ds([Một nút giao thông mở rộng có hình dạng tam giác $A B C$ với độ dài ba cạnh biên lần lượt là $a = B C = 70" m", b = C A = 80" m", c = A B = 50" m"$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#ds([Một giàn thép không gian dạng tam giác cân $A B C$ có đáy $B C = 8" m"$ và hai thanh chống nghiêng $A B = A C = 5" m"$. Tại đỉnh $A$ đặt bồn nước nóng năng lượng mặt trời. Để gia cố chịu lực, kỹ sư lắp thêm một thanh giằng ngang $M N$ nối trung điểm $M$ của $A B$ và trung điểm $N$ của $A C$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#tln([Một cột thu lôi chống sét được dựng thẳng đứng trên mặt đất. Khi mặt trời tạo với mặt đất một góc nâng $30^circ$, cột thu lôi đổ bóng râm dài $52" m"$ trên mặt đất nằm ngang.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#tln([Một khu đất xây dựng trang trại điện mặt trời có dạng hình tam giác $A B C$ với hai cạnh biên $A B = 300" m", A C = 400" m"$ và góc xen giữa $hat(A) = 30^circ$. Tính diện tích khu đất trang trại này theo đơn vị hécta (biết $1" ha" = 10000" m"^2$).],
    [3],
    loigiai: [
...
#tln([Hai trụ tuabin gió $A$ và $B$ được quan sát từ trung tâm điều hành $O$. Đo được $O A = 500" m", O B = 700" m"$ và góc ngắm $hat(A O B) = 60^circ$. Tính khoảng cách giữa hai trụ tuabin gió (làm tròn đến hàng đơn vị theo mét).],
    [624],
    loigiai: [
...
#tln([Để lắp đặt tấm pin mặt trời trên đỉnh đồi, kỹ sư đo góc nâng từ chân đồi $A$ lên đỉnh đồi $D$ được $30^circ$. Sau khi tiến lại gần chân đồi một khoảng $60" m"$ đến vị trí $B$, góc nâng đo được là $45^circ$. Tính chiều cao của ngọn đồi so với mặt bằng đo (làm tròn đến hàng đơn vị theo mét).],
    [82],
    loigiai: [
...
#tln([Một cánh quạt tuabin gió $O A$ đang quay tròn tạo với mặt ngang một góc $45^circ$. Khoảng cách từ đầu mút cánh quạt $A$ đến trục quay $O$ theo phương thẳng đứng là $28" m"$. Tính chiều dài của cánh quạt $O A$ (làm tròn đến hàng đơn vị theo mét).],
    [40],
    loigiai: [
