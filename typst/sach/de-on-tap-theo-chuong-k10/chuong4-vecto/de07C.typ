...
#show: thpt-school-exam.with(
  ...
  exam-title: "BÀI 7: CÁC KHÁI NIỆM MỞ ĐẦU VỀ VECTƠ (ĐỀ SỐ 3)",
  subject: "TOÁN",
...
// TN 1
#tn([Cho ba điểm phân biệt $A, B, C$. Mệnh đề nào sau đây là điều kiện cần và đủ để ba điểm $A, B, C$ thẳng hàng?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 2
#tn([Cho đoạn thẳng $A B$ có độ dài bằng $8" cm"$. Điểm $M$ nằm trên đường thẳng $A B$ sao cho vectơ $vec(A M)$ cùng hướng với vectơ $vec(A B)$ và $|vec(A M)| = 3" cm"$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 3
#tn([Cho tam giác $A B C$. Có bao nhiêu vectơ khác vectơ-không có điểm đầu là đỉnh $A$ và điểm cuối là một đỉnh của tam giác $A B C$?],
    (
        True([$2$]),
        [$3$],
        [$4$],
...
// TN 4
#tn([Cho hình thoi $A B C D$ có độ dài cạnh bằng $a$. Khẳng định nào sau đây là đúng về độ dài các vectơ cạnh?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 5
#tn([Trong cơ học, một lực tác dụng lên một vật thể được mô hình hóa bằng một vectơ vì lực có đầy đủ các yếu tố:],
    (
...
// TN 6
#tn([Hai lực cân bằng cùng tác dụng vào một chất điểm là hai lực được biểu diễn bởi hai vectơ:],
    (
...
// TN 7
#tn([Cho hình bình hành $A B C D$ tâm $O$. Vectơ đối của vectơ $vec(O A)$ là vectơ nào sau đây?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 8
#tn([Cho hình vuông $A B C D$ có cạnh bằng $6" cm"$. Độ dài của vectơ đường chéo $vec(A C)$ bằng
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 9
#tn([Cho tam giác đều $A B C$. Khẳng định nào sau đây là SAI?],
    (
...
// TN 10
#tn([Cho hình chữ nhật $A B C D$ tâm $O$. Cặp vectơ nào sau đây là hai vectơ đối nhau?],
    (
...
// TN 11
#tn([Cho đoạn thẳng $A B$ có trung điểm là $I$. Tổng của hai vectơ $vec(I A) + vec(I B)$ bằng],
    (
        True([$vec(0)$]),
        [$vec(A B)$],
        [$vec(B A)$],
...
// TN 12
#tn([Một chất điểm $M$ đứng yên chịu tác dụng của hai lực $vec(F)_1$ và $vec(F)_2$. Mối quan hệ giữa hai vectơ lực là],
    (
...
#ds([Trên đường thẳng $d$, cho đoạn thẳng $A B$ có độ dài bằng $12" cm"$. Lấy điểm $C$ thuộc đoạn thẳng $A B$ sao cho $A C = 4" cm"$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#ds([Cho hình chữ nhật $A B C D$ có $A B = 8" cm"$, $B C = 6" cm"$. Gọi $O$ là giao điểm của hai đường chéo $A C$ và $B D$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#ds([Một chiếc xà lan chở container ở vị trí $O$ được hai chiếc tàu kéo lai dắt bằng hai sợi dây cáp thép. Hai dây cáp tạo với nhau một góc vuông $90^circ$. Tàu kéo thứ nhất tác dụng lực kéo $vec(F)_1$ có độ lớn $6000" N"$, tàu kéo thứ hai tác dụng lực kéo $vec(F)_2$ có độ lớn $8000" N"$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TLN 1 (Câu 17)
#tln([Cho lục giác đều $A B C D E F$. Hỏi có bao nhiêu vectơ khác vectơ-không có điểm đầu và điểm cuối là hai đỉnh phân biệt của lục giác đã cho?],
    [30],
    loigiai: [
...
// TLN 2 (Câu 18)
#tln([Cho hình thoi $A B C D$ có độ dài hai đường chéo lần lượt là $A C = 16" cm"$ và $B D = 12" cm"$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TLN 3 (Câu 19)
#tln([Cho tam giác $A B C$ vuông tại $A$ có góc $hat(B) = 30^circ$ và cạnh huyền $B C = 10" cm"$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TLN 4 (Câu 20)
#tln([Cho tam giác đều $A B C$ có độ dài cạnh bằng $9" cm"$. Gọi $M$ là trung điểm của cạnh $B C$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#tln([Một chất điểm $O$ nằm cân bằng trên mặt phẳng dưới tác dụng của ba lực đồng phẳng $vec(F)_1, vec(F)_2, vec(F)_3$ thỏa mãn $vec(F)_1 + vec(F)_2 + vec(F)_3 = vec(0)$. Biết rằng hai lực $vec(F)_1$ và $vec(F)_2$ có phương vuông góc với nhau và có độ lớn lần lượt là $F_1 = 36" N"$, $F_2 = 48" N"$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#tln([Hai chiếc tàu kéo cùng tác dụng hai lực kéo $vec(F)_1$ và $vec(F)_2$ vào một chiếc sà lan tại điểm $O$. Hai lực kéo này tạo với nhau một góc $60^circ$ và có độ lớn lần lượt là $F_1 = 4000" N"$, $F_2 = 6000" N"$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
