...
#show: thpt-school-exam.with(
  ...
  exam-title: "BÀI 8: TỔNG VÀ HIỆU CỦA HAI VECTƠ (ĐỀ SỐ 2)",
  subject: "TOÁN",
...
// TN 1
#tn([Cho tam giác $A B C$ có trọng tâm là $G$. Đẳng thức vectơ nào sau đây luôn đúng?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 2
#tn([Cho đoạn thẳng $A B$ có trung điểm là $I$. Với điểm $M$ tùy ý trong mặt phẳng, khẳng định nào sau đây là đúng?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 3
#tn([Cho lục giác đều $A B C D E F$ có tâm đối xứng $O$. Tổng hai vectơ $vec(O A) + vec(O D)$ bằng vectơ nào?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 4
#tn([Cho hình bình hành $A B C D$. Biểu thức $vec(C B) + vec(C D)$ bằng vectơ nào sau đây?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 5
#tn([Cho hình vuông $A B C D$ có cạnh bằng $a$. Độ dài của vectơ $vec(A B) - vec(B C)$ bằng
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 6
#tn([Cho ba điểm phân biệt $A, B, C$. Biểu thức $vec(A B) - vec(C B)$ bằng vectơ nào sau đây?],
    (
        True([$vec(A C)$]),
        [$vec(C A)$],
        [$vec(B C)$],
...
// TN 7
#tn([Cho tam giác $A B C$. Gọi $M$ là trung điểm của cạnh $B C$. Hiệu hai vectơ $vec(B M) - vec(C M)$ bằng vectơ nào?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 8
#tn([Điều kiện cần và đủ để điểm $G$ là trọng tâm của tam giác $A B C$ là],
    (
        True([$vec(G A) + vec(G B) + vec(G C) = vec(0)$]),
        [$G A = G B = G C$],
...
// TN 9
#tn([Cho hình thoi $A B C D$ có cạnh bằng $a$ và góc $hat(B A D) = 60^circ$. Độ dài của vectơ $vec(A B) + vec(A D)$ bằng
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 11
#tn([Cho hai vectơ khác vectơ-không $vec(a)$ và $vec(b)$. Đẳng thức $|vec(a) + vec(b)| = |vec(a) - vec(b)|$ xảy ra khi và chỉ khi],
    (
...
// TN 12
#tn([Cho bốn điểm bất kỳ $A, B, C, D$. Khẳng định nào sau đây luôn đúng?],
    (
...
#ds([Cho lục giác đều $A B C D E F$ có tâm $O$ và cạnh bằng $a$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#ds([Cho hình thoi $A B C D$ có cạnh bằng $a$ và góc $hat(B A D) = 60^circ$. Gọi $O$ là giao điểm của hai đường chéo $A C$ và $B D$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#ds([Một chiếc đèn chùm trang trí phòng khách có trọng lượng $P = 400" N"$ được treo cố định thăng bằng tại điểm $O$ nhờ hai sợi dây cáp đối xứng $O A$ và $O B$ gắn chặt vào trần nhà. Hai sợi dây cáp tạo với nhau một góc $120^circ$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#ds([Một chiếc cano chạy xuôi dòng trên một khúc sông từ bến $A$ đến bến $B$ cách nhau $72" km"$. Vận tốc riêng của cano đối với dòng nước là $v_c = 30" km/h"$, vận tốc dòng chảy của nước đối với bờ sông là $v_n = 6" km/h"$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TLN 1 (Câu 17)
#tln([Cho hình thoi $A B C D$ có cạnh bằng $12" cm"$ và góc $hat(B A D) = 120^circ$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TLN 2 (Câu 18)
#tln([Cho tam giác $A B C$ vuông tại $A$ có $A B = 9" cm"$ và $A C = 12" cm"$. Gọi $M$ là trung điểm của cạnh huyền $B C$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TLN 3 (Câu 19)
#tln([Cho hình chữ nhật $A B C D$ có $A B = 8" cm"$ và $B C = 6" cm"$. Gọi $O$ là giao điểm của hai đường chéo $A C$ và $B D$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TLN 4 (Câu 20)
#tln([Cho tam giác đều $A B C$ có cạnh bằng $12" cm"$. Gọi $G$ là trọng tâm của tam giác $A B C$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#tln([Hai lực $vec(F)_1$ và $vec(F)_2$ có cùng độ lớn $F_1 = F_2 = 250" N"$ cùng tác dụng vào một móc cẩu tại điểm $O$, tạo với nhau một góc $120^circ$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#tln([Một chiếc máy bay phản lực đang bay thẳng theo hướng Bắc với vận tốc riêng của động cơ là $500" km/h"$. Luồng gió thổi mạnh theo phương nằm ngang từ hướng Tây sang Đông với vận tốc $120" km/h"$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
