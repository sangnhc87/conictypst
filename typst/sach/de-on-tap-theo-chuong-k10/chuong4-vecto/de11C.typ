...
#show: thpt-school-exam.with(
  ...
  exam-title: "BÀI 11: TÍCH VÔ HƯỚNG CỦA HAI VECTƠ (ĐỀ SỐ 3 - NÂNG CAO)",
  subject: "TOÁN",
...
// TN 1
#tn([Trong mặt phẳng, cho hai điểm cố định $A$ và $B$ phân biệt. Tập hợp tất cả các điểm $M$ thỏa mãn đẳng thức vectơ $vec(M A) dot vec(M B) = 0$ là
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 2
#tn([Cho đoạn thẳng $A B$ có độ dài bằng $2a$ và trung điểm $I$. Với điểm $M$ tùy ý trong mặt phẳng, biểu thức $vec(M A) dot vec(M B)$ luôn bằng
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 3
#tn([Cho đoạn thẳng $A B$ có độ dài $A B = 6$, gọi $I$ là trung điểm của $A B$. Tập hợp các điểm $M$ thỏa mãn hệ thức $vec(M A) dot vec(M B) = 16$ là một đường tròn tâm $I$ có bán kính $R$ bằng
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 4
#tn([Cho tam giác $A B C$ có độ dài ba cạnh $A B = 5, A C = 7$ và $B C = 8$. Giá trị của tích vô hướng $vec(A B) dot vec(A C)$ bằng
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 5
#tn([Cho hai vectơ $vec(u)$ và $vec(v)$ thỏa mãn $|vec(u)| = 2, |vec(v)| = 3$ và góc giữa chúng $(vec(u), vec(v)) = 60^circ$. Độ dài của vectơ tổng $|vec(u) + vec(v)|$ bằng
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 6
#tn([Cho hai vectơ $vec(a)$ và $vec(b)$ thỏa mãn $|vec(a)| = 4, |vec(b)| = 5$ và tích vô hướng $vec(a) dot vec(b) = -10$. Độ dài của vectơ hiệu $|vec(a) - vec(b)|$ bằng
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 7
#tn([Cho tam giác đều $A B C$ có cạnh bằng $a$. Điểm $M$ thuộc cạnh $B C$ sao cho $B M = 1/3 B C$. Tích vô hướng $vec(A M) dot vec(B C)$ bằng
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 8
#tn([Cho hai vectơ không cùng phương $vec(a)$ và $vec(b)$ có $|vec(a)| = 2, |vec(b)| = 3$ và góc $(vec(a), vec(b)) = 60^circ$. Giá trị của tham số thực $k$ để vectơ $vec(u) = 2 vec(a) + k vec(b)$ vuông góc với vectơ $vec(v) = vec(a) - vec(b)$ là],
    (
        True([$k = 1/3$]),
        [$k = - 1/3$],
        [$k = 5/6$],
...
// TN 9
#tn([Cho tam giác $A B C$ có $A B = 5, A C = 6$ và $B C = 7$. Gọi $M$ là trung điểm của cạnh $B C$. Giá trị của tích vô hướng $vec(A M) dot vec(A B)$ bằng
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 10
#tn([Cho hình thang vuông $A B C D$ tại $A$ và $D$ có $A B = a, A D = a$ và đáy lớn $C D = 2a$. Tích vô hướng $vec(A C) dot vec(B D)$ bằng
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 11
#tn([Cho tam giác $A B C$ có trọng tâm là $G$. Đẳng thức nào sau đây luôn đúng với mọi điểm $M$ trong mặt phẳng?],
    (
...
// TN 12
#tn([Cho tam giác $A B C$ có độ dài cạnh $B C = a$. Gọi $I, J$ lần lượt là trung điểm của các cạnh $A B$ và $A C$. Tập hợp tất cả các điểm $M$ thỏa mãn hệ thức vectơ $(vec(M A) + vec(M B)) dot (vec(M A) + vec(M C)) = 0$ là
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#ds([Trong mặt phẳng tọa độ $O x y$, cho hai điểm $A(1; 2)$ và $B(5; 6)$. Gọi $(C)$ là tập hợp các điểm $M(x; y)$ thỏa mãn hệ thức vectơ: $vec(M A) dot vec(M B) = 0$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#ds([Trong mặt phẳng tọa độ $O x y$, cho hai điểm $A(2; 3)$ và $B(6; 1)$. Điểm $M(x; 0)$ di động trên trục hoành $O x$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#ds([Một vệ tinh viễn thám bay trên quỹ đạo thấp (LEO) với vận tốc ban đầu $vec(v)_0$ có độ lớn $v_0 = 7.5" km/s"$. Để nâng quỹ đạo, động cơ đẩy kích hoạt sinh ra xung vận tốc bổ sung $Delta vec(v)$ có độ lớn $1.0" km/s"$, hợp với vectơ vận tốc ban đầu một góc $theta = 60^circ$. Vectơ vận tốc mới của vệ tinh sau khi đốt động cơ là $vec(v) = vec(v)_0 + Delta vec(v)$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TLN 2 (Câu 18)
#tln([Trong mặt phẳng tọa độ $O x y$, cho hai điểm $A(1; 3)$ và $B(5; 1)$. Điểm $M(x; 0)$ thuộc trục hoành $O x$ sao cho tích vô hướng $vec(M A) dot vec(M B)$ đạt giá trị nhỏ nhất.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TLN 3 (Câu 19)
#tln([Cho tam giác $A B C$ có độ dài ba cạnh là $a = 7, b = 8$ và $c = 5$ (với $a = B C, b = A C, c = A B$).
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TLN 4 (Câu 20)
#tln([Cho hai vectơ $vec(u)$ và $vec(v)$ thỏa mãn $|vec(u)| = 3, |vec(v)| = 4$ và $|vec(u) + vec(v)| = 5$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#tln([Một đầu máy xe lửa kéo đoàn toa tàu chuyển động thẳng đều trên đường ray với lực kéo không đổi $F = 40000" N"$ cùng hướng với chuyển động. Tàu di chuyển với tốc độ ổn định $v = 15" m/s"$. Công suất kéo cơ học tức thời của đầu máy được tính theo tích vô hướng $P = vec(F) dot vec(v)$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
