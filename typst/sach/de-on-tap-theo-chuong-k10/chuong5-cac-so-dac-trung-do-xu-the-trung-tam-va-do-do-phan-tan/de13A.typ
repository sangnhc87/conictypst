...
#show: thpt-school-exam.with(
  ...
  exam-title: "BÀI 13: CÁC SỐ ĐẶC TRƯNG ĐO XU THẾ TRUNG TÂM (ĐỀ A: CƠ BẢN - VẬN DỤNG)",
  subject: "TOÁN",
...
// TN 1
#tn([Kết quả đo chiều dài của một cây cầu vượt đường cao tốc được ghi nhận là $L = 152" m" plus.minus 0.2" m"$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 2
#tn([Khi làm tròn số $pi approx 3.14159265...$ đến hàng phần nghìn (chữ số thập phân thứ ba), ta được số gần đúng là],
    (
        True([$3.142$]),
        [$3.141$],
        [$3.140$],
...
// TN 3
#tn([Một thửa ruộng hình chữ nhật có chiều dài đo được là $a = 40" m" plus.minus 0.1" m"$. Sai số tương đối của phép đo chiều dài thửa ruộng không vượt quá],
    (
        True([$0.25%$]),
        [$0.5%$],
        [$0.1%$],
...
// TN 4
#tn([Điểm kiểm tra thường xuyên môn Toán của một nhóm gồm $7$ học sinh là:
#align(center)[
  #table(
    columns: 7,
    align: center,
...
// TN 5
#tn([Thời gian chạy cự ly $100" m"$ (tính bằng giây) của $9$ vận động viên được ghi lại như sau:
#align(center)[
...
// TN 6
#tn([Khối lượng của $8$ quả bưởi da xanh (đơn vị: kilôgam) được cân trong đợt thu hoạch là:
#align(center)[
...
// TN 7
#tn([Cho mẫu số liệu điểm đánh giá năng lực của $7$ ứng viên:
#align(center)[
...
// TN 8
#tn([Khảo sát cỡ áo phông của $30$ bạn học sinh trong một lớp học, kết quả thu được bảng tần số như sau:
#align(center)[
  #table(
    columns: 5,
    align: center,
...
// TN 9
#tn([Thu nhập hàng tháng (triệu đồng) của $5$ nhân viên trong một công ty khởi nghiệp là:
#align(center)[
...
// TN 10
#tn([Cho biết $sqrt(3) = 1.7320508...$. Khi làm tròn số $sqrt(3)$ với độ chính xác $d = 0.005$, ta được kết quả là],
    (
        True([$1.73$]),
        [$1.732$],
        [$1.7$],
...
// TN 11
#tn([Một xạ thủ bắn $20$ phát súng vào bia, điểm số đạt được ghi lại trong bảng tần số sau:
#align(center)[
  #table(
    columns: 5,
    align: center,
...
// TN 12
#tn([Biểu đồ hộp (Box plot) dưới đây tóm tắt kết quả kiểm tra chất lượng sản phẩm của một dây chuyền tự động:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
#ds([Kỹ sư cầu đường sử dụng thiết bị laser để đo bề rộng mặt cầu $b$ và chiều dài nhịp cầu chính $L$. Kết quả đo đạc kỹ thuật được công bố như sau:
...
#ds([Một giáo viên chủ nhiệm thống kê thời gian sử dụng điện thoại thông minh phục vụ học tập và tra cứu trực tuyến trong một tuần của $10$ học sinh (đơn vị: giờ):
#align(center)[
...
#ds([Quản đốc phân xưởng theo dõi số lượng kiện hàng được đóng gói trong một giờ của hai công nhân $A$ và $B$ trong $7$ ngày làm việc liên tục:
...
#ds([Cho mẫu số liệu điểm kiểm tra $x_1, x_2, dots, x_n$ có số trung bình là $overline(x) = 7.0$, trung vị $M_e = 7.5$ và mốt $M_o = 8.0$. Do đề bài có một câu bị nhầm lẫn dữ kiện, giáo viên quyết định cộng thêm $1.0$ điểm cho tất cả học sinh và sau đó nhân hệ số $1.2$ theo công thức quy đổi mới: $y_i = 1.2(x_i + 1.0) = 1.2 x_i + 1.2$.
Xét tính đúng sai của các nhận định sau:],
  (
...
// TLN 1 (Số trung bình)
#tln([Điểm thi môn Ngữ văn của $8$ học sinh trong một tổ học tập là:
#align(center)[
...
#tln([Thời gian chờ khám bệnh (tính bằng phút) của $9$ bệnh nhân tại phòng khám đa khoa là:
#align(center)[
...
#tln([Khảo sát số giờ tự học mỗi tuần của $8$ sinh viên:
#align(center)[
...
#tln([Với cùng mẫu số liệu số giờ tự học của $8$ sinh viên ở Câu 19:
#align(center)[
  $14; quad 16; quad 18; quad 20; quad 22; quad 24; quad 26; quad 30$
]
...
// TLN 5 (Mốt)
#tln([Khảo sát số ly trà sữa bán được trong ngày của một quán giải khát theo từng loại cỡ: Cỡ nhỏ bán $25$ ly; Cỡ vừa bán $84$ ly; Cỡ lớn bán $46$ ly.
...
#tln([Một nhà trắc địa đo chiều cao ngọn đồi được kết quả $h = 345.68" m" plus.minus 0.2" m"$.
