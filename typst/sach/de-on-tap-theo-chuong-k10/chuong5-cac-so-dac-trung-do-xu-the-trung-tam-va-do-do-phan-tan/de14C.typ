...
#show: thpt-school-exam.with(
  ...
  exam-title: "BÀI 14: CÁC SỐ ĐẶC TRƯNG ĐO ĐỘ PHÂN TÁN (ĐỀ C: NÂNG CAO - VD, VDC)",
  subject: "TOÁN",
...
// TN 1
#tn([Cho mẫu số liệu gồm $5$ giá trị: $1; quad 2; quad 3; quad 4; quad x$. Biết rằng số trung bình của mẫu số liệu này bằng $3.0$. Phương sai $s^2$ của mẫu số liệu bằng],
    (
        True([$2.0$]),
        [$1.5$],
        [$2.5$],
...
// TN 2
#tn([Một nhà nghiên cứu thu thập mẫu số liệu gồm $n = 10$ quan sát. Sử dụng máy tính cầm tay thu được tổng các giá trị $sum_(i=1)^(10) x_i = 60$ và tổng các bình phương $sum_(i=1)^(10) x_i^2 = 460$. Phương sai $s^2$ của mẫu số liệu trên bằng],
    (
        True([$10$]),
        [$16$],
        [$6$],
...
// TN 3
#tn([Hệ số biến thiên $C V = s / overline(x) dot 100%$ (với $overline(x) > 0$) được sử dụng để so sánh mức độ phân tán tương đối giữa hai mẫu số liệu.
...
// TN 4
#tn([Cho mẫu số liệu $x_1, x_2, dots, x_n$ ($n >= 2$). Điều kiện cần và đủ để phương sai của mẫu số liệu bằng $0$ ($s^2 = 0$) là],
    (
...
// TN 5
#tn([Biểu đồ hộp CeTZ dưới đây mô tả sự phân bố thời gian tải trang (mili-giây) của một cổng dịch vụ công trực tuyến:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 6
#tn([Với mọi số thực $c$, xét hàm số $f(c) = sum_(i=1)^n (x_i - c)^2$. Giá trị của $c$ để hàm số $f(c)$ đạt giá trị nhỏ nhất là],
    (
...
// TN 7
#tn([Cho mẫu số liệu gồm $4$ số thực: $2; quad 4; quad 6; quad x$ với $x > 6$. Biết khoảng biến thiên của mẫu số liệu bằng $R = 8$. Giá trị của $x$ là],
    (
        True([$10$]),
        [$8$],
        [$12$],
...
// TN 8
#tn([Cho mẫu số liệu $x_1, x_2, dots, x_n$ có phương sai $s^2 = 4$. Nếu mỗi giá trị $x_i$ được nhân với $-3$ và trừ đi $7$, ta được mẫu số liệu mới $y_i = -3 x_i - 7$. Phương sai $s_y^2$ của mẫu mới bằng],
    (
        True([$36$]),
        [$-12$],
        [$12$],
...
// TN 9
#tn([Khảo sát độ hao mòn (micromét) của hai loại lưỡi dao tiện cơ khí $M$ và $N$ sau $100$ giờ gia công chi tiết:
...
// TN 10
#tn([Cho mẫu số liệu điểm đánh giá chất lượng phần mềm của $10$ chuyên gia:
#align(center)[
...
// TN 11
#tn([Một mẫu số liệu gồm $n$ quan sát có phương sai $s^2$. Nếu ta nhân đôi tất cả các giá trị của mẫu số liệu đó thì độ lệch chuẩn của mẫu số liệu mới bằng],
    (
...
// TN 12
#tn([Khảo sát thời gian phản hồi (giây) của một trợ lý ảo AI trong $6$ truy vấn liên tiếp: $0.8; 1.0; 1.2; 1.4; 1.6; 3.0$. Khoảng biến thiên $R$ của thời gian phản hồi là],
    (
        True([$2.2$ giây]),
        [$2.0$ giây],
        [$1.8$ giây],
...
#ds([Một nhà máy có hai phân xưởng cùng sản xuất một loại linh kiện điện tử:
...
#ds([Để so sánh mức độ biến động giá giữa hai thị trường bất động sản có quy mô giá cả chênh lệch lớn:
...
#ds([Cho mẫu số liệu điểm thi gồm $4$ số: $2; quad 4; quad 6; quad m$ với $m$ là tham số thực.
...
#ds([Cho mẫu số liệu ban đầu gồm $11$ giá trị không có ngoại lệ:
#align(center)[
...
#tln([Một mẫu số liệu gồm $n = 5$ quan sát có tổng các giá trị $sum_(i=1)^5 x_i = 35$ và tổng các bình phương $sum_(i=1)^5 x_i^2 = 285$.
...
#tln([Cho mẫu số liệu điểm kiểm tra chất lượng đầu vào của $10$ học sinh:
#align(center)[
...
#tln([Cho mẫu số liệu gồm $3$ số: $1; quad 5; quad x$ với $x > 5$.
...
#tln([Một mẫu số liệu có số trung bình $overline(x) = 50$ và độ lệch chuẩn $s = 4$.
...
#tln([Một cuộc khảo sát lương (triệu đồng) của nhân viên có tứ phân vị thứ nhất $Q_1 = 12$ triệu đồng và tứ phân vị thứ ba $Q_3 = 20$ triệu đồng.
...
#tln([Cho mẫu số liệu $x_1, x_2, dots, x_n$ có phương sai $s_x^2 = 9$.
