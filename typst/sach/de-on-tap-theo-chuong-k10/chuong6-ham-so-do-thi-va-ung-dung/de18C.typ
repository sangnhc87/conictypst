...
#show: thpt-school-exam.with(
  ...
  exam-title: "BÀI 17: DẤU CỦA TAM THỨC BẬC HAI (ĐỀ SỐ 18C - NÂNG CAO & VDC)",
  subject: "TOÁN",
...
// TN 1
#tn([Tập xác định của hàm số $y = sqrt((x^2 - x - 6) / (2 - x))$ là],
    (
...
// TN 3
#tn([Tìm tất cả các giá trị của tham số $m$ để bất phương trình $m x^2 - 2(m - 1)x + 4m <= 0$ VÔ NGHIỆM.],
    (
        True([$m > 1/3$]),
        [$m < -1$],
        [$-1 < m < 1/3$],
...
// TN 4
#tn([Cho hàm số bậc hai $f(x) = a x^2 + b x + c$ có đồ thị nằm hoàn toàn phía dưới trục hoành $O x$. Dấu của hệ số $a$ và biệt thức $Delta = b^2 - 4a c$ là],
    (
        True([$a < 0$ và $Delta < 0$]),
        [$a < 0$ và $Delta > 0$],
        [$a > 0$ và $Delta < 0$],
...
// TN 5
#tn([Số nghiệm nguyên của bất phương trình $(x^2 - 16) / (x^2 - 2x - 3) <= 0$ là],
    (
        True([$4$]),
        [$5$],
        [$6$],
...
// TN 6
#tn([Bất phương trình $x^2 - 2|x| - 3 <= 0$ có bao nhiêu nghiệm nguyên?],
    (
        True([$7$]),
        [$6$],
        [$5$],
...
// TN 7
#tn([Tìm tất cả các giá trị của tham số $m$ để phương trình bậc hai $x^2 - 2(m + 1)x + m^2 + 3m - 2 = 0$ có hai nghiệm trái dấu.],
    (
...
// TN 8
#tn([Tập nghiệm của bất phương trình bậc bốn trùng phương $x^4 - 5x^2 + 4 <= 0$ là],
    (
        True([$[-2; -1] union [1; 2]$]),
        [$[1; 4]$],
        [$[-2; 2]$],
...
// TN 9
#tn([Độ bền chịu tải của một thanh dầm bê tông cốt thép phụ thuộc vào khoảng cách đặt lực $x text(" (m)")$ ($2 <= x <= 12$) theo công thức:
...
// TN 10
#tn([Cho hai tam thức bậc hai $f(x) = x^2 - 4x + 3$ và $g(x) = -x^2 + 6x - 5$. Khoảng nghiệm chung của hai bất phương trình $f(x) < 0$ và $g(x) > 0$ là],
    (
        True([$(1; 3)$]),
        [$(1; 5)$],
        [$(3; 5)$],
...
// TN 11
#tn([Tìm tất cả các giá trị của tham số $m$ để phương trình bậc hai $x^2 - 2m x + 2m - 1 = 0$ có hai nghiệm phân biệt đều lớn hơn $1$.],
    (
        True([$m > 1$]),
        [$m < 1$],
        [$m > 2$],
...
// TN 12
#tn([Tìm tập xác định của hàm số $y = 1 / sqrt(x^2 - 2|x| + 1)$ là],
    (
...
#ds([Cho tam thức bậc hai $f(x) = x^2 - 2x - 8$ có đồ thị parabol $(P)$ cắt trục hoành tại hai điểm $A$ và $B$ như sau:
#align(center)[
#cetz.canvas({
  import cetz.draw: *
...
#ds([Cho bất phương trình: $x^2 - 2(m + 1)x + m^2 + 2m >= 0$ (với $m$ là tham số thực).
Xét tính đúng sai của các nhận định sau:],
  (
...
#ds([Một chuỗi cửa hàng trà sữa đang bán với giá $40$ nghìn đồng/ly, mỗi ngày bán được trung bình $500$ ly.
...
#tln([Tìm số nghiệm nguyên của bất phương trình $x^4 - 10x^2 + 9 <= 0$.],
    [6],
    loigiai: [
...
#tln([Tìm giá trị nguyên nhỏ nhất của tham số $m$ để tam thức bậc hai $f(x) = (m + 1)x^2 - 2(m - 1)x + 3m - 3 > 0$ nghiệm đúng với mọi số thực $x$.],
    [2],
    loigiai: [
...
#tln([Độ bền chịu tải an toàn của một thanh dầm cầu bê tông cốt thép tại vị trí $x text(" (m)")$ ($2 <= x <= 12$) được cho bởi:
$ B(x) = -0.5 x^2 + 8x + 100 text(" (kN)") $
...
#tln([Tìm giá trị của tham số $m$ để bất phương trình bậc hai:
...
#tln([Tìm số giá trị nguyên của tham số $m in [-10; 10]$ để phương trình bậc hai:
$ x^2 - 2(m - 1)x + m^2 - 2m = 0 $
