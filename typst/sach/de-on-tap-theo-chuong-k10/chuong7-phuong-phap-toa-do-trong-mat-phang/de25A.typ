...
#show: thpt-school-exam.with(
  ...
  exam-title: "ÔN TẬP TỔNG HỢP CHƯƠNG VII (ĐỀ SỐ 25A - MÃ ĐỀ 167)",
  subject: "TOÁN",
...
// TN 1
#tn([Trong mặt phẳng $O x y$, một vectơ pháp tuyến của đường thẳng $d: 2x - 3y + 5 = 0$ là],
    (
        True([$vec(n) = (2; -3)$]),
        [$vec(n) = (3; 2)$],
        [$vec(n) = (2; 3)$],
...
// TN 2
#tn([Góc tạo bởi hai đường thẳng $d_1: x + 2y - 1 = 0$ và $d_2: 2x - y + 3 = 0$ bằng],
    (
        True([$90^circ$]),
        [$45^circ$],
        [$60^circ$],
...
// TN 3
#tn([Khoảng cách từ điểm $M(1; -2)$ đến đường thẳng $d: 3x - 4y + 4 = 0$ bằng],
    (
        True([$3$]),
        [$5$],
        [$15$],
...
// TN 4
#tn([Tọa độ tâm $I$ và bán kính $R$ của đường tròn $(C): x^2 + y^2 - 4x + 6y - 3 = 0$ là],
    (
        True([$I(2; -3)$ và $R = 4$]),
        [$I(-2; 3)$ và $R = 4$],
        [$I(2; -3)$ và $R = 16$],
...
// TN 5
#tn([Phương trình tiếp tuyến của đường tròn $(C): x^2 + y^2 = 25$ tại điểm $M(3; -4) in (C)$ là],
    (
        True([$3x - 4y - 25 = 0$]),
        [$3x - 4y + 25 = 0$],
        [$4x + 3y = 0$],
...
// TN 6
#tn([Tiêu cự của Elip $(E): x^2 / 16 + y^2 / 7 = 1$ bằng],
    (
        True([$6$]),
        [$3$],
        [$8$],
...
// TN 7
#tn([Phương trình các đường tiệm cận của Hypebol $(H): x^2 / 25 - y^2 / 16 = 1$ là],
    (
...
// TN 8
#tn([Tọa độ tiêu điểm $F$ của Parabol $(P): y^2 = 12x$ là],
    (
        True([$F(3; 0)$]),
        [$F(6; 0)$],
        [$F(-3; 0)$],
...
// TN 9
#tn([Tìm điều kiện của tham số $m$ để đường thẳng $d_1: 2x - y + 1 = 0$ song song với đường thẳng $d_2: 4x - 2y + m = 0$.],
    (
        True([$m != 2$]),
        [$m = 2$],
        [$m != -2$],
...
// TN 10
#tn([Vị trí tương đối của hai đường tròn $(C_1): (x - 1)^2 + y^2 = 4$ và $(C_2): (x - 4)^2 + y^2 = 9$ là],
    (
...
// TN 11
#tn([Một đường cáp treo nối đỉnh núi được mô hình bởi cung Parabol có đỉnh tại $M(0; 10)$ và đi qua hai điểm tháp cáp treo $A(-400; 50)$ và $B(400; 50)$ (đơn vị: mét). Phương trình của đường cáp treo là],
    (
...
// TN 12
#tn([Một vệ tinh địa tĩnh bay quanh Trái Đất theo quỹ đạo tròn có bán kính $R = 42164 text(" km")$.
...
#ds([Trong mặt phẳng tọa độ $O x y$, cho ba điểm $A(1; 3)$, $B(-2; -1)$ và $C(4; -1)$ tạo thành tam giác $A B C$ như hình vẽ:
#align(center)[
#cetz.canvas({
  import cetz.draw: *
...
#ds([Cho đường tròn $(C): (x - 2)^2 + (y - 1)^2 = 25$ và đường thẳng $d: 3x - 4y + m = 0$ (với $m$ là tham số thực).
Xét tính đúng sai của các nhận định sau:],
  (
...
#ds([Cổng chào của một trung tâm triển lãm có dạng một đường Parabol với chiều rộng chân cổng là $12 text(" m")$ và chiều cao tại đỉnh cổng là $9 text(" m")$.
...
#ds([Cho ba đường cong conic: $(E): x^2 / 25 + y^2 / 9 = 1$, $(H): x^2 / 16 - y^2 / 9 = 1$ và $(P): y^2 = 8x$.
Xét tính đúng sai của các nhận định sau:],
  (
...
#tln([Cho tam giác $A B C$ có các đỉnh $A(1; 3)$, $B(-2; -1)$ và $C(4; -1)$. Diện tích của tam giác $A B C$ bằng bao nhiêu?],
    [12],
    loigiai: [
...
#tln([Trong bài toán cổng vòm Parabol rộng $12 text(" m")$ cao $9 text(" m")$, chiều cao của cổng tại vị trí cách tâm chân cổng $4 text(" m")$ bằng bao nhiêu mét?],
    [5],
    loigiai: [
...
#tln([Tìm giá trị dương của tham số $m$ để đường thẳng $3x - 4y + m = 0$ tiếp xúc với đường tròn $(x - 2)^2 + (y - 1)^2 = 25$.],
    [23],
    loigiai: [
...
#tln([Cho Hypebol $(H): x^2 / 16 - y^2 / 9 = 1$. Tiêu cự của Hypebol bằng bao nhiêu?],
    [10],
    loigiai: [
        #step([Tính c và 2c])
        $ c = sqrt(16 + 9) = 5 => 2c = 10 $
...
// TLN 5 (Tiêu cự Elip)
#tln([Cho Elip $(E): x^2 / 16 + y^2 / 7 = 1$. Tiêu cự của Elip bằng bao nhiêu?],
    [6],
    loigiai: [
        #step([Tính c và 2c])
        $ c = sqrt(16 - 7) = 3 => 2c = 6 $
...
#tln([Cho Parabol $(P): y^2 = 12x$. Khoảng cách từ tiêu điểm $F$ đến đường chuẩn $Delta$ của Parabol bằng bao nhiêu?],
    [6],
    loigiai: [
