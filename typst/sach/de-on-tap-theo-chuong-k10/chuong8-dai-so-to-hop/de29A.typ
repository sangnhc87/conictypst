...
#show: thpt-school-exam.with(
  ...
  exam-title: "ÔN TẬP CHƯƠNG VIII: ĐẠI SỐ TỔ HỢP (ĐỀ SỐ 29A - MÃ ĐỀ 182)",
  subject: "TOÁN",
...
// TN 1
#tn([Từ các chữ số thuộc tập hợp $X = {0, 1, 2, 3, 4, 5, 6, 7}$, có thể lập được bao nhiêu số tự nhiên chẵn gồm $4$ chữ số đôi một khác nhau sao cho chữ số hàng nghìn luôn là một số lẻ?],
    (
        True([$480$]),
        [$360$],
        [$420$],
...
#tn([Phương trình $x_1 + x_2 + x_3 = 10$ có bao nhiêu bộ nghiệm nguyên dương $(x_1, x_2, x_3)$?],
    (
        True([$36$]),
        [$45$],
        [$66$],
...
#tn([Cho đa giác đều có $16$ đỉnh nội tiếp trong đường tròn tâm $O$. Có bao nhiêu hình chữ nhật có $4$ đỉnh là các đỉnh của đa giác đều đã cho?],
    (
        True([$28$]),
        [$56$],
        [$16$],
...
#tn([Tìm hệ số của số hạng chứa $x^3$ trong khai triển của biểu thức $P(x) = (1 + 2x)^4 (1 - x)$.],
    (
        True([$8$]),
        [$32$],
        [$24$],
...
#tn([Một robot di chuyển trên mặt phẳng tọa độ từ điểm gốc $O(0, 0)$ đến điểm $B(5, 3)$ bằng các bước sang phải hoặc lên trên $1$ đơn vị. Biết robot không được đi qua điểm chướng ngại vật $M(2, 1)$. Có bao nhiêu lộ trình hợp lệ?],
    (
        True([$26$]),
        [$56$],
        [$30$],
...
#tn([Có $4$ bạn nam và $4$ bạn nữ ngồi quanh một bàn tròn có $8$ chiếc ghế. Có bao nhiêu cách xếp chỗ sao cho nam và nữ ngồi xen kẽ nhau?],
    (
        True([$144$]),
        [$288$],
        [$576$],
...
#tn([Trong khai triển của biểu thức $(root(3, x) + 1 / (sqrt(x)))^5$ với $x > 0$, số hạng không chứa biến $x$ là],
    (
        True([$10$]),
        [$5$],
        [$20$],
...
#tn([Tìm số nguyên dương $n$ thỏa mãn phương trình: $C_n^1 + C_n^2 + C_n^3 = (7n) / 2$.],
    (
        True([$n = 4$]),
        [$n = 5$],
        [$n = 6$],
...
// TN 9 (Tổng hệ số)
#tn([Tổng tất cả các hệ số trong khai triển của đa thức $P(x) = (3 - 2x)^5$ bằng],
    (
        True([$1$]),
        [$3125$],
        [$243$],
...
#tn([Có $7$ học sinh gồm $5$ nam và $2$ nữ xếp thành một hàng dọc. Có bao nhiêu cách xếp chỗ sao cho hai bạn nữ không đứng cạnh nhau?],
    (
        True([$3600$]),
        [$5040$],
        [$1440$],
...
#tn([Cho tập hợp $S$ gồm $6$ phần tử. Số tập hợp con của $S$ có số phần tử là một số chẵn (kể cả tập rỗng) bằng],
    (
        True([$32$]),
        [$64$],
        [$16$],
...
#tn([Hệ số có giá trị lớn nhất trong khai triển nhị thức Newton của biểu thức $(1 + 2x)^5$ bằng],
    (
        True([$80$]),
        [$40$],
        [$32$],
...
#ds([Một robot di chuyển trên lưới từ $O(0, 0)$ đến $B(5, 3)$ bằng các bước sang phải hoặc lên trên. Biết điểm $M(2, 1)$ là chướng ngại vật cấm đi qua.
...
#ds([Cho biểu thức khai triển $Q(x) = (2x - 1 / x^2)^5$ với $x != 0$.
Xét tính đúng sai của các nhận định sau:],
  (
...
#ds([Cho đa giác đều có $16$ đỉnh nội tiếp trong đường tròn tâm $O$.
Xét tính đúng sai của các nhận định sau:],
  (
...
#ds([Xét phương trình $C_n^1 + C_n^2 + C_n^3 = (7n) / 2$ với $n in NN, n >= 3$.
Xét tính đúng sai của các nhận định sau:],
  (
...
// TLN 2
#tln([Một robot di chuyển trên lưới từ $O(0, 0)$ đến $B(5, 3)$ bằng các bước sang phải hoặc lên trên. Biết không được qua $M(2, 1)$, có bao nhiêu lộ trình hợp lệ?],
    [26],
    loigiai: [
        #step([Phần bù])
...
// TLN 3
#tln([Cho đa giác đều có $16$ đỉnh. Có bao nhiêu hình chữ nhật có $4$ đỉnh là các đỉnh của đa giác đều đó?],
    [28],
    loigiai: [
...
// TLN 4
#tln([Tìm số nguyên dương $n$ thỏa mãn phương trình: $C_n^1 + C_n^2 + C_n^3 = (7n) / 2$.],
    [4],
    loigiai: [
...
// TLN 5
#tln([Tìm giá trị lớn nhất của các hệ số trong khai triển nhị thức Newton $(1 + 2x)^5$.],
    [80],
    loigiai: [
        #step([Hệ số lớn nhất])
...
// TLN 6
#tln([Phương trình $x_1 + x_2 + x_3 = 10$ có bao nhiêu bộ nghiệm nguyên dương $(x_1, x_2, x_3)$?],
    [36],
    loigiai: [
        #step([Stars and Bars])
