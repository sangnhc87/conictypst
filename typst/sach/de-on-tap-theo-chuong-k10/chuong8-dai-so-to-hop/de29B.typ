...
#show: thpt-school-exam.with(
  ...
  exam-title: "ÔN TẬP CHƯƠNG VIII: ĐẠI SỐ TỔ HỢP (ĐỀ SỐ 29B - THỰC TIỄN & VẬN DỤNG)",
  subject: "TOÁN",
...
// TN 1 (Mã PIN thẻ ATM)
#tn([Một ngân hàng phát hành mã PIN bảo mật cho thẻ ATM gồm $6$ chữ số $overline(c_1 c_2 c_3 c_4 c_5 c_6)$. Để hạn chế các mã quá dễ đoán, hệ thống quy định chữ số đầu tiên $c_1$ phải khác $0$ và hai chữ số đứng cạnh nhau bất kỳ không được giống nhau ($c_(i + 1) != c_i$ với mọi $i in {1, 2, 3, 4, 5}$). Số mã PIN hợp lệ có thể tạo ra là],
    (
        True([$531441$]),
        [$1000000$],
        [$900000$],
...
#tn([Một hãng hàng không đang khai thác mạng lưới bay kết nối giữa $8$ thành phố lớn. Giữa hai thành phố bất kỳ có tối đa một đường bay thẳng hai chiều nối trực tiếp. Hiện tại hãng đã thiết lập được $20$ đường bay thẳng. Hỏi hãng hàng không đó có thể mở thêm tối đa bao nhiêu đường bay thẳng mới nữa?],
    (
        True([$8$]),
        [$16$],
        [$28$],
...
#tn([Một dây chuyền sản xuất vi mạch có tỉ lệ sản phẩm khuyết tật là $10%$ ($p = 0.1$). Một kỹ sư kiểm định chất lượng chọn ngẫu nhiên độc lập $5$ vi mạch từ dây chuyền. Xác suất để trong $5$ vi mạch lấy ra có đúng $2$ vi mạch bị khuyết tật là],
    (
        True([$0.0729$]),
        [$0.081$],
        [$0.027$],
...
#tn([Một bệnh viện dã chiến nhận được $15$ liều vắc-xin đặc hiệu để phân bổ cho $4$ trạm y tế $A, B, C, D$. Để đảm bảo năng lực cấp cứu tại chỗ, mỗi trạm y tế phải nhận được ít nhất $2$ liều vắc-xin. Có bao nhiêu cách phân bổ các liều vắc-xin này?],
    (
        True([$120$]),
        [$165$],
        [$364$],
...
#tn([Một doanh nghiệp đầu tư khoản vốn $P_0$ với lãi suất $r = 5% = 0.05$/năm theo hình thức lãi kép hàng năm. Sau $4$ năm, tổng giá trị vốn và lãi là $P = P_0 (1 + 0.05)^4$. Sử dụng công thức nhị thức Newton, giá trị chính xác của hệ số tăng trưởng $(1 + 0.05)^4$ bằng],
    (
        True([$1.21550625$]),
        [$1.20000000$],
        [$1.21550000$],
...
// TN 6 (Lập đội dự án)
#tn([Một công ty công nghệ có $10$ kỹ sư phần mềm và $6$ chuyên viên thiết kế đồ họa. Giám đốc muốn thành lập một đội dự án gồm $5$ thành viên sao cho trong đội có ít nhất $3$ kỹ sư phần mềm và có ít nhất $1$ chuyên viên đồ họa. Số cách thành lập đội dự án là],
    (
        True([$3060$]),
        [$4368$],
        [$1800$],
...
#tn([Một công ty vận tải gán mã vận đơn gồm $5$ chữ số $overline(c_1 c_2 c_3 c_4 c_5)$ cho các gói hàng. Chữ số đầu $c_1 != 0$, các chữ số $c_2, c_3, c_4 in {0, 1, ..., 9}$. Chữ số cuối $c_5$ là chữ số kiểm tra (checksum) được xác định duy nhất bởi điều kiện $(c_1 + 2c_2 + 3c_3 + 4c_4 + c_5)$ chia hết cho $10$. Hỏi có bao nhiêu mã vận đơn hợp lệ?],
    (
        True([$9000$]),
        [$10000$],
        [$8100$],
...
#tn([Một khu đô thị mới được quy hoạch gồm $10$ giao lộ tạo thành các đỉnh của một đa giác lồi. Người ta muốn thiết kế các đoạn đường thẳng nối từng cặp giao lộ với nhau (gồm cả đường bao quanh và đường nội bộ xuyên tâm). Hỏi có tất cả bao nhiêu đoạn đường thẳng nối các cặp giao lộ?],
    (
        True([$45$]),
        [$35$],
        [$90$],
...
#tn([Có $5$ bạn học sinh trực nhật trong $5$ ngày từ thứ Hai đến thứ Sáu, mỗi ngày đúng một bạn. Biết rằng bạn An không thể trực nhật vào ngày thứ Hai và ngày thứ Sáu. Số cách phân công lịch trực nhật là],
    (
        True([$72$]),
        [$120$],
        [$96$],
...
#tn([Theo định luật phân li độc lập của Mendel, khi lai hai cây đậu Hà Lan dị hợp $A a times A a$, xác suất sinh hạt vàng ($A-$) là $3 / 4$ và hạt xanh ($a a$) là $1 / 4$. Thu hoạch ngẫu nhiên $4$ hạt đậu từ phép lai. Xác suất để có đúng $3$ hạt đậu màu vàng là],
    (
        True([$27 / 64$]),
        [$81 / 256$],
        [$27 / 128$],
...
#tn([Một mật khẩu máy tính gồm $8$ ký tự, trong đó có $6$ chữ cái in hoa (chọn từ $26$ chữ cái tiếng Anh) và $2$ chữ số (chọn từ $0$ đến $9$). Có bao nhiêu mật khẩu thỏa mãn nếu hai chữ số bắt buộc phải đứng cạnh nhau? (Các chữ cái và chữ số được phép lặp lại).],
    (
...
#tn([Hệ số của số hạng chứa $x^2$ trong khai triển nhị thức Newton của biểu thức $(2 - 3x)^5$ bằng],
    (
        True([$720$]),
        [$-720$],
        [$1080$],
...
// DS 1 (Đội ngũ dự án)
#ds([Một công ty công nghệ có $10$ kỹ sư phần mềm và $6$ chuyên viên thiết kế đồ họa. Cần chọn một nhóm công tác gồm $5$ người.
...
#ds([Xét mạng lưới giao thông kết nối giữa $8$ thành phố phân biệt.
Xét tính đúng sai của các nhận định sau:],
  (
...
#ds([Xét bài toán phân phối $15$ liều vắc-xin cho $4$ trạm y tế $A, B, C, D$.
Xét tính đúng sai của các nhận định sau:],
  (
...
#ds([Xét công thức lãi kép tích lũy bậc $4$: $f(r) = (1 + r)^4$ với $r = 0.05$.
Xét tính đúng sai của các nhận định sau:],
  (
...
// TLN 1
#tln([Một công ty có $10$ kỹ sư và $6$ chuyên viên thiết kế. Có bao nhiêu cách lập đội gồm $5$ người có ít nhất $3$ kỹ sư và ít nhất $1$ chuyên viên thiết kế?],
    [3060],
    loigiai: [
...
// TLN 2
#tln([Giữa $8$ thành phố hiện có $20$ đường bay thẳng trực tiếp. Hãng hàng không có thể mở thêm tối đa bao nhiêu đường bay thẳng mới nữa?],
    [8],
    loigiai: [
        #step([Lấy hiệu số])
...
// TLN 3
#tln([Có bao nhiêu cách phân bổ $15$ liều vắc-xin cho $4$ trạm y tế sao cho mỗi trạm nhận được ít nhất $2$ liều?],
    [120],
    loigiai: [
...
// TLN 4
#tln([Tìm hệ số của số hạng chứa $x^2$ trong khai triển của biểu thức $(2 - 3x)^5$.],
    [720],
    loigiai: [
...
// TLN 5
#tln([Có $5$ bạn học sinh trực nhật trong $5$ ngày từ thứ Hai đến thứ Sáu. Bạn An không trực thứ Hai và thứ Sáu. Có bao nhiêu cách phân công?],
    [72],
    loigiai: [
        #step([Quy tắc nhân])
...
// TLN 6
#tln([Có bao nhiêu mã PIN gồm $6$ chữ số có chữ số đầu khác $0$ và hai chữ số đứng cạnh nhau bất kỳ không giống nhau?],
    [531441],
    loigiai: [
        #step([Lũy thừa 9^6])
