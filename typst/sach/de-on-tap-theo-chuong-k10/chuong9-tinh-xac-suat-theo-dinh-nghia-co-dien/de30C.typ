...
#show: thpt-school-exam.with(
  ...
  exam-title: "BÀI 26: BIẾN CỐ VÀ ĐỊNH NGHĨA CỔ ĐIỂN CỦA XÁC SUẤT (ĐỀ SỐ 30C - NÂNG CAO & VDC)",
  subject: "TOÁN",
...
#tn([Cho tập hợp các số tự nhiên $S = {1, 2, 3, ..., 15}$. Chọn ngẫu nhiên đồng thời $3$ số từ tập $S$. Xác suất để tổng của $3$ số được chọn là một số chia hết cho $3$ bằng],
    (
        True([$31 / 91$]),
        [$1 / 3$],
        [$30 / 91$],
...
#tn([Cho đa giác đều có $18$ đỉnh nội tiếp trong một đường tròn. Chọn ngẫu nhiên $3$ đỉnh từ $18$ đỉnh của đa giác để tạo thành một tam giác. Xác suất để tam giác được tạo thành là tam giác nhọn bằng],
    (
        True([$7 / 34$]),
        [$1 / 4$],
        [$3 / 17$],
...
#tn([Xếp ngẫu nhiên $7$ bạn nam và $3$ bạn nữ thành một hàng dọc gồm $10$ học sinh. Xác suất để không có bất kỳ hai bạn nữ nào đứng cạnh nhau bằng],
    (
        True([$7 / 15$]),
        [$1 / 3$],
        [$8 / 15$],
...
#tn([Từ tập hợp các chữ số $X = {1, 2, 3, 4, 5, 6, 7, 8, 9}$, chọn ngẫu nhiên một số tự nhiên gồm $4$ chữ số đôi một khác nhau. Xác suất để số được chọn có các chữ số theo thứ tự tăng dần từ trái sang phải bằng],
    (
        True([$1 / 24$]),
        [$1 / 12$],
        [$1 / 6$],
...
#tn([Tám người bạn (trong đó có An và Bình) ngồi ngẫu nhiên quanh một chiếc bàn tròn gồm $8$ ghế cách đều nhau. Xác suất để An và Bình ngồi ở hai vị trí đối diện nhau qua tâm bàn tròn bằng],
    (
        True([$1 / 7$]),
        [$1 / 8$],
        [$2 / 7$],
...
#tn([Một hạt chuyển động trên mặt phẳng tọa độ bắt đầu từ gốc $O(0, 0)$. Tại mỗi bước, hạt di chuyển sang phải một đơn vị ($(x + 1, y)$) hoặc lên trên một đơn vị ($(x, y + 1)$) với xác suất bằng nhau là $1 / 2$. Hạt thực hiện tổng cộng $6$ bước di chuyển độc lập. Xác suất để sau $6$ bước, hạt dừng lại tại điểm $M(4, 2)$ bằng],
    (
        True([$15 / 64$]),
        [$5 / 16$],
        [$3 / 32$],
...
#tn([Từ tập hợp $A = {1, 2, 3, ..., 20}$, chọn ngẫu nhiên đồng thời $4$ số. Xác suất để trong $4$ số được chọn không có bất kỳ hai số nào là hai số tự nhiên liên tiếp bằng],
    (
        True([$28 / 57$]),
        [$1 / 2$],
        [$29 / 57$],
...
#tn([Có $9$ chiếc bút chì giống hệt nhau được chia ngẫu nhiên cho $3$ bạn An, Bình, Cường sao cho bạn nào cũng nhận được ít nhất một chiếc bút chì. Xác suất để bạn An nhận được nhiều bút chì nhất (nhiều hơn hẳn số bút của cả Bình và Cường) bằng],
    (
        True([$9 / 28$]),
        [$1 / 3$],
        [$5 / 14$],
...
#tn([Chọn ngẫu nhiên một bộ ba nghiệm nguyên không âm $(x, y, z)$ của phương trình $x + y + z = 10$. Xác suất để bộ nghiệm được chọn thỏa mãn điều kiện $x >= 2$, $y >= 2$ và $z >= 2$ bằng],
    (
        True([$5 / 22$]),
        [$1 / 4$],
        [$3 / 22$],
...
#tn([Từ tập hợp ${1, 2, 3, 4, 5, 6, 7}$, chọn ngẫu nhiên một số tự nhiên gồm $3$ chữ số đôi một khác nhau. Xác suất để tích các chữ số của số được chọn là một số chia hết cho $10$ bằng],
    (
        True([$18 / 35$]),
        [$1 / 2$],
        [$17 / 35$],
...
#tn([Gieo đồng thời hai con xúc xắc cân đối và đồng chất. Xác suất để tổng số chấm xuất hiện trên hai con xúc xắc là một số nguyên tố bằng],
    (
        True([$5 / 12$]),
        [$1 / 2$],
        [$7 / 18$],
...
#ds([Bốn bạn An, Bình, Cường, Dũng chuẩn bị $4$ món quà khác nhau và rút ngẫu nhiên mỗi người một món.
...
#ds([Cho tập hợp $S = {1, 2, 3, ..., 15}$. Chọn ngẫu nhiên đồng thời $3$ số từ tập $S$.
Xét tính đúng sai của các nhận định sau:],
  (
...
#ds([Cho đa giác đều $18$ đỉnh nội tiếp đường tròn. Chọn ngẫu nhiên $3$ đỉnh lập thành tam giác.
Xét tính đúng sai của các nhận định sau:],
  (
...
#ds([Xét phương trình $x + y + z = 10$ với các ẩn $x, y, z in NN$.
Xét tính đúng sai của các nhận định sau:],
  (
...
// TLN 1
#tln([Bốn người rút ngẫu nhiên quà giáng sinh. Tính xác suất để không ai nhận đúng quà của mình (dưới dạng phân số tối giản).],
    [3/8],
    loigiai: [
        #step([Derangement D4])
        $ D_4 / 24 = 9 / 24 = 3 / 8 $
...
// TLN 2
#tln([Từ tập ${1, 2, ..., 15}$, chọn ngẫu nhiên $3$ số. Tính xác suất để tổng $3$ số chia hết cho $3$ (dưới dạng phân số tối giản).],
    [31/91],
    loigiai: [
        #step([Tính xác suất])
        $ 155 / 455 = 31 / 91 $
...
// TLN 3
#tln([Chọn ngẫu nhiên $3$ đỉnh từ $18$ đỉnh của đa giác đều. Tính xác suất để tam giác tạo thành là tam giác nhọn (dưới dạng phân số tối giản).],
    [7/34],
    loigiai: [
        #step([Tính xác suất])
        $ 168 / 816 = 7 / 34 $
...
// TLN 4
#tln([Xếp ngẫu nhiên $7$ nam và $3$ nữ thành hàng dọc. Tính xác suất để không có hai nữ nào đứng cạnh nhau (dưới dạng phân số tối giản).],
    [7/15],
    loigiai: [
        #step([Vách ngăn])
...
// TLN 5
#tln([Tám người ngồi quanh bàn tròn (có An và Bình). Tính xác suất để An và Bình ngồi đối diện nhau (dưới dạng phân số tối giản).],
    [1/7],
    loigiai: [
        #step([Tính xác suất])
        $ 1 / 7 $
...
// TLN 6
#tln([Gieo hai con xúc xắc cân đối. Tính xác suất để tổng số chấm xuất hiện là số nguyên tố (dưới dạng phân số tối giản).],
    [5/12],
    loigiai: [
        #step([Tính xác suất])
        $ 15 / 36 = 5 / 12 $
