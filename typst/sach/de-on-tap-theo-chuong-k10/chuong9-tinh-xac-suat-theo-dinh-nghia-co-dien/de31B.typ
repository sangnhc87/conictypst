...
#show: thpt-school-exam.with(
  ...
  exam-title: "BÀI 27: THỰC HÀNH TÍNH XÁC SUẤT CỔ ĐIỂN (ĐỀ SỐ 31B - THỰC TIỄN & VẬN DỤNG)",
  subject: "TOÁN",
...
#tn([Một lô vi mạch gồm $50$ sản phẩm, trong đó có $45$ vi mạch đạt chuẩn và $5$ vi mạch có sai sót kỹ thuật. Kỹ thuật viên kiểm tra chất lượng chọn ngẫu nhiên một mẫu gồm $4$ vi mạch. Xác suất để trong mẫu kiểm tra có đúng $1$ vi mạch có sai sót kỹ thuật bằng],
    (
        True([$1419 / 4606$]),
        [$1 / 10$],
        [$1419 / 23030$],
...
#tn([Một công ty vận tải có $4$ xe tải hoạt động độc lập trong năm. Xác suất gặp tai nạn phải bồi thường bảo hiểm trong năm của mỗi xe là $0.05$. Xác suất để công ty bảo hiểm chỉ phải bồi thường cho đúng một chiếc xe trong năm đó xấp xỉ bằng],
    (
        True([$0.1715$]),
        [$0.2000$],
        [$0.0500$],
...
#tn([Một tòa nhà cao tầng trang bị $2$ máy bơm cứu hỏa hoạt động hoàn toàn độc lập khi có hỏa hoạn. Xác suất để máy bơm 1 khởi động thành công là $0.95$, và của máy bơm 2 là $0.90$. Hệ thống chữa cháy hoạt động an toàn nếu có ít nhất một máy bơm khởi động thành công. Xác suất để hệ thống chữa cháy hoạt động an toàn bằng],
    (
        True([$0.995$]),
        [$0.855$],
        [$0.925$],
...
#tn([Giải quần vợt có $16$ tay vợt tham gia (trong đó có $4$ tay vợt hạt giống hàng đầu thế giới). Ban tổ chức bốc thăm chia ngẫu nhiên $16$ tay vợt vào $4$ nhánh đấu độc lập $A, B, C, D$ (mỗi nhánh gồm $4$ tay vợt). Xác suất để $4$ tay vợt hạt giống rơi vào $4$ nhánh đấu khác nhau bằng],
    (
        True([$64 / 455$]),
        [$1 / 4$],
        [$16 / 455$],
...
#tn([Trong trò chơi xổ số tự chọn Mega 6/45, người chơi chọn $6$ số từ tập hợp ${1, 2, ..., 45}$. Giải Nhì được trao cho vé trùng đúng $5$ số trong $6$ số của kết quả quay thưởng. Xác suất để một người chơi mua một vé ngẫu nhiên trúng giải Nhì bằng],
    (
        True([$39 / 1357510$]),
        [$1 / 8145060$],
        [$6 / 8145060$],
...
#tn([Một hội đồng phỏng vấn gồm $5$ chuyên gia (trong đó có $3$ nam và $2$ nữ). Ban giám khảo bốc thăm chọn ngẫu nhiên $3$ chuyên gia để trực tiếp phỏng vấn ứng viên. Xác suất để nhóm chuyên gia được chọn có cả nam và nữ bằng],
    (
        True([$9 / 10$]),
        [$1 / 10$],
        [$3 / 5$],
...
#tn([Trong một phòng làm việc có $10$ nhân viên, trong đó có $2$ người đang mang mầm bệnh cúm mùa. Quản lý chọn ngẫu nhiên $3$ nhân viên đi công tác bên ngoài. Xác suất để trong $3$ người được chọn có đúng một người mang mầm bệnh bằng],
    (
        True([$7 / 15$]),
        [$8 / 15$],
        [$1 / 5$],
...
#tn([Một dãy bãi đỗ xe gồm $6$ vị trí đỗ xe liên tiếp thẳng hàng đang để trống. Có $2$ chiếc ô tô lần lượt vào đỗ ngẫu nhiên vào hai vị trí khác nhau. Xác suất để hai chiếc xe đỗ ở hai vị trí cạnh nhau bằng],
    (
        True([$1 / 3$]),
        [$1 / 5$],
        [$2 / 5$],
...
#tn([Một ngân hàng máu tiếp nhận $8$ đơn vị máu gồm $3$ đơn vị nhóm O, $3$ đơn vị nhóm A và $2$ đơn vị nhóm B. Bác sĩ lấy ngẫu nhiên $2$ đơn vị máu để cấp cứu khẩn cấp. Xác suất để lấy được $2$ đơn vị máu cùng nhóm máu bằng],
    (
        True([$1 / 4$]),
        [$3 / 14$],
        [$5 / 28$],
...
#tn([Ba người bạn độc lập cùng sinh vào tháng 12 (tháng 12 có đúng $31$ ngày). Xác suất để có ít nhất hai người có cùng ngày sinh nhật xấp xỉ bằng],
    (
        True([$0.0951$]),
        [$0.0323$],
        [$0.9049$],
...
#tn([Một thông điệp số được gửi đồng thời qua $3$ kênh truyền thông tin song song độc lập. Xác suất truyền thành công của các kênh lần lượt là $0.80$; $0.70$ và $0.60$. Thông điệp được coi là nhận thành công nếu có ít nhất một kênh truyền thành công. Xác suất để thông điệp được nhận thành công bằng],
    (
        True([$0.976$]),
        [$0.336$],
        [$0.024$],
...
#tn([Một hộp bánh kẹo có $6$ cái kẹo sô-cô-la và $4$ cái kẹo dâu. Một em bé lấy ngẫu nhiên $2$ cái kẹo. Xác suất để em bé lấy được ít nhất một cái kẹo sô-cô-la bằng],
    (
        True([$13 / 15$]),
        [$2 / 15$],
        [$3 / 5$],
...
#ds([Xét bài toán bốc thăm $16$ tay vợt (có $4$ tay vợt hạt giống) vào $4$ nhánh đấu $A, B, C, D$ (mỗi nhánh $4$ tay vợt).
...
#ds([Xét hệ thống $2$ máy bơm cứu hỏa độc lập có xác suất hoạt động tốt lần lượt là $p_1 = 0.95$ và $p_2 = 0.90$.
Xét tính đúng sai của các nhận định sau:],
  (
...
#ds([Xét trò chơi xổ số tự chọn Mega 6/45 với kết quả quay thưởng gồm $6$ số may mắn.
Xét tính đúng sai của các nhận định sau:],
  (
...
// DS 4 (Bãi đỗ xe)
#ds([Xét $6$ vị trí đỗ xe thẳng hàng liên tiếp. Hai xe ô tô đỗ ngẫu nhiên vào hai vị trí khác nhau.
Xét tính đúng sai của các nhận định sau:],
  (
...
// TLN 1
#tln([Kiểm tra $4$ vi mạch từ $50$ chiếc (có $5$ lỗi). Tính xác suất có đúng $1$ vi mạch lỗi (dưới dạng phân số tối giản).],
    [1419/4606],
    loigiai: [
        #step([Tính xác suất])
...
// TLN 2
#tln([Hai máy bơm cứu hỏa độc lập có xác suất hoạt động tốt là $0.95$ và $0.90$. Tính xác suất hệ thống an toàn (có ít nhất một máy hoạt động tốt).],
    [0.995],
    loigiai: [
        #step([Biến cố đối])
        $ 1 - (0.05)(0.10) = 0.995 $
...
// TLN 3
#tln([Bốc thăm $16$ tay vợt (có $4$ hạt giống) vào $4$ nhánh đấu. Tính xác suất để $4$ hạt giống ở $4$ nhánh khác nhau (dưới dạng phân số tối giản).],
    [64/455],
    loigiai: [
        #step([Tính xác suất])
        $ 256 / 1820 = 64 / 455 $
...
// TLN 4
#tln([Chọn $3$ chuyên gia từ $5$ người (gồm $3$ nam, $2$ nữ). Tính xác suất để nhóm có cả nam và nữ (dưới dạng phân số tối giản).],
    [9/10],
    loigiai: [
        #step([Biến cố đối])
        $ 1 - 1 / 10 = 9 / 10 $
...
// TLN 5
#tln([Trong phòng có $10$ người (có $2$ người mang mầm bệnh). Chọn ngẫu nhiên $3$ người. Tính xác suất có đúng $1$ người mang mầm bệnh (dưới dạng phân số tối giản).],
    [7/15],
    loigiai: [
        #step([Tính xác suất])
        $ 56 / 120 = 7 / 15 $
...
// TLN 6
#tln([Ba kênh truyền song song độc lập có xác suất thành công là $0.80$; $0.70$; $0.60$. Tính xác suất để có ít nhất một kênh truyền thành công.],
    [0.976],
    loigiai: [
        #step([Biến cố đối])
        $ 1 - (0.2)(0.3)(0.4) = 0.976 $
