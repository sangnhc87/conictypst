...
#show: thpt-school-exam.with(
  ...
  exam-title: "ÔN TẬP CUỐI CHƯƠNG IX: TÍNH XÁC SUẤT CỔ ĐIỂN (ĐỀ SỐ 32C - NÂNG CAO & VDC)",
  subject: "TOÁN",
...
#tn([Cho đa giác đều gồm $20$ đỉnh nội tiếp trong một đường tròn tâm $O$. Chọn ngẫu nhiên $3$ đỉnh từ $20$ đỉnh của đa giác để tạo thành một tam giác. Xác suất để tam giác được tạo thành là một tam giác nhọn (chứa tâm $O$ ở miền trong tam giác) bằng],
    (
        True([$4 / 19$]),
        [$3 / 19$],
        [$5 / 19$],
...
#tn([Từ tập hợp các số tự nhiên $A = {1, 2, 3, ..., 16}$, chọn ngẫu nhiên đồng thời $3$ số. Xác suất để $3$ số được chọn có thể sắp xếp thành một cấp số cộng bằng],
    (
        True([$1 / 10$]),
        [$1 / 20$],
        [$3 / 20$],
...
#tn([Mười người bạn (trong đó có An và Bình) ngồi ngẫu nhiên quanh một chiếc bàn tròn gồm $10$ ghế cách đều nhau. Xác suất để hai bạn An và Bình không ngồi cạnh nhau trên bàn tròn bằng],
    (
        True([$7 / 9$]),
        [$2 / 9$],
        [$4 / 5$],
...
#tn([Chia ngẫu nhiên $8$ học sinh (trong đó có An và Bình) vào $4$ phòng thi độc lập, mỗi phòng thi có đúng $2$ chỗ ngồi. Xác suất để hai bạn An và Bình được xếp vào cùng một phòng thi bằng],
    (
        True([$1 / 7$]),
        [$1 / 4$],
        [$2 / 7$],
...
#tn([Chọn ngẫu nhiên một bộ ba nghiệm nguyên không âm $(x_1, x_2, x_3)$ của phương trình $x_1 + x_2 + x_3 = 9$. Xác suất để bộ nghiệm được chọn thỏa mãn điều kiện $x_1 <= 4$ bằng],
    (
        True([$8 / 11$]),
        [$3 / 11$],
        [$1 / 2$],
...
#tn([Năm bạn học sinh nộp vở bài tập cho thầy giáo. Thầy giáo phát ngẫu nhiên mỗi bạn một cuốn vở từ $5$ cuốn vở đó. Xác suất để có đúng một bạn nhận đúng cuốn vở của chính mình bằng],
    (
        True([$3 / 8$]),
        [$1 / 5$],
        [$9 / 24$],
...
#tn([Một con kiến di chuyển trên lưới ô vuông từ điểm $O(0, 0)$ đến điểm $B(4, 4)$ bằng các bước đi sang phải một đơn vị hoặc đi lên một đơn vị. Biết điểm $A(2, 2)$ có bẫy dính nên kiến không được đi qua. Giả sử các lộ trình từ $O$ đến $B$ là đồng khả năng. Xác suất để kiến chọn được một lộ trình an toàn từ $O$ đến $B$ bằng],
    (
        True([$17 / 35$]),
        [$18 / 35$],
        [$1 / 2$],
...
#tn([Từ tập hợp $S = {1, 2, 3, ..., 30}$, chọn ngẫu nhiên đồng thời $3$ số. Xác suất để tổng của $3$ số được chọn là một số chia hết cho $3$ bằng],
    (
        True([$68 / 203$]),
        [$1 / 3$],
        [$67 / 203$],
...
#tn([Gieo đồng thời ba con xúc xắc cân đối và đồng chất. Xác suất để tổng số chấm xuất hiện trên ba con xúc xắc bằng $9$ bằng],
    (
        True([$25 / 216$]),
        [$1 / 8$],
        [$27 / 216$],
...
#tn([Cho một bát giác đều có $8$ đỉnh. Chọn ngẫu nhiên $2$ đường chéo của bát giác. Xác suất để hai đường chéo được chọn cắt nhau tại một điểm nằm ở miền trong của bát giác bằng],
    (
        True([$7 / 19$]),
        [$5 / 19$],
        [$1 / 2$],
...
#tn([Xếp ngẫu nhiên $6$ bạn nam và $4$ bạn nữ ngồi quanh một chiếc bàn tròn gồm $10$ ghế cách đều nhau. Xác suất để không có bất kỳ hai bạn nữ nào ngồi cạnh nhau trên bàn tròn bằng],
    (
        True([$5 / 42$]),
        [$1 / 14$],
        [$1 / 7$],
...
#tn([Gieo đồng thời hai con xúc xắc cân đối và đồng chất. Xác suất để tổng số chấm xuất hiện trên hai con xúc xắc là một bội số của $4$ bằng],
    (
        True([$1 / 4$]),
        [$7 / 36$],
        [$2 / 9$],
...
#ds([Cho đa giác đều gồm $20$ đỉnh nội tiếp trong đường tròn tâm $O$. Chọn ngẫu nhiên $3$ đỉnh lập thành tam giác.
...
#ds([Xếp ngẫu nhiên $10$ người (trong đó có An và Bình) ngồi quanh một bàn tròn gồm $10$ ghế.
Xét tính đúng sai của các nhận định sau:],
  (
...
#ds([Xét phương trình $x_1 + x_2 + x_3 = 9$ với $x_1, x_2, x_3 in NN$.
Xét tính đúng sai của các nhận định sau:],
  (
...
#ds([Xét bát giác đều $8$ đỉnh và tập hợp tất cả các đường chéo của nó.
Xét tính đúng sai của các nhận định sau:],
  (
...
// TLN 1
#tln([Từ $20$ đỉnh của đa giác đều, có bao nhiêu tam giác nhọn được tạo thành?],
    [240],
    loigiai: [
...
// TLN 2
#tln([Từ tập ${1, 2, ..., 16}$, chọn ngẫu nhiên $3$ số. Tính xác suất để $3$ số lập thành cấp số cộng (dưới dạng phân số tối giản).],
    [1/10],
    loigiai: [
        #step([Tính xác suất])
        $ 56 / 560 = 1 / 10 $
...
// TLN 3
#tln([Xếp $10$ người quanh bàn tròn. Tính xác suất để An và Bình không ngồi cạnh nhau (dưới dạng phân số tối giản).],
    [7/9],
    loigiai: [
        #step([Tính xác suất])
        $ 7 / 9 $
...
// TLN 4
#tln([Chia $8$ người vào $4$ phòng thi (mỗi phòng $2$ người). Tính xác suất để An và Bình ở cùng một phòng (dưới dạng phân số tối giản).],
    [1/7],
    loigiai: [
        #step([Tính xác suất])
        $ 1 / 7 $
...
// TLN 5
#tln([Đổi ngẫu nhiên vở kiểm tra của $5$ bạn. Tính xác suất có đúng một bạn nhận đúng vở của mình (dưới dạng phân số tối giản).],
    [3/8],
    loigiai: [
        #step([Derangement])
...
// TLN 6
#tln([Xếp $6$ nam và $4$ nữ quanh bàn tròn. Tính xác suất để không có hai nữ nào ngồi cạnh nhau (dưới dạng phân số tối giản).],
    [5/42],
    loigiai: [
        #step([Tính xác suất])
