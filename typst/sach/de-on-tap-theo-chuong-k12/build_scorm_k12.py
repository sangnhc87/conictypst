#!/usr/bin/env python3
"""
build_scorm_k12.py — Chuyển đổi toàn bộ đề Toán Khối 12 sang chuẩn SCORM 1.2 cho K12Online.
Tối ưu hóa đặc thù Khối 12:
...
# Cấu hình đường dẫn Khối 12
K12_ROOT = Path('/Users/admin/conictypst/typst/sach/de-on-tap-theo-chuong-k12')
OUTPUT_DIR = K12_ROOT / "scorm_output"
TMP_SVG_DIR = K12_ROOT / ".tmp_svg"
SVG_CACHE_DIR = K12_ROOT / ".svg_cache"
...
def process_single_exam(typ_file_path: str) -> dict:
    ...
    if m_title:
        ...
        title = f"Toán 12 - {folder_name.replace('-', ' ').title()} - {de_id.upper()}"
    ...
    # Đóng gói ZIP
    zip_name = f"scorm-k12-{folder_name}-{de_id}.zip"
...
# ─── MAIN RUNNER ─────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="Build SCORM 1.2 packages for K12")
    ...
    all_typ = sorted(glob.glob(str(K12_ROOT / "**" / "*.typ"), recursive=True))
    real_files = []
    seen = set()
    ...
    print(f"🚀 Bắt đầu chuyển đổi SCORM Khối 12: {len(real_files)} đề thi...")
    ...
    print(f"\n🎉 HOÀN TẤT CHUYỂN ĐỔI SCORM KHỐI 12!")
