...
// ═══════════════════════════════════════════════════════════
#let mode = "loigiai"
#let accent = rgb("1e40af") // Royal Blue
...
#show: thpt-school-exam.with(
  ...
  exam-title: "CHỦ ĐỀ 1A: TỐI ƯU HÓA HÌNH HỌC 2D & UỐN DÂY TẠO HÌNH (VD - VDC)",
  subject: "TOÁN",
...
// TN 1
#tn([Một người nông dân muốn rào một khu đất hình chữ nhật giáp bờ sông thẳng để trồng rau. Do giáp bờ sông nên không cần rào cạnh phía bờ sông. Người đó có $80$ m lưới thép B40. Diện tích lớn nhất của mảnh vườn mà người đó có thể rào được bằng bao nhiêu?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 2
#tn([Một tấm thép hình tam giác vuông cân $A B C$ có cạnh huyền $B C = 12$ cm. Người thợ cắt từ tấm thép đó ra một hình chữ nhật $M N P Q$ có cạnh $M N$ nằm trên cạnh huyền $B C$, hai đỉnh $P, Q$ lần lượt nằm trên hai cạnh góc vuông $A C, A B$. Diện tích lớn nhất của hình chữ nhật $M N P Q$ cắt được là
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 3
#tn([Cắt một đoạn dây thép dài $20$ m thành hai phần để uốn thành một hình vuông và một hình tròn. Để tổng diện tích của hình vuông và hình tròn nhận được là nhỏ nhất thì cạnh của hình vuông phải bằng bao nhiêu mét?],
    (
        True([$20 / (4 + pi)$ m]),
        [$10 / (4 + pi)$ m],
        [$40 / (4 + pi)$ m],
...
// TN 4
#tn([Một khung cửa sổ có dạng hình chữ nhật được gắn thêm phía trên một nửa hình tròn (cửa sổ vòm Norman). Chu vi toàn bộ của khung cửa sổ là $p = 10$ m. Để diện tích của cửa sổ lớn nhất (thu được nhiều ánh sáng nhất) thì bán kính của nửa hình tròn phía trên bằng bao nhiêu?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 5
#tn([Cho một nửa đường tròn đường kính $A B = 2 R$. Một hình thang cân $A B C D$ nội tiếp trong nửa đường tròn đó có đáy lớn là $A B$, đáy nhỏ $C D$. Diện tích lớn nhất của hình thang cân $A B C D$ bằng bao nhiêu?],
    (
        True([$(3 sqrt(3)) / 4 R^2$]),
        [$sqrt(3) R^2$],
        [$(3 sqrt(3)) / 2 R^2$],
...
// TN 7
#tn([Tìm diện tích lớn nhất $S_"max"$ của một hình chữ nhật nội tiếp trong elip $(E): x^2/16 + y^2/9 = 1$, biết các cạnh của hình chữ nhật song song với các trục tọa độ.],
    (
        True([$24$]),
        [$12$],
        [$48$],
...
// TN 8
#tn([Cho một miền phẳng giới hạn bởi parabol $(P): y = 9 - x^2$ với $y >= 0$ và trục hoành $O x$. Một hình chữ nhật có hai đỉnh nằm trên trục hoành và hai đỉnh còn lại nằm trên parabol $(P)$. Diện tích lớn nhất của hình chữ nhật đó bằng bao nhiêu?],
    (
        True([$12 sqrt(3)$]),
        [$16$],
        [$18$],
...
// TN 9
#tn([Một sợi dây kim loại dài $36$ cm được chia làm hai phần. Phần thứ nhất uốn thành một tam giác đều, phần thứ hai uốn thành một hình vuông. Để tổng diện tích của hai hình này đạt giá trị nhỏ nhất thì chu vi của tam giác đều phải bằng bao nhiêu cm?],
    (
...
// TN 10
#tn([Một trang sách có diện tích phần in chữ là $384 "cm"^2$. Lề trên và lề dưới đều rộng $3$ cm, lề trái và lề phải đều rộng $2$ cm. Kích thước trang sách (chiều rộng $times$ chiều cao) để diện tích của cả trang sách là nhỏ nhất (tiết kiệm giấy nhất) là],
    (
...
// TN 12
#tn([Trong một phòng hòa nhạc hình chữ nhật có chiều dài $20$ m và chiều rộng $12$ m, người ta muốn dựng một sân khấu hình tam giác vuông $A M N$ ở một góc phòng sao cho đỉnh góc vuông là góc phòng $A$, hai đỉnh nhọn $M, N$ lần lượt nằm trên hai bức tường xuất phát từ $A$. Do yêu cầu kỹ thuật âm thanh, cạnh huyền $M N$ phải luôn đi qua một vị trí cố định $K$ cách hai bức tường lần lượt là $1$ m và $8$ m. Diện tích nhỏ nhất của sân khấu tam giác $A M N$ bằng bao nhiêu?],
    (
        True([$16 "m"^2$]),
        [$18 "m"^2$],
        [$24 "m"^2$],
...
// DS 2
#ds([Một mảnh vườn hình tam giác $A B C$ có đáy $B C = 60$ m và chiều cao tương ứng $A H = 40$ m. Chủ nhà muốn ngăn ra một khu đất hình chữ nhật $M N P Q$ bên trong mảnh vườn để xây dựng nhà kính trồng hoa công nghệ cao, với cạnh $P Q$ nằm trên cạnh đáy $B C$, hai đỉnh $M, N$ lần lượt nằm trên hai cạnh $A B, A C$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// DS 3
#ds([Cho một đoạn dây dẫn điện bằng đồng có chiều dài cố định $L = 100$ cm. Người ta cắt đoạn dây thành hai phần: phần thứ nhất uốn thành một hình tam giác đều cạnh $a$, phần thứ hai uốn thành một hình tròn bán kính $r$.],
    (
...
// DS 4
#ds([Một kỹ sư ánh sáng cần thiết kế một bóng đèn chiếu điểm đặt tại điểm $S(0; h)$ trên trục tung thẳng đứng ($h > 0$) để chiếu sáng một vật thể cố định nằm tại điểm $A(d; 0)$ trên mặt đất phẳng ngang ($d > 0$ là hằng số cho trước). Biết rằng cường độ chiếu sáng $I$ tại $A$ tỉ lệ thuận với sin của góc nghiêng tia sáng so với mặt đất và tỉ lệ nghịch với bình phương khoảng cách từ nguồn sáng $S$ đến điểm $A$, theo công thức thực nghiệm:
...
// TLN 3
#tln([Một tấm kính hình tròn bán kính $R = 50$ cm. Người thợ muốn cắt từ tấm kính đó một hình chữ nhật có diện tích lớn nhất để làm mặt bàn kính. Biết rằng hình chữ nhật đó nội tiếp trong đường tròn. Diện tích lớn nhất của tấm kính hình chữ nhật đó bằng bao nhiêu nghìn xăng-ti-mét vuông ($1000 "cm"^2$)?],
    [5],
    loigiai: [
        #step([Lời giải chi tiết])
...
// TLN 4
#tln([Cho một tam giác vuông có cạnh huyền bằng $10$ cm. Diện tích lớn nhất của tam giác vuông đó bằng bao nhiêu xăng-ti-mét vuông?],
    [25],
    loigiai: [
        #step([Lời giải chi tiết])
...
// TLN 5
#tln([Một đoạn dây thép dài $34$ m được cắt thành hai phần: phần thứ nhất uốn thành một hình vuông cạnh $x$ (m), phần còn lại uốn thành một hình chữ nhật có chiều dài gấp đôi chiều rộng. Để tổng diện tích của hình vuông và hình chữ nhật là nhỏ nhất thì cạnh hình vuông $x$ phải bằng bao nhiêu mét?],
    [4],
    loigiai: [
        #step([Lời giải chi tiết])
...
// TLN 6
#tln([Một khu đất hình chữ nhật có diện tích $8100 "m"^2$. Người ta muốn xây tường rào bao quanh toàn bộ khu đất. Để chi phí xây tường rào là nhỏ nhất thì chu vi của khu đất đó bằng bao nhiêu mét?],
    [360],
    loigiai: [
        #step([Lời giải chi tiết])
