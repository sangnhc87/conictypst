...
// ═══════════════════════════════════════════════════════════
#let mode = "loigiai"
#let accent = rgb("b45309") // Amber Brown
...
#show: thpt-school-exam.with(
  ...
  exam-title: "CHỦ ĐỀ 1C: BÀI TOÁN CẮT GIẤY GẤP HÌNH & THIẾT KẾ BAO BÌ (VD - VDC)",
  subject: "TOÁN",
...
// TN 1
#tn([Từ một tấm tôn hình vuông cạnh $a = 60$ cm, người thợ cắt bỏ ở bốn góc bốn hình vuông nhỏ bằng nhau có cạnh bằng $x$ (cm) rồi gập các mép tôn lên để tạo thành một chiếc hộp hình hộp chữ nhật không có nắp. Để thể tích của chiếc hộp nhận được là lớn nhất thì cạnh $x$ của các hình vuông bị cắt bỏ bằng bao nhiêu?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 2
#tn([Từ một tấm bìa các-tông hình chữ nhật có kích thước $50 "cm" times 80 "cm"$, người ta cắt ở bốn góc bốn hình vuông bằng nhau có cạnh bằng $x$ (cm) rồi gập các cạnh lên để tạo thành một chiếc hộp không nắp. Thể tích của chiếc hộp đạt giá trị lớn nhất khi $x$ bằng bao nhiêu?],
    (
        True([$10$ cm]),
        [$12$ cm],
        [$8$ cm],
...
// TN 4
#tn([Một tờ giấy hình chữ nhật $A B C D$ có chiều dài $A B = 8$ cm và chiều rộng $B C = 6$ cm. Người ta gấp tờ giấy sao cho đỉnh $C$ trùng với một điểm trên cạnh đối diện $A D$. Độ dài nếp gấp nhỏ nhất bằng bao nhiêu?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 5
#tn([Người ta muốn tạo một máng dẫn nước mặt cắt hình chữ U từ một dải tôn dài có chiều rộng $W = 40$ cm bằng cách gấp hai mép lên vuông góc với đáy một góc $90 degree$. Để diện tích mặt cắt ngang của máng nước là lớn nhất thì chiều cao của hai thành máng phải bằng bao nhiêu cm?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 6
#tn([Từ một tấm tôn hình tam giác đều cạnh $a = 30$ cm, người ta cắt bỏ ở ba đỉnh ba hình thoi bằng nhau rồi gập các mép lại để tạo thành một khối hộp có đáy là tam giác đều không nắp. Thể tích lớn nhất của khối hộp này đạt được khi chiều cao của khối hộp bằng bao nhiêu cm?],
    (
        True([$5 / sqrt(3)$ cm]),
        [$5$ cm],
        [$2","5$ cm],
...
// TN 7
#tn([Một hộp quà bằng bìa cứng không có nắp hình lăng trụ đứng có đáy là hình vuông, thể tích $V = 108 "cm"^3$. Để diện tích toàn phần của hộp quà là nhỏ nhất (tiết kiệm bìa cứng nhất) thì kích thước các cạnh của hộp quà bằng bao nhiêu?],
    (
...
// TN 8
#tn([Một công ty dịch vụ chuyển phát nhanh quy định rằng: Một gói bưu kiện hình hộp chữ nhật có đáy là hình vuông được chấp nhận vận chuyển nếu tổng của chiều dài và chu vi đáy không vượt quá $108$ inch ($h + 4x <= 108$). Thể tích lớn nhất của một gói bưu kiện hình hộp chữ nhật đáy vuông thỏa mãn quy định trên bằng bao nhiêu inch khối ($"in"^3$)?],
    (
        True([$11664 "in"^3$]),
        [$5832 "in"^3$],
        [$8748 "in"^3$],
...
// TN 9
#tn([Người ta cần cắt một tấm tôn hình tròn có bán kính $R = 12$ cm thành hai phần bằng nhau bởi một đường kính, sau đó uốn một nửa hình tròn đó thành một hình nón (không có đáy). Thể tích của khối nón tạo thành bằng bao nhiêu?],
    (
...
// TN 10
#tn([Một xưởng mộc cần cưa một thân cây gỗ hình trụ có đường kính đáy $d = 40$ cm để xẻ thành một cây xà hình hộp chữ nhật có tiết diện ngang lớn nhất. Chiều rộng và chiều cao của tiết diện cây xà xẻ được bằng bao nhiêu?],
    (
...
// TN 12
#tn([Người ta cần đóng một chiếc thùng carton kín có nắp đậy dung tích $V = 72 "dm"^3$, đáy là hình chữ nhật có chiều dài gấp đôi chiều rộng. Diện tích bề mặt toàn phần nhỏ nhất của chiếc thùng carton này bằng bao nhiêu đề-xi-mét vuông ($"dm"^2$)?],
    (
        True([$108 "dm"^2$]),
        [$144 "dm"^2$],
        [$216 "dm"^2$],
...
// DS 1
#ds([Một xưởng in bao bì nhận hợp đồng sản xuất các hộp đựng quà không có nắp từ các tấm bìa các-tông hình chữ nhật có kích thước $40 "cm" times 60 "cm"$. Quy trình sản xuất là cắt ở 4 góc 4 hình vuông có cạnh $x$ (cm) với $0 < x < 20$, sau đó gập các mép lại thành hình hộp chữ nhật.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// DS 2
#ds([Một chiếc hộp bánh pizza có dạng hình hộp chữ nhật có nắp đậy liền thân, được dập từ một tấm bìa các-tông duy nhất. Đáy hộp là hình vuông cạnh $x$ (cm) và chiều cao của hộp là $h$ (cm). Biết rằng diện tích toàn bộ tấm bìa các-tông cần dùng (bao gồm cả đáy, nắp trên và 4 thành bên xung quanh) là $S = 1800 "cm"^2$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// DS 3
#ds([Một công ty sản xuất đồ gia dụng muốn dập một chiếc khay nướng bánh bằng nhôm hình chữ nhật không nắp có dung tích $V = 1000 "cm"^3$. Để thuận tiện khi sử dụng và xếp vào lò nướng, chiều dài đáy khay gấp đôi chiều rộng đáy khay. Gọi $x$ (cm) là chiều rộng đáy khay và $h$ (cm) là chiều cao của khay.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// DS 4
#ds([Một tờ giấy A4 hình chữ nhật có kích thước $20 "cm" times 30 "cm"$. Người ta có thể cuộn tròn tờ giấy theo hai cách khác nhau để tạo thành mặt xung quanh của một ống hình trụ (không tính mép dán):
...
// TLN 1
#tln([Từ một tấm tôn hình chữ nhật kích thước $36 "cm" times 60 "cm"$, người ta cắt bỏ ở bốn góc bốn hình vuông cạnh $x$ (cm) rồi gập các mép lại để tạo thành một chiếc hộp không nắp có thể tích lớn nhất. Cạnh $x$ của hình vuông bị cắt bỏ bằng bao nhiêu cm?],
    [6],
    loigiai: [
        #step([Lời giải chi tiết])
...
// TLN 2
#tln([Một tấm bìa hình vuông cạnh $30$ cm được cắt ở bốn góc bốn hình vuông cạnh $x = 5$ cm rồi gập thành một chiếc hộp không nắp. Thể tích của chiếc hộp này bằng bao nhiêu nghìn xăng-ti-mét khối ($1000 "cm"^3$)?],
    [2],
    loigiai: [
        #step([Lời giải chi tiết])
...
// TLN 3
#tln([Người ta muốn tạo một chiếc phễu hình nón bằng cách cuộn một hình quạt tròn có bán kính $R = 3 sqrt(3)$ cm. Thể tích lớn nhất của chiếc phễu hình nón này bằng bao nhiêu $pi "cm"^3$? (Điền hệ số trước $pi$).],
    [18],
    loigiai: [
        #step([Lời giải chi tiết])
...
// TLN 4
#tln([Một khối gỗ hình hộp chữ nhật có ba kích thước lần lượt là $4$ cm, $6$ cm và $12$ cm. Độ dài đường chéo lớn nhất của khối gỗ này bằng bao nhiêu cm?],
    [14],
    loigiai: [
        #step([Lời giải chi tiết])
...
// TLN 5
#tln([Một tấm tôn hình chữ nhật kích thước $40 "cm" times 60 "cm"$ được cuộn tròn lại thành một hình trụ (không nắp, không đáy). Gọi $V$ là thể tích lớn nhất có thể đạt được của hình trụ này ($"cm"^3$). Giá trị của tích $V dot pi$ bằng bao nhiêu nghìn xăng-ti-mét khối ($1000 "cm"^3$)?],
    [36],
    loigiai: [
        #step([Lời giải chi tiết])
...
// TLN 6
#tln([Một xưởng cơ khí cần dập một chiếc lon hình trụ có thể tích $V = 128 pi "cm"^3$. Diện tích toàn phần của lon hình trụ này là nhỏ nhất khi chiều cao của lon bằng bao nhiêu cm?],
    [8],
    loigiai: [
        #step([Lời giải chi tiết])
