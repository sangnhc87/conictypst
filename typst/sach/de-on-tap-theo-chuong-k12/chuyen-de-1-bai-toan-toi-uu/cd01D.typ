...
// ═══════════════════════════════════════════════════════════
#let mode = "loigiai"
#let accent = rgb("15803d") // Forest Green
...
#show: thpt-school-exam.with(
  ...
  exam-title: "CHỦ ĐỀ 1D: TỐI ƯU HÓA KINH TẾ, DOANH THU & LỢI NHUẬN (VD - VDC)",
  subject: "TOÁN",
...
// TN 2
#tn([Một công ty sản xuất một loại sản phẩm với hàm chi phí tổng cộng là $C(x) = x^3 - 30x^2 + 400x + 500$ (nghìn đồng), trong đó $x$ là số sản phẩm sản xuất ra ($1 <= x <= 30$). Chi phí trung bình cho mỗi đơn vị sản phẩm được tính bởi $bar(C)(x) = C(x) / x$. Chi phí trung bình nhỏ nhất bằng bao nhiêu nghìn đồng?],
    (
        True([$200$ nghìn đồng]),
        [$175$ nghìn đồng],
        [$225$ nghìn đồng],
...
// TN 3
#tn([Một doanh nghiệp độc quyền bán một loại sản phẩm với hàm cầu thị trường là $p = 200 - 2x$, trong đó $p$ là đơn giá bán (nghìn đồng/sản phẩm) và $x$ là số lượng sản phẩm tiêu thụ ($x > 0$). Chi phí để sản xuất $x$ sản phẩm là $C(x) = x^2 + 20x + 300$ (nghìn đồng). Để lợi nhuận đạt mức tối đa thì doanh nghiệp nên định giá bán mỗi sản phẩm bằng bao nhiêu?],
    (
        True([$140$ nghìn đồng]),
        [$30$ nghìn đồng],
        [$120$ nghìn đồng],
...
// TN 4
#tn([Một hãng xe khách chạy tuyến cố định Hà Nội – Sapa có $40$ chỗ ngồi. Khi giá vé là $200.000$ đồng/vé thì tất cả các ghế đều kín chỗ. Hãng xe khảo sát và thấy rằng: nếu tăng giá vé thêm $10.000$ đồng/vé thì lượng hành khách sẽ giảm đi $1$ người. Chi phí vận hành mỗi chuyến xe cố định là $3.000.000$ đồng, cộng thêm chi phí phục vụ $20.000$ đồng cho mỗi hành khách thực tế trên xe. Giá vé tối ưu để hãng xe đạt lợi nhuận cao nhất trong mỗi chuyến là],
    (
        True([$310.000$ đồng]),
        [$280.000$ đồng],
        [$300.000$ đồng],
...
// TN 6
#tn([Chính phủ áp dụng mức thuế $t$ (đồng) trên mỗi đơn vị sản phẩm bán ra của một doanh nghiệp độc quyền. Doanh nghiệp này có hàm cầu $p = 100 - x$ (nghìn đồng) và hàm chi phí sản xuất $C(x) = 20x + 100$ (nghìn đồng). Doanh nghiệp sẽ chọn sản lượng $x$ để tối đa hóa lợi nhuận sau thuế. Để tổng số tiền thuế thu được $T(t) = t dot x$ là lớn nhất thì chính phủ nên ấn định mức thuế $t$ bằng bao nhiêu nghìn đồng trên một đơn vị sản phẩm?],
    (
        True([$40$ nghìn đồng]),
        [$30$ nghìn đồng],
        [$50$ nghìn đồng],
...
// TN 9
#tn([Chi phí tồn kho và chi phí đặt hàng của một công ty phân phối hàng hóa được xác định bởi mô hình EOQ (Economic Order Quantity):
...
// TN 12
#tn([Một doanh nghiệp chi số tiền $x$ (triệu đồng) cho quảng cáo sản phẩm trên mạng xã hội ($x >= 0$). Doanh thu nhận được từ việc bán sản phẩm đó được mô hình hóa theo công thức:
...
// DS 1
#ds([Một tòa nhà văn phòng cho thuê gồm $80$ căn hộ dịch vụ cao cấp. Nếu giá thuê mỗi căn là $15$ triệu đồng/tháng thì toàn bộ $80$ căn đều có khách thuê. Do nhu cầu thị trường tăng, chủ tòa nhà muốn điều chỉnh tăng giá thuê. Một công ty tư vấn bất động sản cho biết: Cứ mỗi lần tăng giá thuê thêm $1$ triệu đồng/căn/tháng thì sẽ có $2$ căn bị bỏ trống. Ngoài ra, chi phí bảo trì và dọn dẹp hàng tháng cho mỗi căn có người ở là $1$ triệu đồng/tháng.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// DS 2
#ds([Một nhà máy sản xuất linh kiện điện tử sản xuất một loại chip bán dẫn. Hàm tổng chi phí sản xuất hằng ngày là:
...
// DS 3
#ds([Một hãng thời trang bán một mẫu áo khoác mùa đông với chi phí sản xuất mỗi chiếc áo là $400.000$ đồng. Hãng đưa ra chính sách giá sỉ cho các đại lý bán lẻ:
...
// DS 4
#ds([Một công ty điện lực độc quyền cung cấp điện cho một hòn đảo du lịch. Doanh thu biên ($M R$) và chi phí biên ($M C$) theo sản lượng điện tiêu thụ $x$ (nghìn kWh) lần lượt là:
...
// TLN 5
#tln([Một cửa hàng kinh doanh trà sữa có chi phí pha chế cố định mỗi ly là $12.000$ đồng. Nếu bán với giá $30.000$ đồng/ly thì mỗi ngày bán được $200$ ly. Cứ mỗi lần tăng giá thêm $1.000$ đồng/ly thì lượng bán mỗi ngày giảm đi $10$ ly. Lợi nhuận lớn nhất mỗi ngày của cửa hàng bằng bao nhiêu triệu đồng?],
    [3.61],
    loigiai: [
        #step([Lời giải chi tiết])
...
// TLN 6
#tln([Hàm sản xuất ngắn hạn của một phân xưởng may mặc được cho bởi công thức $Q(L) = -L^3 + 12 L^2 + 60 L$, trong đó $L$ là số lượng công nhân làm việc ($1 <= L <= 10$) và $Q$ là số sản phẩm may được mỗi ngày. Năng suất lao động bình quân của mỗi công nhân được xác định bởi $A P(L) = Q(L) / L$. Năng suất lao động bình quân đạt giá trị lớn nhất khi phân xưởng bố trí bao nhiêu công nhân?],
    [6],
    loigiai: [
        #step([Lời giải chi tiết])
