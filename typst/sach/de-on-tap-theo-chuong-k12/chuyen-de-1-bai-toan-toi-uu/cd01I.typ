...
#show: thpt-school-exam.with(
  ...
  exam-title: "CHỦ ĐỀ 1I: MÔ HÌNH SINH THÁI, KHAI THÁC BỀN VỮNG & DỊCH TỄ (VD - VDC)",
  subject: "TOÁN",
...
// TN 1
#tn([Một quần thể cá trong một hồ sinh thái phát triển theo mô hình tăng trưởng Logistic:
...
// TN 2
#tn([Theo mô hình khai thác ngư nghiệp bền vững Schaefer, một quần thể cá hồi có tốc độ tăng trưởng sinh khối tự nhiên hàng năm là $G(P) = 0.6 P (1 - P/5000)$ (tấn/năm), với $P$ là sinh khối cá hiện có (tấn). Sản lượng khai thác bền vững tối đa (MSY - Maximum Sustainable Yield) hàng năm có thể đánh bắt mà không làm suy giảm trữ lượng cá qua các năm là:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 3
#tn([Sau khi tiêm một liều thuốc kháng sinh vào bắp tay bệnh nhân, nồng độ thuốc trong máu sau $t$ giờ được mô tả bởi hàm số:
...
// TN 4
#tn([Trong một đợt bùng phát dịch cúm mùa ở một trường học nội trú có $1000$ học sinh, số học sinh bị lây nhiễm sau $t$ ngày kể từ ca đầu tiên được cho bởi:
...
// TN 5
#tn([Một công ty lâm nghiệp khai thác một cánh rừng bạch đàn thương dụng. Thể tích gỗ $V(t)$ ($m^3$/ha) theo tuổi cây $t$ (năm) được cho bởi $V(t) = 240 (1 - e^(-0.08 t))^2$. Tốc độ tích lũy sinh khối gỗ hàng năm $V'(t)$ đạt giá trị cực đại khi rừng đạt độ tuổi bao nhiêu năm?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 6
#tn([Một phản ứng sinh hóa tự xúc tác (autocatalytic reaction) tạo ra hoạt chất $X$ từ cơ chất ban đầu với nồng độ ban đầu là $a$ (mol/L). Tốc độ của phản ứng biến thiên theo nồng độ sản phẩm $x$ đã hình thành theo mô hình:
$ v(x) = k x (a - x) $
...
// TN 7
#tn([Một trang trại nuôi tôm thâm canh. Khối lượng trung bình của một con tôm sau $t$ tuần tuổi tuân theo hàm số $W(t) = 45 / (1 + 8 e^(-0.25 t))$ (gam). Người nuôi muốn xác định thời điểm tôm có tốc độ tăng trọng lớn nhất để tập trung bổ sung dinh dưỡng tối ưu. Tôm đạt tốc độ tăng trọng lớn nhất sau bao nhiêu tuần?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 8
#tn([Hiệu quả của việc phun thuốc bảo vệ thực vật trên cánh đồng lúa được đánh giá qua tỷ lệ sâu bệnh bị tiêu diệt $E(x) = (90 x) / (x + 2)$ (%), với $x$ là liều lượng phun (lít/ha). Biết chi phí phun thuốc là $600$ nghìn đồng/lít và giá trị nông sản được bảo toàn cứ mỗi $1%$ sâu bệnh bị diệt đem lại $30$ nghìn đồng/ha. Liều lượng phun $x$ để tối đa hóa lợi ích ròng (tiền bảo vệ trừ chi phí) là:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 10
#tn([Một khu bảo tồn thả một loài chim quý hiếm có nguy cơ tuyệt chủng vào tự nhiên. Tốc độ thay đổi số lượng cá thể $N(t)$ tuân theo mô hình có ngưỡng sinh tồn tối thiểu (hiệu ứng Allee):
...
// DS 1
#ds([Một đàn hươu trong một vườn quốc gia được bảo vệ nghiêm ngặt. Số lượng cá thể $N(t)$ sau $t$ năm ($t >= 0$) được mô hình hóa bởi phương trình tăng trưởng Logistic:
...
// DS 2
#ds([Để quản lý bền vững một vùng biển có loài cá ngừ đại dương với sức chứa tự nhiên $K = 8000$ tấn và hệ số sinh trưởng tự nhiên $r = 0.5$ năm#super[-1]. Chi cục Thủy sản áp dụng mô hình thu hoạch Schaefer:
$ Y(E) = q E P $
...
// DS 3
#ds([Một bệnh nhân suy tim được điều trị bằng thuốc trợ tim theo đường uống. Nồng độ thuốc trong máu sau khi uống một liều duy nhất tuân theo mô hình dược động học:
...
// DS 4
#ds([Một nhóm chuyên gia dịch tễ nghiên cứu đợt bùng phát của một chủng virus lây truyền qua đường hô hấp tại một thành phố có $100000$ dân. Số ca nhiễm bệnh tích lũy $S(t)$ sau $t$ ngày được dự báo bởi mô hình:
...
// TLN 1
#tln([Một khu rừng trồng cây gỗ công nghiệp có sinh khối gỗ sau $t$ năm tuân theo quy luật Logistic $V(t) = 500 / (1 + 9 e^(-0.1 t))$ ($m^3$/ha). Tốc độ tích lũy gỗ hàng năm lớn nhất của khu rừng đạt bao nhiêu mét khối trên một hecta trong một năm ($m^3$/ha/năm)? (làm tròn kết quả đến hàng đơn vị)
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TLN 4
#tln([Theo mô hình dịch tễ học lây truyền trong một quần thể khép kín có quy mô $N = 10000$ người, số ca lây nhiễm virus tại thời điểm tốc độ phát sinh ca mới đạt đỉnh điểm là bao nhiêu người?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TLN 5
#tln([Một xí nghiệp nuôi nghêu ven biển khảo sát thấy lợi nhuận hàng năm phụ thuộc vào nỗ lực thu hoạch $E$ (số ngày cào nghêu) theo công thức: $Pi(E) = - 0.2 E^2 + 48 E - 800$ (triệu đồng). Để tối đa hóa lợi nhuận kinh tế, xí nghiệp cần thực hiện bao nhiêu ngày thu hoạch trong năm?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TLN 6
#tln([Để xử lý nước thải công nghiệp nhiễm kim loại nặng trong hồ chứa sinh học, nồng độ chất ô nhiễm sau $t$ ngày xử lý bằng vi sinh vật biến tính được xác định bởi: $C(t) = 120 e^(-0.05 t)$ (mg/L). Chi phí vận hành trạm vi sinh mỗi ngày là $0.5$ triệu đồng/ngày. Giá trị thiệt hại môi trường tồn đọng sau $t$ ngày là $D(t) = 0.25 C(t)$ triệu đồng. Tổng chi phí gồm phí vận hành và thiệt hại tồn đọng là $T(t) = 0.5 t + 30 e^(-0.05 t)$. Thời gian xử lý $t$ (ngày) để tổng chi phí nhỏ nhất là bao nhiêu ngày? (làm tròn kết quả đến hàng đơn vị)
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
