...
#show: thpt-school-exam.with(
  ...
  exam-title: "CHỦ ĐỀ 2A: LÃI ĐƠN, LÃI KÉP & LÃI SUẤT THỰC TẾ (VD - VDC)",
  subject: "TOÁN",
...
// TN 1
#tn([Một người gửi tiết kiệm $200$ triệu đồng vào ngân hàng theo thể thức lãi kép kỳ hạn $1$ tháng với lãi suất $0","5\%$/tháng. Giả sử lãi suất không thay đổi và người đó không rút tiền trong suốt quá trình gửi. Sau đúng $3$ năm ($36$ tháng), tổng số tiền cả gốc lẫn lãi người đó nhận được là bao nhiêu?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 2
#tn([Một nhà đầu tư gửi số tiền $P$ vào quỹ tài chính với lãi suất công bố $8\%$/năm theo hình thức ghép lãi liên tục. Hỏi sau bao nhiêu năm số tiền tích lũy trong quỹ sẽ tăng lên gấp đôi số vốn ban đầu?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 3
#tn([Ngân hàng $A$ áp dụng mức lãi suất danh nghĩa $7","2\%$/năm với hình thức ghép lãi hàng tháng. Ngân hàng $B$ áp dụng mức lãi suất danh nghĩa $7","25\%$/năm với hình thức ghép lãi nửa năm một lần ($6$ tháng/lần). So sánh lãi suất thực tế hiệu dụng hàng năm (EAR - Effective Annual Rate) của hai ngân hàng, khẳng định nào sau đây là đúng?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 5
#tn([Một người gửi tiết kiệm số tiền $300$ triệu đồng vào ngân hàng trong thời hạn $5$ năm. Trong $2$ năm đầu tiên, lãi suất tiền gửi cố định là $6\%$/năm ghép lãi hàng năm. Do thị trường biến động, trong $3$ năm tiếp theo ngân hàng điều chỉnh nâng mức lãi suất lên $7\%$/năm ghép lãi hàng năm. Biết toàn bộ tiền lãi của mỗi năm đều được tự động nhập vào vốn gốc để tính lãi cho năm tiếp theo. Sau $5$ năm, tổng số tiền người đó nhận được xấp xỉ bằng bao nhiêu?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 8
#tn([Cùng với số vốn ban đầu $P$ và mức lãi suất $8\%$/năm, gói đầu tư thứ nhất áp dụng thể thức lãi đơn, gói đầu tư thứ hai áp dụng thể thức lãi kép hàng năm. Hỏi sau khoảng bao nhiêu năm thì số tiền lãi thu được từ gói lãi kép sẽ cao gấp đôi số tiền lãi thu được từ gói lãi đơn?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 9
#tn([Một dịch vụ cho vay tiêu dùng ngắn hạn áp dụng mức lãi suất công bố là $0","1\%$/ngày, tính theo thể thức lãi kép hàng ngày ($365$ ngày/năm). Lãi suất thực tế hiệu dụng hàng năm (EAR) của khoản vay này xấp xỉ bằng bao nhiêu?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TN 12
#tn([Một người gửi tiết kiệm ngân hàng với lãi suất danh nghĩa $6\%$/năm, nhưng trong cùng thời kỳ đó, tỷ lệ lạm phát hàng năm của nền kinh tế lên tới $8\%$/năm. Sau $5$ năm gửi tiền, tỷ lệ thay đổi về sức mua thực tế của số vốn người đó nhận được so với thời điểm ban đầu là bao nhiêu?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// DS 1
#ds([Một khách hàng có khoản tiền nhàn rỗi $500$ triệu đồng dự định gửi tiết kiệm vào ngân hàng trong thời hạn $3$ năm. Ngân hàng đưa ra hai gói sản phẩm:
...
// DS 2
#ds([Một doanh nghiệp phát hành chứng chỉ tiền gửi thời hạn $6$ năm nhằm huy động vốn mở rộng sản xuất. Khách hàng tham gia được cam kết biểu lãi suất bậc thang lũy tiến như sau:
...
// DS 3
#ds([Một quỹ tài chính gia đình lập kế hoạch chuẩn bị chi phí đại học cho con trong tương lai. Chi phí đại học hiện tại ước tính là $400$ triệu đồng cho toàn bộ $4$ năm học. Dự kiến sau đúng $12$ năm nữa con mới bắt đầu bước vào giảng đường đại học.
...
// DS 4
#ds([Một doanh nghiệp công nghệ khởi nghiệp (Startup) ghi nhận doanh thu năm đầu tiên đạt $10$ tỷ đồng. Ban giám đốc đặt mục tiêu doanh thu tăng trưởng liên tục theo hàm số mũ $S(t) = 10 e^(0","25 t)$ (tỷ đồng), trong đó $t$ là thời gian tính theo năm ($t >= 0$).
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TLN 1
#tln([Một khoản vay tiêu dùng có lãi suất danh nghĩa công bố là $12\%$/năm, tính theo hình thức ghép lãi hàng tháng ($12$ kỳ/năm). Lãi suất thực tế hiệu dụng hàng năm (EAR) của khoản vay này bằng bao nhiêu phần trăm? (làm tròn kết quả đến hàng phần mười)
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TLN 2
#tln([Một người gửi tiết kiệm một số tiền vào ngân hàng với lãi suất cố định $6","5\%$/năm theo hình thức lãi kép hàng năm. Hỏi sau ít nhất bao nhiêu năm thì tổng số tiền người đó nhận về (cả gốc và lãi) gấp từ $3$ lần số vốn ban đầu trở lên? (kết quả là số nguyên năm)
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TLN 3
#tln([Một nhà đầu tư gửi số tiền $200$ triệu đồng vào một quỹ đầu tư sinh lời theo thể thức lãi kép liên tục với lãi suất danh nghĩa $7\%$/năm. Sau đúng $10$ năm, tổng số tiền lãi thu được từ khoản đầu tư này là bao nhiêu triệu đồng? (làm tròn kết quả đến hàng đơn vị)
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TLN 4
#tln([Giả sử tỷ lệ lạm phát bình quân hàng năm của nền kinh tế duy trì ổn định ở mức $4","5\%$/năm. Hỏi sau khoảng bao nhiêu năm thì sức mua thực tế của một khoản tiền mặt bị giảm đi một nửa so với ban đầu? (làm tròn kết quả đến hàng đơn vị)
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
...
// TLN 5
#tln([Một chiếc máy xúc công trình được mua mới với giá $3$ tỷ đồng ($3000$ triệu đồng). Hàng năm, giá trị của chiếc máy xúc bị khấu hao giảm $15\%$ so với giá trị còn lại của năm trước đó. Sau đúng $5$ năm sử dụng, giá trị còn lại của chiếc máy xúc là bao nhiêu triệu đồng? (làm tròn kết quả đến hàng đơn vị)
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    set-style(stroke: 0.8pt)
