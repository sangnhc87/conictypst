...
#show: thpt-school-exam.with(
  ...
  exam-title: "ĐỀ ÔN TẬP CHUYÊN ĐỀ 3C: PHƯƠNG SAI V(X), ĐỘ LỆCH CHUẨN SIGMA(X) & ĐO LƯỜNG MỨC ĐỘ RỦI RO",
  subject: "TOÁN",
...
// TN 2
#tn([Cho biến ngẫu nhiên rời rạc $X$ có phương sai $V(X) = 4$ (tương ứng độ lệch chuẩn $sigma(X) = 2$). Xét biến ngẫu nhiên mới được tạo bởi phép biến đổi affine $Y = -3 X + 5$. Phương sai $V(Y)$ và độ lệch chuẩn $sigma(Y)$ bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 3
#tn([Biến ngẫu nhiên rời rạc $X$ nhận các giá trị $1, 2, 3$ với bảng phân bố xác suất chứa tham số $m$:
#align(center)[
  #table(
    columns: 4,
    align: center,
...
// TN 4
#tn([Một hộp chứa $3$ quả cầu màu đỏ và $5$ quả cầu màu xanh có cùng kích thước. Lấy ngẫu nhiên không hoàn lại $2$ quả cầu từ hộp. Gọi $X$ là số quả cầu màu đỏ lấy được. Phương sai $V(X)$ của biến ngẫu nhiên $X$ bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 5
#tn([Một nhà đầu tư đang cân nhắc hai cổ phiếu $A$ và $B$ có cùng tỷ suất sinh lời kỳ vọng $E(R_A) = E(R_B) = 12%$. Bảng phân bố xác suất của tỷ suất sinh lời (%) của hai cổ phiếu như sau:
#align(center)[
  #table(
    columns: 4,
    align: center,
...
// TN 6
#tn([Hai dự án kinh doanh $D_1$ và $D_2$ có quy mô vốn khác nhau. Lợi nhuận kỳ vọng và độ lệch chuẩn của hai dự án được cho như sau:
...
// TN 7
#tn([Biến ngẫu nhiên $X$ chỉ thị thành công trong một phép thử Bernoulli, tức là $X$ nhận giá trị $1$ (thành công) với xác suất $p$ và nhận giá trị $0$ (thất bại) với xác suất $1 - p$ ($0 <= p <= 1$). Phương sai $V(X)$ đạt giá trị lớn nhất bằng bao nhiêu và tại giá trị $p$ nào?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 8
#tn([Hai trạm biến áp cung cấp điện cho hai phân xưởng độc lập. Gọi $X$ và $Y$ (đơn vị: MW) lần lượt là công suất tiêu thụ của phân xưởng 1 và phân xưởng 2 trong giờ cao điểm. Biết $X$ và $Y$ là hai biến ngẫu nhiên độc lập có phương sai lần lượt là $V(X) = 1","44$ và $V(Y) = 2","56$. Độ lệch chuẩn của tổng công suất tiêu thụ $Z = X + Y$ bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 10
#tn([Một trò chơi quay số trúng thưởng có bảng phân bố số tiền nhận được $X$ (nghìn đồng) như sau:
#align(center)[
  #table(
    columns: 4,
    align: center,
...
// TN 11
#tn([Cho biến ngẫu nhiên $X$ có $E(X) = 50$ và phương sai $V(X) = 16$ (độ lệch chuẩn $sigma = 4$). Dựa vào bất đẳng thức Chebyshev, xác suất để giá trị của $X$ rơi vào khoảng $(40; 60)$, tức $P(40 < X < 60)$, thỏa mãn:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 12
#tn([Một nhà đầu tư phân bổ danh mục gồm hai cổ phiếu độc lập $X$ và $Y$ có phương sai lần lượt là $V(X) = 4$ và $V(Y) = 9$. Gọi $w$ ($0 <= w <= 1$) là tỷ trọng vốn đầu tư vào cổ phiếu $X$, phần còn lại $1 - w$ đầu tư vào cổ phiếu $Y$. Khi đó, lợi nhuận của danh mục là biến ngẫu nhiên $R_p = w X + (1 - w) Y$. Tỷ trọng $w$ cần đầu tư vào cổ phiếu $X$ để phương sai của danh mục $V(R_p)$ đạt giá trị nhỏ nhất (danh mục tối thiểu hóa rủi ro) bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// DS 1 (Câu 13)
#ds([Hai dây chuyền sản xuất tự động $A$ và $B$ cùng gia công một loại linh kiện điện tử vi mạch. Để đánh giá độ ổn định chất lượng, người ta kiểm tra ngẫu nhiên các lô hàng 100 sản phẩm và ghi nhận số vi mạch bị lỗi. Bảng phân bố xác suất của số vi mạch lỗi $X_A$ (dây chuyền A) và $X_B$ (dây chuyền B) như sau:
#align(center)[
  #table(
    columns: 5,
    align: center,
...
// DS 2 (Câu 14)
#ds([Một quỹ đầu tư mạo hiểm phân bổ vốn vào hai dự án độc lập $X$ và $Y$. Tỷ suất lợi nhuận (%) của hai dự án là các biến ngẫu nhiên có bảng phân bố xác suất như sau:
#align(center)[
  #table(
    columns: 4,
    align: center,
...
// DS 3 (Câu 15)
#ds([Một công ty bảo hiểm nhân thọ phát hành gói sản phẩm bảo hiểm tai nạn cho $100$ khách hàng độc lập. Mỗi hợp đồng có thời hạn 1 năm, số tiền chi trả bồi thường nếu xảy ra tai nạn là $100$ triệu đồng. Xác suất xảy ra tai nạn của mỗi khách hàng trong năm đều bằng $p = 0","01$ ($1%$). Gọi $X_i$ là số tiền bồi thường cho khách hàng thứ $i$ ($i = 1, 2, ..., 100$) và $S = sum_(i=1)^100 X_i$ là tổng số tiền công ty phải bồi thường trong năm.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// DS 4 (Câu 16)
#ds([Một xe vận chuyển hàng bưu chính di chuyển qua hai cung đường độc lập để đến điểm giao: chặng 1 trên quốc lộ có thời gian di chuyển $T_1$ (phút) và chặng 2 qua nội đô có thời gian di chuyển $T_2$ (phút). Bảng phân bố xác suất của $T_1$ và $T_2$ như sau:
#align(center)[
  #table(
    columns: 4,
    align: center,
...
// TLN 1 (Câu 17)
#tln([Cho biến ngẫu nhiên rời rạc $X$ có bảng phân bố xác suất:
#align(center)[
  #table(
    columns: 4,
    align: center,
...
// TLN 2 (Câu 18)
#tln([Cho biến ngẫu nhiên rời rạc $X$ nhận 3 giá trị $-1, 0, 1$ với bảng phân bố xác suất đối xứng phụ thuộc tham số $p$ ($0 < p < 0","5$):
#align(center)[
  #table(
    columns: 4,
    align: center,
...
// TLN 3 (Câu 19)
#tln([Cho biến ngẫu nhiên rời rạc $X$ có phương sai $V(X) = 3","6$. Xét biến ngẫu nhiên $Y = -2","5 X + 7$. Phương sai $V(Y)$ có giá trị bằng bao nhiêu?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TLN 5 (Câu 21)
#tln([Một dây chuyền đóng gói bánh kẹo tự động đóng các hộp bánh với khối lượng mỗi hộp là một biến ngẫu nhiên có độ lệch chuẩn $sigma_0 = 5" gam"$. Người ta đóng $16$ hộp bánh độc lập vào một thùng hàng lớn. Độ lệch chuẩn của tổng khối lượng $16$ hộp bánh trong thùng bằng bao nhiêu gam?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TLN 6 (Câu 22)
#tln([Doanh thu bán hàng trong tuần lễ khuyến mãi của một siêu thị là biến ngẫu nhiên $X$ có kỳ vọng $E(X) = 150$ triệu đồng và độ lệch chuẩn $sigma(X) = 24$ triệu đồng. Hệ số biến thiên $C V = frac(sigma(X), E(X))$ của doanh thu siêu thị bằng bao nhiêu phần trăm? (Chỉ ghi số phần trăm, ví dụ ghi 16 nếu là 16%).
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
