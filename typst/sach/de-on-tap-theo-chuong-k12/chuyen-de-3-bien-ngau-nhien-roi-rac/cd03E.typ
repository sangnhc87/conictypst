...
#let mode = "loigiai"
#let accent = rgb("92400e") // Burnt Amber
...
#show: thpt-school-exam.with(
  ...
  exam-title: "ĐỀ ÔN TẬP CHUYÊN ĐỀ 3E: CÁC SỐ ĐẶC TRƯNG CỦA PHÂN BỐ NHỊ THỨC: KỲ VỌNG & PHƯƠNG SAI",
  subject: "TOÁN",
...
// TN 1
#tn([Cho biến ngẫu nhiên rời rạc $X$ tuân theo phân bố nhị thức với các tham số $n$ và $p$, ký hiệu $X ~ B(n, p)$. Công thức tính kỳ vọng toán học $E(X)$ và phương sai $V(X)$ của $X$ là:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 2
#tn([Một trang trại gieo $n = 200$ hạt giống ngô lai độc lập nhau. Xác suất để mỗi hạt nảy mầm phát triển thành cây con khỏe mạnh là $p = 0","85$. Kỳ vọng số cây con ngô nảy mầm bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 3
#tn([Một học sinh làm bài thi trắc nghiệm khách quan gồm $n = 60$ câu hỏi. Mỗi câu có $4$ phương án lựa chọn và chỉ có $1$ phương án đúng. Học sinh này khoanh ngẫu nhiên hoàn toàn cả $60$ câu. Phương sai của số câu trả lời đúng bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 4
#tn([Một biến ngẫu nhiên rời rạc $X$ tuân theo quy luật phân bố nhị thức $X ~ B(n, p)$. Biết rằng kỳ vọng $E(X) = 24$ và phương sai $V(X) = 4","8$. Số phép thử $n$ và xác suất thành công $p$ lần lượt là:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 5
#tn([Một xạ thủ bắn độc lập $n = 25$ viên đạn vào bia. Xác suất bắn trúng bia trong mỗi lần bắn đều là $p = 0","8$. Độ lệch chuẩn của số phát đạn bắn trúng bia bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 7
#tn([Một biến ngẫu nhiên $X$ tuân theo phân bố nhị thức $X ~ B(n, p)$ có kỳ vọng $E(X) = 12$ và độ lệch chuẩn $sigma(X) = 2$. Xác suất để biến ngẫu nhiên $X$ nhận đúng giá trị kỳ vọng của nó $P(X = 12)$ bằng (làm tròn kết quả đến hàng phần nghìn):
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 8
#tn([Cho trước số phép thử độc lập $n$ không đổi ($n >= 1$). Khi xác suất thành công $p$ thay đổi trong khoảng $(0; 1)$, phương sai $V(X)$ của biến ngẫu nhiên nhị thức $X ~ B(n, p)$ đạt giá trị lớn nhất bằng bao nhiêu và tại giá trị $p$ nào?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 9
#tn([Một tổng đài chăm sóc khách hàng ghi nhận số cuộc gọi bị nghẽn mạng trong một giờ là biến ngẫu nhiên $X ~ B(100; 0","04)$. Theo quy tắc kinh nghiệm $2 sigma$, khoảng giá trị $[E(X) - 2 sigma(X); E(X) + 2 sigma(X)]$ chứa phần lớn giá trị của $X$ là:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 10
#tn([Hai trạm khí tượng thủy văn độc lập quan sát bão nhiệt đới trong mùa mưa bão. Số cơn bão đổ bộ qua trạm $1$ là $X_1 ~ B(8; 0","3)$ và qua trạm $2$ là $X_2 ~ B(12; 0","3)$. Biến ngẫu nhiên tổng số cơn bão $X = X_1 + X_2$ đổ bộ có kỳ vọng và phương sai lần lượt là:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    // Hai trạm hợp nhất
...
// TN 11
#tn([Một công ty bảo hiểm xe máy phát hành $n = 1000$ hợp đồng bảo hiểm độc lập trong một năm. Xác suất để một chủ xe gặp sự cố cần yêu cầu bồi thường trong năm là $p = 0","004$. Độ lệch chuẩn của số vụ yêu cầu bồi thường mà công ty phải giải quyết trong năm bằng (làm tròn đến hàng phần trăm):
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 12
#tn([Cho biến ngẫu nhiên $X ~ B(n, p)$. Xét biến ngẫu nhiên tần suất thành công mẫu $F = frac(X, n)$. Kỳ vọng $E(F)$ và phương sai $V(F)$ của tần suất thành công $F$ là:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TLN 2
#tln([Một nhà máy sản xuất lon nước ngọt vận hành dây chuyền đóng nắp lon với tỷ lệ lon bị lỗi hở mép là $p = 0","015$. Trong một ca sản xuất, dây chuyền đóng được $n = 4000$ lon (các lon được đóng độc lập nhau). Gọi $X$ là số lon bị lỗi hở mép trong ca sản xuất đó. Tính phương sai $V(X)$ của số lon bị lỗi.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TLN 4
#tln([Một sàn thương mại điện tử thống kê thấy mỗi lượt truy cập vào trang giới thiệu sản phẩm của một gian hàng công nghệ có xác suất chuyển đổi thành đơn hàng mua thành công là $p = 0","06$. Trong một ngày khuyến mãi, gian hàng ghi nhận có $n = 2500$ lượt truy cập độc lập. Mỗi đơn hàng thành công đem lại cho gian hàng khoản lợi nhuận ròng trung bình là $300$ nghìn đồng. Tính kỳ vọng tổng lợi nhuận ròng thu được từ các đơn hàng trong ngày khuyến mãi đó (đơn vị: triệu đồng).
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TLN 5
#tln([Cho biến ngẫu nhiên rời rạc $X$ tuân theo quy luật phân bố nhị thức $X ~ B(n, p)$ thỏa mãn đồng thời hai điều kiện: kỳ vọng $E(X) = 18$ và độ lệch chuẩn $sigma(X) = 3$. Tìm giá trị của số phép thử $n$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
    // Sơ đồ xác định n
...
// TLN 6
#tln([Một dự án phủ xanh đồi trọc tiến hành trồng thử nghiệm $n = 1500$ cây lát hoa. Do điều kiện thổ nhưỡng khắc nghiệt, xác suất để mỗi cây sống sót và phát triển tốt sau một năm là $p = 0","8$ (sự sống sót của các cây là độc lập nhau). Dự án quy định: mỗi cây sống sót được đầu tư chi phí chăm sóc dưỡng cây là $120$ nghìn đồng; mỗi cây bị chết phải chịu chi phí đào bỏ và trồng dặm lại là $40$ nghìn đồng. Gọi $C$ là tổng chi phí chăm sóc và trồng dặm sau một năm của dự án (đơn vị: triệu đồng). Tính kỳ vọng $E(C)$ của tổng chi phí đó.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
