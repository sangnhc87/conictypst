...
#let mode = "loigiai"
#let accent = rgb("155e75") // Dark Cyan
...
#show: thpt-school-exam.with(
  ...
  exam-title: "ĐỀ ÔN TẬP CHUYÊN ĐỀ 3F: BIẾN NGẪU NHIÊN ĐỘC LẬP, TỔNG & HIỆU BIẾN NGẪU NHIÊN RỜI RẠC",
  subject: "TOÁN",
...
// TN 1
#tn([Hai biến ngẫu nhiên rời rạc $X$ và $Y$ được gọi là độc lập với nhau khi và chỉ khi với mọi giá trị $x_i$ của $X$ và mọi giá trị $y_j$ của $Y$, ta luôn có:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 2
#tn([Cho $X$ và $Y$ là hai biến ngẫu nhiên rời rạc tùy ý (không nhất thiết độc lập) có kỳ vọng lần lượt là $E(X) = 8$ và $E(Y) = 14$. Kỳ vọng của biến ngẫu nhiên tổng $Z = X + Y$ bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 3
#tn([Cho hai biến ngẫu nhiên rời rạc $X$ và $Y$ độc lập với nhau, có phương sai lần lượt là $V(X) = 16$ và $V(Y) = 9$. Phương sai của biến ngẫu nhiên hiệu $D = X - Y$ bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 5
#tn([Cho hai biến ngẫu nhiên độc lập $X$ và $Y$ có các tham số: $E(X) = 3, V(X) = 4$ và $E(Y) = -2, V(Y) = 1$. Xét biến ngẫu nhiên $Z = 2 X - 3 Y + 5$. Kỳ vọng $E(Z)$ và phương sai $V(Z)$ lần lượt là:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 6
#tn([Hai xạ thủ A và B thi đấu bắn súng độc lập. Xạ thủ A bắn $n_1 = 10$ phát với xác suất trúng mỗi phát là $p_1 = 0","8$. Xạ thủ B bắn $n_2 = 12$ phát với xác suất trúng mỗi phát là $p_2 = 0","75$. Gọi $X$ và $Y$ lần lượt là số phát bắn trúng của xạ thủ A và B. Kỳ vọng tổng số phát trúng $E(X + Y)$ bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 8
#tn([Nếu hai biến ngẫu nhiên rời rạc $X$ và $Y$ độc lập với nhau thì kỳ vọng của tích $E(X dot Y)$ thỏa mãn hệ thức nào sau đây?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 9
#tn([Hai bóng đèn sợi đốt $Đ_1$ và $Đ_2$ được lắp vào cùng một mạch điện. Số lần hỏng trong năm của đèn $Đ_1$ là $X$, của đèn $Đ_2$ là $Y$ (hai đèn hoạt động độc lập). Cho biết phân bố của $X$ có $E(X) = 1","2, V(X) = 0","8$ và của $Y$ có $E(Y) = 1","8, V(Y) = 1","2$. Phương sai của biến ngẫu nhiên $W = 3 X + 2 Y$ bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 11
#tn([Một hộ gia đình sử dụng điện cho hai mục đích: sinh hoạt và kinh doanh tiệm tạp hóa. Lượng điện tiêu thụ hàng tháng cho sinh hoạt là $X$ (kWh) có $E(X) = 250, sigma(X) = 30$. Lượng điện tiêu thụ cho kinh doanh là $Y$ (kWh) có $E(Y) = 450, sigma(Y) = 40$. Giả sử $X$ và $Y$ độc lập nhau. Kỳ vọng và độ lệch chuẩn của tổng lượng điện tiêu thụ $S = X + Y$ trong tháng lần lượt là:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 12
#tn([Cho ba biến ngẫu nhiên $X_1, X_2, X_3$ đôi một độc lập và có cùng phân bố xác suất với kỳ vọng $mu$ và phương sai $sigma^2$. Xét biến ngẫu nhiên trung bình cộng $bar(X) = frac(X_1 + X_2 + X_3, 3)$. Phương sai của $bar(X)$ bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// DS 1
#ds([Cho bảng phân bố xác suất đồng thời của hai biến ngẫu nhiên rời rạc $X$ (nhận giá trị trong tập ${1; 2}$) và $Y$ (nhận giá trị trong tập ${0; 1}$) như sau:
#align(center)[
  #table(
...
// DS 2
#ds([Một công ty dịch vụ chuyển phát nhanh có hai xe tải $A$ và $B$ chuyên chở hàng giữa hai thành phố. Số vụ hỏng hóc kỹ thuật trong một tháng của xe $A$ là biến ngẫu nhiên $X$, của xe $B$ là biến ngẫu nhiên $Y$. Giả thiết $X$ và $Y$ là hai biến ngẫu nhiên độc lập, có các đặc trưng số:
...
// DS 3
#ds([Hai dây chuyền sản xuất đồ uống đóng chai hoạt động hoàn toàn độc lập. Dây chuyền $1$ đóng $n_1 = 500$ chai với xác suất bị lỗi thể tích là $p_1 = 0","02$. Dây chuyền $2$ đóng $n_2 = 800$ chai với xác suất bị lỗi thể tích là $p_2 = 0","01$. Gọi $X_1$ và $X_2$ lần lượt là số chai bị lỗi thể tích của dây chuyền $1$ và dây chuyền $2$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TLN 1
#tln([Cho hai biến ngẫu nhiên rời rạc $X$ và $Y$ thỏa mãn $E(X) = 15$ và $E(Y) = 25$. Xét biến ngẫu nhiên $Z = 4 X - 2 Y + 18$. Tính kỳ vọng $E(Z)$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TLN 2
#tln([Cho hai biến ngẫu nhiên $X$ và $Y$ độc lập với nhau, có độ lệch chuẩn lần lượt là $sigma(X) = 6$ và $sigma(Y) = 8$. Tính độ lệch chuẩn của biến ngẫu nhiên hiệu $D = X - Y$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TLN 4
#tln([Hai sinh viên cùng tham gia kỳ thi tiếng Anh trực tuyến độc lập. Điểm thi của sinh viên thứ nhất là biến ngẫu nhiên $X$ có $E(X) = 75, V(X) = 25$. Điểm thi của sinh viên thứ hai là biến ngẫu nhiên $Y$ có $E(Y) = 80, V(Y) = 39$. Tìm phương sai của tổng điểm số của cả hai sinh viên $V(X + Y)$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TLN 5
#tln([Cho hai biến ngẫu nhiên $X$ và $Y$ độc lập thỏa mãn $E(X) = 5, V(X) = 3$ và $E(Y) = 4, V(Y) = 2$. Xét biến ngẫu nhiên $U = 3 X - 2 Y + 7$. Tính phương sai $V(U)$.
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
