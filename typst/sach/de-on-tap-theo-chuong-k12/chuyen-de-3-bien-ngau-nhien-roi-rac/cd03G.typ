...
#let mode = "loigiai"
#let accent = rgb("be123c") // Rose Pink
...
#show: thpt-school-exam.with(
  ...
  exam-title: "ĐỀ ÔN TẬP CHUYÊN ĐỀ 3G: BÀI TOÁN QUYẾT ĐỊNH KINH DOANH & QUẢN TRỊ RỦI RO BẰNG KỲ VỌNG",
  subject: "TOÁN",
...
// TN 1
#tn([Trong lý thuyết ra quyết định kinh doanh trong điều kiện rủi ro, tiêu chuẩn Giá trị tiền tệ kỳ vọng ($"EMV"$ - Expected Monetary Value) của một phương án hành động được tính bằng:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 2
#tn([Một doanh nghiệp xem xét hai phương án: Mở rộng nhà máy ($A_1$) hoặc Duy trì quy mô cũ ($A_2$). Dự báo thị trường năm tới có hai trạng thái: "Nhu cầu cao" (xác suất $p_1 = 0","6$) và "Nhu cầu thấp" (xác suất $p_2 = 0","4$). Kết quả lợi nhuận ròng (đơn vị: tỷ đồng) được cho trong bảng ma trận sau:
#align(center)[
  #table(
...
// TN 3
#tn([Trong sơ đồ Cây quyết định (Decision Tree), các nút hình vuông ($square$) và các nút hình tròn ($circle$) lần lượt đại diện cho:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 4
#tn([Giá trị kỳ vọng của thông tin hoàn hảo ($"EVPI"$ - Expected Value of Perfect Information) là số tiền tối đa mà nhà quản trị sẵn sàng chi trả để có được thông tin dự báo hoàn toàn chính xác trước khi ra quyết định. Nếu gọi $"EVwPI"$ là giá trị kỳ vọng khi có thông tin hoàn hảo và $"EMV"^*$ là giá trị tiền tệ kỳ vọng tối ưu khi không có thông tin, thì công thức xác định $"EVPI"$ là:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 6
#tn([Một công ty chế tạo máy đang đứng trước $3$ phương án sản xuất linh kiện $A_1, A_2, A_3$. Lợi nhuận kỳ vọng của các phương án được tính toán là $"EMV"(A_1) = 85$ triệu đồng, $"EMV"(A_2) = 110$ triệu đồng, và $"EMV"(A_3) = 95$ triệu đồng. Nếu áp dụng tiêu chuẩn tối đa hóa lợi nhuận kỳ vọng, nhà quản trị nên chọn phương án nào và mức lợi nhuận kỳ vọng đạt được là bao nhiêu?
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 7
#tn([Mối quan hệ giữa Tiêu chuẩn Tổn thất cơ hội kỳ vọng ($"EOL"$ - Expected Opportunity Loss) và Giá trị kỳ vọng của thông tin hoàn hảo ($"EVPI"$) tại phương án hành động tối ưu là:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// TN 11
#tn([Một nhà sản xuất bánh trung thu dự báo nhu cầu thị trường của một loại bánh đặc biệt có bảng phân bố xác suất như sau:
#align(center)[
  #table(
...
// TN 12
#tn([Một nhà đầu tư có thái độ "ngại rủi ro" (Risk-averse) được lựa chọn giữa hai dự án đầu tư $X$ và $Y$ độc lập. Cả hai dự án đều có cùng mức lợi nhuận kỳ vọng là $E(X) = E(Y) = 500$ triệu đồng. Tuy nhiên, phương sai lợi nhuận của hai dự án lần lượt là $V(X) = 400$ và $V(Y) = 900$. Quyết định hợp lý của nhà đầu tư này là:
#align(center)[
  #cetz.canvas({
    import cetz.draw: *
...
// DS 2
#ds([Một công ty dược phẩm đang phân tích dự án nghiên cứu một loại thuốc điều trị mới bằng Cây quyết định qua hai giai đoạn:
...
// TLN 4
#tln([Một sạp báo đặt mua một tờ tạp chí tuần với tỷ số chi phí tới hạn được xác định là $r = frac(C_u, C_u + C_o) = 0","65$. Nhu cầu hàng tuần của khách hàng đối với loại tạp chí này có hàm phân bố xác suất tích lũy $F(k) = P(X <= k)$ như sau:
