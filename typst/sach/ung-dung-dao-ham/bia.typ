...
// Có thể ghi đè bất kỳ biến màu nào từ _config.typ:
// #let c-book = rgb("#1e3a5f")  // ví dụ: tối hơn

// ─── Nền toàn trang ──────────────────────────────────────────────
#{
  place(top + left, dx: -2cm, dy: -1.8cm, rect(width: 210mm, height: 297mm, fill: c-book, radius: 0pt))
  // Dải màu trái
  place(top + left, dx: -2cm, dy: -1.8cm, rect(width: 7mm, height: 297mm, fill: c-p1, radius: 0pt))
  // Dải 5 màu dưới
  place(bottom + left, dx: -2cm, dy: 1.8cm, block(width: 210mm, height: 20mm)[
    #grid(
  ...
  align(center, {
    ...
    v(1.5em)
    // ── Công thức trang trí ──
    text(fill: white.transparentize(50%), size: 11pt)[
    ...
    block(
      ...
      radius: 12pt,
      {
        ...
        v(1.2em)
        text(size: 26pt, weight: "bold", fill: c-book)[ỨNG DỤNG THỰC TẾ]
        linebreak()
        text(size: 26pt, weight: "bold", fill: c-p1)[CỦA ĐẠO HÀM]
        v(0.7em)
        line(length: 100%, stroke: 1.5pt + c-p1.lighten(55%))
        v(0.6em)
        text(size: 11.5pt, style: "italic", fill: rgb("#475569"))[
          Mô hình hóa Toán học & Tư duy Conan Logic
        ]
        v(1.2em)
        // ── Nhãn phần ──
        grid(
    ...
    grid(
      ...
      box(
        ...
        #text(fill: white, weight: "bold", size: 17pt)[13]
        #linebreak()
