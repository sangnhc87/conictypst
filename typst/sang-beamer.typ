// =========================================================
// SANG-BEAMER.TYP v4.1 — Slide trình chiếu Premium
// ✅ Auto-counter đúng: step() NGOÀI context, get() TRONG context
// =========================================================
...
// Đặt lại counter: #bm-resetcau() hoặc #bm-setcau(13)
        ...
        grid(
          ...
          pad(left: 8pt)[#author — #institution],
          {
            ...
            for i in range(1, total-q + 1) {
              ...
              if has {
                dots.push(link(lbl, box(
                  width: 13pt, height: 13pt, radius: 50%,
                  fill: accent.lighten(70%), stroke: 0.5pt + accent,
                  align(center + horizon)[#text(size: 5.5pt, weight: "bold", fill: accent)[#i]],
                )))
              } else {
                dots.push(box(
                  width: 10pt, height: 10pt, radius: 50%,
...
#let exam-part(title, count: auto, reset-counter: true, accent: bm-colors.accent) = {
  slide(title: none)[
    #align(center + horizon)[
      #v(-1em)
      #block(width: 78%, stroke: (top: 2.5pt + accent, bottom: 2.5pt + accent), inset: (x: 0pt, y: 20pt))[
        #align(center)[
...
// ── MCQ ──────────────────────────────────────────────────
// FIX: step() đặt NGOÀI context, get() đọc TRONG context
#let mcq(
  stem,
  options,
  correct: (),
  ...
  let opt-oks = options.enumerate()
    .map(((i, o)) => if type(o) == dictionary { o.at("correct", default: false) } else { correct.contains(i + 1) })

  // ✅ Bước 1: step/update NGOÀI context (đặt vào document flow)
  if num == auto {
    _bm-q-cnt.step()
  } else {
  ...
  // ✅ Bước 2: đọc giá trị TRONG context (thấy được step ở trên)
  context {
    let q-num = _bm-q-cnt.get().first()

    // SLIDE 1: Câu hỏi
    slide(title: none)[
      #[#metadata("q") #label("bm-q-" + str(q-num))]
      #v(-0.5em)
      #grid(
        columns: (auto, 1fr), column-gutter: 10pt,
        align: (left + top, left + top),
        box(fill: accent, inset: (x: 10pt, y: 6pt), radius: 4pt)[
          #text(weight: "bold", fill: white, size: 16pt)[#prefix #q-num]
        ],
        text(size: 17pt, fill: white)[#stem],
      )
      #if fig != none { v(0.3em); align(center, block(width: fig-width)[#fig]) }
      #v(0.5em)
      #grid(
        columns: (1fr, 1fr), row-gutter: 10pt, column-gutter: 12pt,
        ..opt-texts.enumerate().map(((i, t)) => {
          box(width: 100%, radius: 8pt, fill: card-fill, stroke: card-stroke)[
            ...
            ][#pad(x: 12pt, y: 10pt)[#text(size: 15pt, fill: white)[#t]]]
          ]
        })
      )
    ]
      ...
      slide(title: none)[
        ...
        #grid(
          ...
          box(fill: bm-colors.correct, inset: (x: 10pt, y: 6pt), radius: 4pt)[
            #text(weight: "bold", fill: white, size: 16pt)[#prefix #q-num — ĐÁP ÁN]
          ],
          [],
        )
        #v(0.4em)
        #grid(
          ...
          ..opt-texts.enumerate().map(((i, t)) => {
            ...
            box(width: 100%, radius: 8pt, fill: bg, stroke: brd)[
              #grid(columns: (auto, 1fr), column-gutter: 0pt)[
              ...
              ][#pad(x: 12pt, y: 10pt)[#text(size: 15pt, fill: if ok { white } else { white.darken(10%) })[#t]]]
            ]
          })
        )
        #if loigiai != none {
          v(0.5em)
          block(width: 100%, fill: rgb("#0c1a3a"), stroke: (left: 3pt + bm-colors.gold),
...
#let tf(
  ...
  context {
    ...
    slide(title: none)[
      ...
      #v(-0.5em)
      #box(fill: accent, inset: (x: 10pt, y: 6pt), radius: 4pt)[
        #text(weight: "bold", fill: white, size: 16pt)[#prefix #q-num — Đúng/Sai]
      ]
      #v(0.3em)
      ...
      #table(
        ...
        ..statements.enumerate().map(((i, s)) => {
          ...
          (
            table.cell(fill: row-bg, pad(x: 10pt, y: 7pt)[
              #text(weight: "bold", fill: accent, size: 13pt)[#alpha.at(i))]
              #h(5pt)
              #text(size: 13pt, fill: white)[#txt]
            ]),
            table.cell(fill: row-bg, align(center)[#box(width: 1.8em, height: 1.8em,
              stroke: 0.6pt + accent.lighten(40%), radius: 3pt, fill: bm-colors.bg)]),
            table.cell(fill: row-bg, align(center)[#box(width: 1.8em, height: 1.8em,
              stroke: 0.6pt + accent.lighten(40%), radius: 3pt, fill: bm-colors.bg)]),
          )
        }).flatten(),
        ...
        #table(
          ...
          ..statements.enumerate().map(((i, s)) => {
            ...
            (
              table.cell(fill: row-bg, pad(x: 10pt, y: 7pt)[
                #text(weight: "bold", fill: accent, size: 13pt)[#alpha.at(i))]
                #h(5pt)#text(size: 13pt, fill: white)[#txt]
              ]),
              table.cell(fill: fd, align(center)[#box(width: 1.8em, height: 1.8em,
                stroke: 0.6pt + bm-colors.muted, radius: 3pt, fill: fd, align(center + horizon)[#md])]),
              table.cell(fill: fs, align(center)[#box(width: 1.8em, height: 1.8em,
                stroke: 0.6pt + bm-colors.muted, radius: 3pt, fill: fs, align(center + horizon)[#ms])]),
            )
          }).flatten(),
        )
        #if loigiai != none {
...
#let short(
  ...
  context {
    ...
    slide(title: none)[
      ...
      #v(-0.5em)
      #box(fill: accent, inset: (x: 10pt, y: 6pt), radius: 4pt)[
        #text(weight: "bold", fill: white, size: 16pt)[#prefix #q-num — Trả lời ngắn]
      ]
      #v(0.4em)
      #text(size: 17pt, fill: white)[#stem]
      #if fig != none { v(0.3em); align(center, fig) }
        ...
        align(center)[
          ...
          #stack(dir: ltr, spacing: 4pt,
            ..range(box-count).map(_ => box(width: 2em, height: 2em,
              stroke: 1.2pt + bm-colors.gold, radius: 4pt, fill: bm-colors.card)))
        ]
      }
    ]
...
#let tl(
  ...
  context {
    ...
    slide(title: none)[
      ...
      #v(-0.5em)
      #box(fill: accent, inset: (x: 10pt, y: 6pt), radius: 4pt)[
        #text(weight: "bold", fill: white, size: 16pt)[#prefix #q-num — Tự luận]
      ]
      #v(0.4em)
      #text(size: 17pt, fill: white)[#stem]
      #if fig != none { v(0.3em); align(center, fig) }
      #if show-boxes {
        v(0.7em)
        block(width: 100%, fill: bm-colors.card, stroke: 1pt + accent.lighten(30%),
          inset: (x: 14pt, y: 12pt), radius: 8pt)[
    ...
    if mode == "loigiai" and loigiai != none {
      slide(title: none)[
        #v(-0.5em)
        #box(fill: bm-colors.correct, inset: (x: 10pt, y: 6pt), radius: 4pt)[
          #text(weight: "bold", fill: white, size: 16pt)[#prefix #q-num — LỜI GIẢI]
        ]
        #v(0.6em)
        #block(width: 100%, fill: rgb("#0c1a3a"), stroke: (left: 3pt + bm-colors.gold),
          inset: (x: 14pt, y: 10pt), radius: (right: 6pt))[
